...
- `points_to_remember` = optional "why", same convention as mcq/true_false/correct_incorrect.
...
- **Bulk upload is explicitly deferred** (Sprint 7.8-C) — the variable-length left/right/mapping shape doesn't fit the flat `option_1..4`/`correct_option` CSV convention mcq/true_false/correct_incorrect use, and a matching-question set is lower-volume, more bespoke authoring. Manual authoring (`FlashcardCreate.jsx`) only.
