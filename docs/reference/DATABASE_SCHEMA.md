...
- **Bulk upload is explicitly deferred** (Sprint 7.8-C) — the variable-length left/right/mapping shape doesn't fit the flat `option_1..4`/`correct_option` CSV convention mcq/correct_incorrect use, and a matching-question set is lower-volume, more bespoke authoring. Manual authoring (`FlashcardCreate.jsx`) only.
...
**Constraint — `study_sessions_duration_floor` (Sprint 8.6a):** `CHECK (duration_seconds >= 600) NOT VALID`. Raises the loggable-session floor from 10 seconds to 10 minutes, enforced server-side (mirrors `StudyTimerContext.jsx`'s `stopAndLog()`, which now returns a `'too_short'` outcome and never issues the insert below this threshold — the constraint is the same integrity backstop as `study_sessions_manual_requires_category` above, for the same reason: nothing but the frontend stopped a future write path from inserting under-threshold rows). `NOT VALID` means prospective only — pre-existing sub-threshold rows (logged when the floor was 10 seconds) are never invalidated or rewritten.
