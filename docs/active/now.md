...
- **⏳ Follow-ups:** push to `main` → live-verify the authed nav on `https://www.revisop.com` (desktop + mobile; nested-route active-state check; re-shoot dashboard/note-detail/study-loop against the baselines for the "non-chrome unchanged" check). Student-session eyeball recommended in addition to the super_admin pass. 6.3 = dashboards + dropdown-surface atom family; 6.4 = study loop + Literata + `AnswerOption` green removal + the three carried items.
