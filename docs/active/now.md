...
- **⏳ Follow-ups before Sprint 7.4:** (1) operator `git commit` + push (Sprint 7.2 is also still unpushed — bundle or sequence both), (2) a manual click-through of the mobile ＋ sheet (Bulk Upload absent, Log Study Time present) and the desktop Create dropdown, since this session's click tooling hiccupped there, (3) a real stale-session test (fake a >4h-old `revisop_manual_timer_started_at` value, reload a non-study-time page, confirm the recovery prompt / discard fires app-wide), (4) a two-tab cross-sync check.
...
- **⏳ Follow-ups before Tuesday:** (1) operator `git commit` + push (see command below), (2) a quick manual click-through of ＋ → Create Group and the merged bell's friend-request accept/decline on a real pending request, (3) optional: a proper network trace of the professor dashboard's 4 RPCs on `revisop.com` post-push.
