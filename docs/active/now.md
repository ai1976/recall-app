...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.6 (Browse/My Study Sets rename + question-type filter) ✅ COMPLETE, live-verified against production, NOT YET COMMITTED.** Sprint 7.5 (MCQ-single vertical slice) is ✅ **committed** (`797629d`, on `main`) — this session's git log confirmed it landed, correcting this file's own prior "NOT YET COMMITTED" note for 7.5, which had gone stale (the commit happened after that note was written and was never updated). Sprint 7.6 sits on top of `797629d`: `get_browsable_decks` v5 (`p_question_type` filter) SQL deployed & verified live, including a real DROP-before-CREATE gotcha hit and fixed mid-sprint; frontend rename + filter built, `npm run build`/`npx eslint` clean, live-verified against production via a real student account (TestOutlook) — nav labels, page headings, filter narrowing (464→3 cards on MCQ), filtered-deck study session unaffected, `clearAllFilters()` all confirmed. → **Next: git commit + push Sprint 7.6, then Sprint 7.7** (remaining-question-type partition — filtering WITHIN a study session, out of scope for 7.6). ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
...
- **⏳ Not yet committed** — awaiting `git commit` + push, then Sprint 7.7.
