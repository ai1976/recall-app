...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.12 (`concept_card` structured authoring + browse-only viewer) — closes the Phase 7 question-type roster. get_browsable_decks v6 SQL deployed & verified (5/5 PASS), frontend live-verified end to end — ready to commit.** Also closed a real gap: `StudyMode.jsx`'s "never-reviewed → new card" fallback filter didn't check `question_type`, so a never-reviewed concept_card could have reached a student despite `get_study_queue` already excluding it from the due-set (D-06) — confirmed via pre-flight that this never actually happened to real content, fixed regardless. Sprint 7.11 (`fitb` authoring + confidence-gated grading) sits underneath, ✅ zero SQL needed, frontend live-verified end to end — ready to commit; see its own section below. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
