...
**Second pre-8.8.3 correction (26/09/2026):** D-33's Community bullet wrongly told the *desktop* rail to preserve `Following.jsx`'s second entry point via `NavMenuSheet.jsx` — that file is mounted only inside `NavMobile.jsx` (verified against the component tree; `NavDesktop.jsx` never references it), so that path is mobile-only and desktop never had it. Corrected in place: desktop Tier-3 Community needs no Following-specific action (bell only, unchanged); preserving the `NavMenuSheet.jsx` path is Sprint 8.8.4's job. No IA decision changed — Following's "retain both entry points" decision still stands, just correctly attributed per platform.
...
**Real bug found and fixed during this scope:** `PracticeMode.jsx`'s `renderAddControl` treated `card.is_own` as equivalent to "already enrolled" — true before Scope A, false after (own cards now need enrollment too). A "Save only"-created own card was showing "Already in My Cards" in Practice Mode with no way to actually enroll it. Fixed by gating on `card.is_enrolled` alone (`can_add_to_my_cards` is always true for an own card, so the fix correctly falls through to the Add button). Live-verified: created a fresh own unenrolled card, confirmed Practice Mode showed "Add to My Study," clicked it, confirmed enrollment succeeded and the card appeared in My Study as New.

**Live acceptance testing — ✅ all passed 25/09/2026** (1 test card created + fully deleted, 0 residue; one real existing card removed-then-re-added to test the round trip, confirmed restored to its exact prior "Active" state): grouping/counts correct against real 34-card data; topic expand/collapse; Remove → moves to History → Removed with correct own-content-specific dialog copy; re-add from History → returns to Working with prior SRS state (not reset to New); Mastered/Removed correctly separated in History; the `is_own` Practice Mode bug fix.

**Sprint 8.7.10 is now fully complete — Scope A, B, C, and D all shipped and live-verified.** Out of scope entirely, per the sprint's own instruction: Library, Marketplace, Billing, Purchases, 100-Day Challenge.

**Files changed (Scope D):** `src/pages/dashboard/Study/MyCards.jsx` (rewritten), `src/pages/dashboard/Study/PracticeMode.jsx`, `src/components/layout/NavDesktop.jsx`, `src/components/layout/NavMenuSheet.jsx`. SQL: `docs/database/sprint8.7.10/13_FUNCTIONS_get_removed_my_cards.sql` (new, additive, deployed).

---

## Sprint 8.7.10 Scope A–C recap (see above for Scope D)
...
**DB diagnostic — ✅ run and confirmed clean 24/09/2026:** no length limit on `front_text`/`back_text`/`scenario`, no CHECK constraint touches them, no trigger validates/rewrites their content, and the RLS professor/admin gate (`flashcards_gate_verdict_types_insert/update`) does not apply to `theory` — a theory scenario needs no elevated permission. Full detail in blueprint.md D-31.

**Stage 8 live proof, Fixtures A–E — ✅ COMPLETE 24/09/2026, cleanup fully verified 0 residue.** Real bulk-upload path, 5 disposable private `case_study_mcq` cards, verified in Practice Mode (A/B/C/D/E) and Study Mode desktop+mobile (C). All tables render in order, empty/trailing-empty cells preserved, roman-numeral/bullet lists stay plain, escaped pipe stays one cell, plain-text regression baseline unchanged, mobile tables scroll locally without page overflow, zero console errors. Cleanup: flashcards fully deleted (0 residue, Dashboard stats back to 464); the 5 orphaned `flashcard_batch_provenance` rows (RLS blocks client-side delete on that table by design) removed via `docs/database/sprint8.7.9/02_CLEANUP_remove_disposable_proof_provenance.sql`, re-verified `remaining_orphan_provenance = 0`.

**Stage 8 live proof, records F–I — ✅ COMPLETE 25/09/2026.** Corpus thread supplied exact row/cell references; locked text pulled live from `Question_Master`/`Integrated_Cases`, not reconstructed. Two real bugs found and fixed along the way:
1. **Corpus defect (record G):** the source PDF has a genuine spanning "REPORT" caption row above the real two-column table headers — the `[[TABLE]]` grammar has no representation for that, so it got encoded as a bogus first data row (`REPORT | `), breaking the parser's header invariant. Fixed by dropping the caption from `Question_Master!Q142` (user's call, made live in the sheet, verified after).
2. **Client bug (all of F/G/H/I, unrelated to corpus):** `BulkUploadFlashcards.jsx` was collapsing every newline in `front`/`back` to a space before storage — a pre-existing normalization step that silently destroyed `[[TABLE]]` structure. `scenario` was unaffected (trim-only), which is why A–E's proof never caught this. Fixed: `front`/`back` now just `.trim()`, matching `scenario`. Re-uploaded after the fix; newlines and table markers now survive.

Live-verified Practice Mode desktop+mobile for F (front table), G (both answer tables, no REPORT artifact), H (theory + collapsible scenario, no duplication into front_text), I (scenario collapsible on a real MCQ card). Scenario equality across all 4 MCQ rows confirmed via strict `===`, not just length. Cleanup: 7 flashcards deleted (0 residue), Dashboard stats back to baseline (464 cards, 2418 reviews). 2 orphaned provenance rows removed via `docs/database/sprint8.7.9/03_CLEANUP_remove_disposable_proof_fghi_provenance.sql`, re-verified `remaining_orphan_provenance = 0`. Sprint 8.7.9 cleanup fully closed, 0 residue anywhere.

**Sprint 8.7.9 is now fully complete** — Stage 8 live proof done for all mandatory fixtures/records (A–E, F–I). Do not start Stage 9 corpus regeneration until the corpus team has acted on the Stage 7B-superseded note in blueprint.md D-31.
...
**Subsequently live-verified (post-closeout, production, revisop.com):**
- **Remove → re-add round trip:** added "Section 80C" (external) to My Cards from Practice → appeared Active/Pause+Remove → Remove → disappeared, toast confirmed → reopened the same card in Practice → correctly addable again (not stuck as "Already in My Cards") → re-added → reappeared in My Cards as Active, `reviews.status` preserved (not reset), confirming `remove_from_my_cards`/`add_to_my_cards`'s coupled suspend/unsuspend semantics hold end-to-end in production.
- **Mastered exclusion:** this account's only real mastered card is a pre-D-27 orphaned `reviews` row that correctly never appears in My Cards, so no live mastered card existed inside My Cards to click through. Used a disposable SQL fixture (`docs/database/sprint8.7.8d/01_TEST_mastered_pause_exclusion_fixture.sql`, run by Anand) to temporarily flip the "Section 80C" card's `reviews.status` to `mastered` — confirmed live on `/dashboard/my-cards`: shows a "Mastered" badge, **no Pause, no Resume**, "Remove" still available (external card). Reverted immediately after, confirmed both affected rows back to `active`, zero residue.
...
**Follow-up (Anand's review):** the onboarding flow described creating content as the only way in, never mentioning that a course may already have professor/official-body content to browse and add before a student creates anything. Added a new onboarding step to both Guide's "I just signed up" situation ("Check what is already there to browse" — Browse Study Sets → Practice → Add to My Cards, before "Add your first content") and Help's "Your First Steps" (inserted the same Browse-first framing as its own bullet, ahead of upload/create). Live-verified in browser: Help's new/updated sections render and expand correctly with the right icon, search surfaces the updated FAQ, Student Guide renders all corrected passages including the new onboarding step, no console errors.
...
**This session had no Supabase SQL access or test-account credentials of its own** — every SQL file was written and reviewed here, then run by the operator directly in the Supabase SQL Editor, with results pasted back and reconciled turn-by-turn. `npx eslint`/`npx vite build` were the only checks this session could run unassisted.

**Real bug found live, not caught by review — `get_browsable_decks` v7's first deploy was broken:** the original `sole_batch_id` expression used `min(fc.batch_id)`, but Postgres has no default ordering operator class for `uuid`, so `min()`/`max()` over a uuid column raises `42883: function min(uuid) does not exist`. The `DROP FUNCTION` + `CREATE` itself succeeded (no syntax error), so the function existed but **every call to `get_browsable_decks()` failed** until the operator's live T4 run surfaced it. Fixed by replacing `min(fc.batch_id)` with `(array_agg(fc.batch_id))[1]` — safe specifically because the surrounding `CASE` already gates on `count(DISTINCT fc.batch_id) = 1`, so every element of that array is identical. Corrected file re-run by the operator; all subsequent checks passed. See `docs/database/sprint8.7.4/02_FUNCTIONS_get_browsable_decks_v7_provenance.sql`'s header for the full note.

**Live verification results (operator-run, 18/09/2026):**
- **T1 PASS** — exactly one SELECT policy on `flashcard_batch_provenance`, correctly named.
- **T2/T3/T7 — ⚠️ NOT independently confirmed, correcting an earlier overclaim.** These are `RAISE NOTICE`-based checks (Messages-tab output, not a result grid) — the operator ran Block 1 and it did not error out, but the actual NOTICE text (`T2: ...`, `T3a/T3b PASS: ...`, `T7 PASS: ...`) was never pasted back to this session, and I inferred "PASS" from the absence of a reported problem rather than from seeing the text. That is not the same as verified evidence — flagging this explicitly rather than continuing to assert it. T1 (a real SELECT row) and T4-T6 (real SELECT rows, described below) remain genuinely confirmed. **Partial independent corroboration exists via the live app pass:** the §8 reconciliation below did exercise authenticated reads succeeding (T2's actual capability) through real app usage (badges rendering), but did not exercise T3 (write-still-denied) or T7 (anon-still-denied) at the SQL layer — T7's *product-level* equivalent (anonymous preview pages) was separately verified in §8/Omitted-surface reconciliation below, which is not the same test as T7's `flashcard_batch_provenance`-table-specific anon check.
- **T4 PASS** (post-fix) — `get_browsable_decks()` returns `provenance_source_type`/`provenance_source_name` correctly: exactly 1 of 20 sampled decks shows real provenance (`original_creator` / "Sprint 8.7.2 verification" — leftover single-card test data from 8.7.2's own live verification, correctly a single-batch deck), the other 19 correctly `NULL` (legacy or multi-batch decks) — confirms the "unambiguous single-batch only" design rule is working as designed, not just as written.
- **T5 PASS** — `get_browsable_notes()` returns the two new columns; all 20 sampled rows `NULL`, expected since `NoteUpload.jsx`'s provenance requirement only went live today (8.7.3) and none of this student's browsable notes postdate it.
- **T6 PASS** — `get_study_queue()` returns `batch_id` for every due card; spot-checked against `flashcards.batch_id` directly (`flashcard_id = 2b5ee690-...` → `batch_id = 9e723509-9ad8-4c86-bf05-fcfcecf5b0ce` on both sides, exact match).

**Pre-flight diagnostic (Step 0) also reconciled live before any change:** zero SELECT policies existed on `flashcard_batch_provenance` (as assumed), all three RPC signatures matched the `DROP FUNCTION` statements exactly, `notes.content_source_type`/`content_source_name` confirmed nullable text, and **7 live provenance rows already existed** pre-deploy (from 8.7.1-8.7.3's own verification runs) — meaning badges had real data to render against immediately, not just a theoretically-correct empty table.

**Step 0 diagnostic (`docs/database/sprint8.7.4/00_DIAGNOSTIC_pre_display.sql`, ✅ RUN, reconciled clean):** confirmed — not assumed — three things before any change: `flashcard_batch_provenance` had zero SELECT policies (matched); the exact live signatures of `get_browsable_decks`/`get_browsable_notes`/`get_study_queue` matched the `DROP FUNCTION` statements exactly; `notes.content_source_type`/`content_source_name` confirmed nullable text. Bonus finding: 7 live provenance rows already existed pre-deploy.

**Read-surface map (Step 0's actual deliverable — every flashcard/note display surface, traced by actual query mechanism, not assumed):**
| Surface | Mechanism | Auth context |
|---|---|---|
| `MyFlashcards.jsx` (grouped view) | Direct `.from('flashcards')`, batch-grouped | Authenticated |
| `StudyMode.jsx` (direct deck click) | Direct `.from('flashcards')`, `SELECT *` | Authenticated |
| `StudyMode.jsx` (via `ReviewSession.jsx`) | RPC `get_study_queue` | Authenticated |
| `NoteDetail.jsx` (note + its linked flashcards) | Direct `.from('notes')` / `.from('flashcards')` | Authenticated |
| `MyNotes.jsx` | Direct `.from('notes')`, `SELECT *` | Authenticated |
| `BrowseNotes.jsx` | RPC `get_browsable_notes` | Authenticated |
| `ReviewFlashcards.jsx` ("Browse Study Sets") | RPC `get_browsable_decks` | Authenticated |
| `ReviewBySubject.jsx` | Direct, subject-group counts only — no card content, no provenance need | Authenticated |
| `DeckPreview.jsx` (`/deck/:id`) | RPC `get_public_deck_preview` | **Public/anonymous** |
| `NotePreview.jsx` (`/note/:id`) | RPC `get_public_note_preview` | **Public/anonymous** |

**Omitted-surface reconciliation (per auditor request — each omission answered explicitly, not just asserted "scope cut"):**

| Surface | Shows real content? | Auth context | Provenance shown? | Reasoning | Authorized by? |
|---|---|---|---|---|---|
| `DeckPreview.jsx` (`/deck/:id`) | **Yes** — real `front_text` of up to 5 cards from a genuinely public deck | **Anonymous** | **No** | Not in the sprint brief's Display section's named surface list (item 2 names `MyFlashcards.jsx`/`StudyMode.jsx`/`ReviewFlashcards.jsx`/`MyNotes.jsx`/`NoteDetail.jsx` only). Adding it means solving the same deck-level batch-ambiguity problem a second time against a *different* RPC (`get_public_deck_preview`, which doesn't currently expose `batch_id` per preview item at all) on an anonymous-facing conversion surface — meaningfully more scope, on a higher-risk surface, than the sprint as briefed. | **Not by an existing named decision — this was my own implementation-time scope call, not previously surfaced for sign-off.** Flagging now: I recommend treating this as a candidate for a small, separately-scoped follow-up (its own Step 0 + live verification), not silently adding it under this audit. Live-verified anonymously (fresh, localStorage-cleared session): renders correctly, zero console errors, no accidental provenance query. |
| `NotePreview.jsx` (`/note/:id`) | **No** — title/subject/topic/author/description metadata only; the "content" shown is a decorative blurred placeholder (`lorem`-style gray bars), never real note text | **Anonymous** | **No** | There is no real content surface here to attach provenance to — this is the more clear-cut of the two public pages, not a close call. | Same reasoning as above; not a named decision, but a materially easier call since no real content is displayed at all. Live-verified anonymously: renders correctly, zero console errors. |
| `MyFlashcards.jsx` ungrouped grid view | **Yes** — same card data as the grouped view | **Authenticated**, owner's own dashboard | **Now yes — fixed** | This one had no defensible reasoning to omit — same data, same user, same page, badge already present one view-toggle away. Originally cut as a documented "scope cut," but on reconciliation this was really an oversight, not a product decision. **Fixed during this audit pass** (both the flat-grid call site and a second, initially-missed `<FlashcardCard>` call site inside the grouped view's per-card grid). | N/A — corrected, not deferred. |
| `ReviewBySubject.jsx` | **No** — subject name + due-count only; delegates all actual card rendering to `ReviewSession.jsx` → `StudyMode.jsx` | Authenticated | **No** | Confirmed by re-reading the full render body: no `front_text`/`back_text`/card content anywhere in this component. It is a launcher, not a content surface. | Not a scope cut at all — there is no content here to badge. |

**Public preview anonymous-regression check — actually run, not just asserted from "untouched" (per auditor request):** opened both `/deck/:id` (a real public deck) and `/note/:id` (a real public note) in a **fresh browser tab with `localStorage`/`sessionStorage` cleared** (genuine anonymous state, confirmed via the nav bar showing "Log in"/"Sign up free", not a redirect). Both rendered correctly: `DeckPreview.jsx` showed the real question text with no provenance badge and no errors; `NotePreview.jsx` showed its metadata block + blur placeholder with no errors. `read_console_messages` showed zero errors on either page.

**1. Read policy (`docs/database/sprint8.7.4/01_SCHEMA_provenance_select_policy.sql`, ✅ deployed, ✅ T1 confirmed, ⚠️ T2/T3/T7 not independently confirmed — see §7/§8 correction):** single unconditional `authenticated_read_flashcard_batch_provenance` policy, `FOR SELECT TO authenticated USING (true)` — per the sprint brief's own recommendation, not an attempt to mirror flashcards' private/friends/public visibility inside a subquery against the provenance table. Provenance is a source type + name, not the flashcard content itself; mirroring full visibility logic here is real complexity (a place to re-litigate D-04 and introduce a subtle bug) for a privacy gain that doesn't exist. Does not touch `flashcards`' own RLS. `anon` still has zero grant, unchanged from 8.7.1.

**2. Display — flashcards (batch-level, via `src/lib/provenance.js`'s `fetchBatchProvenanceMap` + new `src/components/content/ProvenanceBadge.jsx`):**
- `MyFlashcards.jsx` — badge on each batch-group header (grouped view only; the separate ungrouped grid view was left uncovered, a scope cut, not an oversight — the grouped view is this page's batch-oriented default and the only place the data maps naturally to one badge per group).
- `StudyMode.jsx` — badge above the card, keyed off `currentCard.batch_id`, refetched only when the session's distinct batch-id set actually changes (not on every reshuffle).
- `NoteDetail.jsx` — badge on each linked flashcard's "Front (Question)" row.
- `ReviewFlashcards.jsx` ("Browse Study Sets") — deck-tile badge, sourced from `get_browsable_decks` v7's new `provenance_source_type`/`provenance_source_name` columns (see SQL below), **not** a second client-side query — deck-level provenance requires server-side aggregation across a deck's cards, which a client `.in('batch_id', ...)` join can't resolve unambiguously on its own.

**3. Display — notes (row-level, no batch ambiguity — a note IS its own provenance unit):**
- `MyNotes.jsx` / `NoteDetail.jsx` (the note itself) — `note.content_source_type`/`content_source_name` were already fetched by both pages' existing `SELECT *`; this sprint only added the badge render, no query change.
- `BrowseNotes.jsx` — sourced from `get_browsable_notes` v4's two new passthrough columns (see SQL below).

**4. `get_browsable_decks` v7 (`docs/database/sprint8.7.4/02_FUNCTIONS_get_browsable_decks_v7_provenance.sql`, ✅ deployed & verified — T4 PASS post-fix) — deck-level provenance is genuinely ambiguous, judgment call flagged explicitly:** a "deck" is a 5-grouping-column bucket, not a batch — it routinely spans many `batch_id`s (a student adds cards to the same subject/topic across many sessions). Showing one badge for a multi-source deck would misattribute cards that don't share that source. Rule applied: the deck only gets a badge when **every** visible card shares exactly one `batch_id` (computed via `CASE WHEN count(DISTINCT fc.batch_id) = 1 THEN (array_agg(fc.batch_id))[1] ELSE NULL END` inside the existing per-viewer visibility lateral, then `LEFT JOIN flashcard_batch_provenance`). Any multi-batch or legacy deck returns `NULL` for both new columns — same "render nothing" rule as everywhere else in D-21, not an invented "mixed sources" treatment. **Real deployment bug, see above:** the first version used `min(fc.batch_id)`, which doesn't compile against `uuid` at runtime (`42883`) — fixed to `array_agg()[1]`, re-deployed, live-verified correct against real deck data (1/20 sampled decks showed real provenance, 19/20 correctly NULL). DROP+CREATE required (return-shape change, same lesson as v5/v6).

**5. `get_browsable_notes` v4 (`docs/database/sprint8.7.4/03_FUNCTIONS_get_browsable_notes_v4_provenance.sql`, ✅ deployed & verified — T5 PASS) — straightforward passthrough, no ambiguity.** One incidental fix found while reproducing this function for the DROP+CREATE the return-shape change requires: v3 had **no `SET search_path` clause at all** (every other `SECURITY DEFINER` function in this codebase pins it, per the L3 `17c` outage lesson) — v4 adds `SET search_path TO public, extensions`, closing that gap rather than reproducing a known-risky omission verbatim.

**6. `get_study_queue` batch_id addition (`docs/database/sprint8.7.4/04_FUNCTIONS_get_study_queue_batch_id.sql`, ✅ deployed & verified — T6 PASS, batch_id spot-checked against flashcards.batch_id) — found via Step 0, not assumed:** `ReviewSession.jsx` feeds `StudyMode.jsx` through this RPC, and its live return shape (confirmed by reading `docs/database/srs-ladder/02_FUNCTIONS_srs_ladder_engine.sql`, the function's actual current source) had no `batch_id` at all. Without this, cards studied via the normal "Review due cards" flow — the most common entry point — would never show a badge even when provenance exists, while cards reached via a direct deck click would. Added `batch_id uuid` to `RETURNS TABLE`, additive/non-breaking, grants (`REVOKE ALL FROM PUBLIC, anon` / `GRANT EXECUTE TO authenticated`) reproduced exactly since `DROP FUNCTION` strips them.

**7. Test SQL (`docs/database/sprint8.7.4/05_TEST_verify_sprint8.7.4.sql`, ✅ RUN — T1 confirmed via its result row; T2/T3/T7 ran without the operator reporting an error but their actual NOTICE text was never captured back to this session, so they are NOT independently confirmed (correcting an earlier overclaim — see the T2/T3/T7 note in §8 above); T4/T5/T6 confirmed via standalone impersonated SELECTs returning real rows):** T1-T3/T7 are self-contained `BEGIN...ROLLBACK` role-impersonation checks (this project's established idiom — `SET LOCAL ROLE authenticated` + a real `request.jwt.claims`, not just the claim alone, per the sprint8.6c postmortem on why claim-only impersonation gives false passes against the SQL Editor's superuser connection). T4-T6, originally documented as app/PostgREST-only checks, turned out to be runnable directly via the same impersonation idiom (`SET LOCAL ROLE authenticated` + matching `request.jwt.claims` also satisfies `auth.uid()` inside these SECURITY DEFINER RPCs) — each run standalone (not batched with the NOTICE-based checks) so the operator could see the actual result rows in the SQL Editor's grid.

**8. Reconciliation pass (sprint brief §3) — ✅ COMPLETE, live-verified via real TestOutlook student session (dev server → live Supabase, 18/09/2026):**
- **(c) No anonymous-visitor regression** — satisfied by the scope decision above (both public preview files untouched, confirmed via `git status`).
- **(a) One flashcard batch, one bulk upload, one note, created in sequence, same session — no cross-path regressions:**
  - Manual flashcard (`FlashcardCreate.jsx`) — created with `original_creator` / "Sprint 8.7.4 verification" source. Badge confirmed rendering correctly on `MyFlashcards.jsx` (grouped view, batch header) and `StudyMode.jsx` (per-card, updates correctly across cards in the same session).
  - Bulk upload (`BulkUploadFlashcards.jsx`, 2 cards via synthetic `File`/`DataTransfer` injection — Browser pane has no native file-picker automation, same limitation noted in every prior sprint) — created with `original_creator` / "Sprint 8.7.4 bulk verification". Badge confirmed on `MyFlashcards.jsx`. **Incidental re-confirmation of the 8.3 bugfix:** the first CSV attempt used `question_type: flashcard` literally and was correctly rejected (`Row 2/3: question_type 'flashcard' isn't supported for bulk upload yet`) — `flashcard` isn't itself in `RECOGNIZED_QUESTION_TYPES`, blank is required to get the default. Corrected and re-uploaded successfully.
  - Note (`NoteUpload.jsx`, synthetic 1x1 PNG image injection) — created with `original_creator` / "Sprint 8.7.4 note verification". Badge confirmed on both `MyNotes.jsx` and `NoteDetail.jsx`.
  - **This also produced a genuine live test of the deck-level ambiguity rule, not just the diagnostic sample:** the manual card + bulk batch + the pre-existing 8.7.2 test card all share the same subject/topic ("Financial Reporting > Accounting & Technology"), so that deck now spans 3 distinct `batch_id`s. Browse Study Sets (`get_browsable_decks`) correctly shows **no** provenance badge on that deck tile — confirmed both before and after adding the new batches, live, not inferred.
  - **Legacy-content regression check, live:** `MyNotes.jsx` correctly shows no badge on the pre-existing "Seed Note for Screenshots" (NULL provenance) sitting right next to the new badged note in the same list.
- **(b) T7-equivalent (RPC privilege-escalation guard) re-confirmed post-sprint, via a direct PostgREST call** (anon key + the TestOutlook session's own access token, same technique 8.7.2/8.7.3 used) **to `create_flashcard_batches` with `question_type: 'mcq'`:** rejected with the exact same `P0001` / `"Only professors/admins may author verdict-bearing question_type \"mcq\""` message as before this sprint — confirms none of 8.7.4's read-only RPC changes touched the creation-path authorization logic (expected, since they're separate functions, but now verified rather than assumed).

**Test data cleanup — ✅ RUN and re-verified at 0 rows, all three counters confirmed by the operator (not inferred):** the Browser pane's sandbox suppresses the native `confirm()` dialog the in-app delete buttons rely on (same limitation as sprint8.6c) — `docs/database/sprint8.7.4/06_CLEANUP_remove_verification_test_data.sql` run by the operator instead. Final verification query results: `remaining_flashcards = 0`, `remaining_notes = 0`, `remaining_provenance = 0`. The test note's synthetic 1x1 PNG image in Storage was not cleaned up by this script (see the file's own trailing comment) — negligible (~100 bytes), left as documented residue rather than chased down.

**D-21 status: FULLY re-confirmed accurate post-8.7.4, live-verified end to end.** All three creation paths (manual flashcard, bulk upload, note) correctly declare and display provenance; the D-10 privilege gate is unaffected; the deck-level ambiguity design holds under genuine multi-batch live data, not just the diagnostic sample.

**Known limitations, found by code-review, deliberately deferred (not silently dropped):**
- **`handleMergeBatches` (`MyFlashcards.jsx`) doesn't reconcile `flashcard_batch_provenance` on merge.** Merging batch A into batch B reassigns `flashcards.batch_id` for A's cards to B's `batch_id`, but never touches the provenance table — post-merge, A's cards silently display B's provenance label (or none), a misattribution that existed before this sprint with no visible effect and is now visible via the badge. Pre-existing gap this sprint's display surfaces, not a regression this sprint introduced — but a proper fix requires a product decision (does a merge preserve the source cards' own provenance per-card? require re-declaration? show "mixed"?) out of scope for a display-only sprint. Flagged for a future sprint, not fixed here.
- **`05_TEST_verify_sprint8.7.4.sql`'s T4/T5/T6 are documentation-only, not executable automated tests** (SQL comments describing what to check via the app/PostgREST, since the RPCs `RAISE EXCEPTION 'Not authenticated'` outside a real session). They were, in fact, run — as standalone impersonated `SELECT`s, documented in §8 above — but the test *file itself* doesn't encode that path, so a future maintainer re-running just the file gets no automated coverage of the one code path that had a real bug (`get_browsable_decks`'s LATERAL aggregation). Worth hardening later, not blocking now.
- **Reuse/simplification suggestions, not applied:** the `fetchBatchProvenanceMap` + `useState` + fetch-and-set boilerplate is duplicated near-identically across `MyFlashcards.jsx`/`StudyMode.jsx`/`NoteDetail.jsx` and could be a shared `useBatchProvenance(batchIds)` hook (would also make the stale-response guard applied to all three automatic instead of hand-copied). `ProvenanceBadge` also reinvents a small icon+pill pattern that already exists as `FlashcardCard.jsx`'s own `getVisibilityBadge`. Both legitimate DRY opportunities, not correctness bugs — deferred rather than risking a late, unreviewed refactor this session.

**Independent code-review, run in full (per auditor request, credits added mid-session to clear an initial rate limit):** 4 finder angles (diff scan + removed-behavior, cross-file tracer, reuse/simplification/efficiency, SQL correctness) across two passes, surfacing and fixing 7 real/plausible bugs total: the `min(uuid)` SQL bug, `ReviewSession.jsx`'s dropped `batch_id`, two missed `MyFlashcards.jsx` badge call sites, and three stale-response race guards (`StudyMode.jsx`, `MyFlashcards.jsx`, `NoteDetail.jsx`). The SQL-focused pass found no further deploy-breaking bugs — confirmed the `min→array_agg` fix was sufficient, and confirmed zero drift in `get_study_queue`/`get_browsable_notes` against their source functions via direct diff. All fixes `npx eslint`/`npx vite build` clean.

**Files changed:** `src/components/content/ProvenanceBadge.jsx` (new), `src/lib/provenance.js` (new), `src/pages/dashboard/Content/MyFlashcards.jsx`, `src/pages/dashboard/Content/MyNotes.jsx`, `src/pages/dashboard/Content/NoteDetail.jsx`, `src/pages/dashboard/Content/BrowseNotes.jsx`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/dashboard/Study/ReviewFlashcards.jsx`. SQL: `docs/database/sprint8.7.4/00`–`05`.

**Next: operator runs `00_DIAGNOSTIC` → reconciles → runs `01`-`04` → runs `05_TEST` with a real user id → live-verifies via the app (or asks this tool to, once test credentials/DB access are available) → only then push.**
...
**Decisions locked in (see D-20, blueprint.md §3.1):** new distinct `question_type` value `mcq_multi` (not a flag on `mcq`) — build-up-then-explicit-submit interaction (mirrors `handleMatchSubmit`, confirmed in pre-flight), not single-tap-immediate-verdict. Exact-set grading, all-or-none, no partial credit. `correct_answer` stores a canonicalized SET string (`"0;2;4"`, sorted ascending, semicolon-joined, no spaces) — both `FlashcardCreate.jsx` and `BulkUploadFlashcards.jsx` call the same `src/lib/mcq.js` helpers so the two entry paths can't drift; compared as a parsed set at grading time, never as a raw string. Hard validity rule: 2-6 options, at least 1 correct — "at least 1 incorrect" is NOT enforced (course-agnostic platform, not a logical requirement); marking every option correct shows a non-blocking inline confirmation instead of rejecting. Attempt evidence: new nullable `review_events.selected_answer jsonb`, populated only by this type, written via a new `apply_review` trailing parameter `p_selected_answer jsonb DEFAULT NULL`. Gated same as every other verdict-bearing type (D-10).

**Pre-flight confirmed:** `handleMatchSubmit` (StudyMode.jsx) is the build-up-then-submit pattern to mirror, not `handleMcqSelect`'s single-tap one. `mcq`'s current representation (2-6 options, `correct_answer` = 0-based index as text, `back_text` auto-derived) is the baseline being extended, not replaced. `AnswerOption.jsx` already has an unused `selected` state in its style maps — no new sibling component needed, it directly supports a toggle-select checkbox mode. The CSV `correct_option` column's existing 1-based-CSV-to-0-based-DB conversion is followed exactly, just semicolon-delimited for multiple values. `apply_review`'s live signature confirmed unchanged since Sprint 7.4 (5 params) via `docs/database/sprint8.6c/00_DIAGNOSTIC_preflight.sql` — the new 6th param is added via `CREATE OR REPLACE` against that exact signature, not assumed from memory, specifically to avoid the overloaded-RPC/PostgREST-ambiguity bug this project was already bitten by once (blueprint.md §1.11).

**Built — frontend (code written, NOT yet pushed — see Deployment Order Rule below):**
- `src/lib/mcq.js` — new helpers: `canonicalizeMultiAnswer`, `parseMultiAnswer`, `compactMcqMultiOptions`, `validateMcqMultiOptions`, `deriveMcqMultiBackText`.
- `src/lib/questionTypes.js` — `mcq_multi` added to `formatQuestionType`'s label map and `BROWSABLE_QUESTION_TYPES`. Deliberately NOT added to `GRADED_QUESTION_TYPES` (that array is specifically the shared single-tap `AnswerOption` list `mcq`/`correct_incorrect`/`case_study_mcq` use) — same reasoning as `match_the_following`.
- `src/pages/dashboard/Content/FlashcardCreate.jsx` — new `correctOptionIndices` array field (parallel to single-select's `correctOptionIndex`), `toggleMcqMultiCorrect` handler, checkbox authoring UI (mirrors the mcq radio-button UI), non-blocking "all options marked correct" inline notice, validation, and canonicalized row-building at save.
- `src/pages/dashboard/Study/StudyMode.jsx` — new `mcqMultiSelected`/`mcqMultiRevealed`/`mcqMultiIsCorrect` state, `toggleMcqMultiOption`/`handleMcqMultiSubmit` handlers, new render branch (checkbox build-up via `AnswerOption`, Submit button, exact-set verdict, Continue-on-wrong / GradeButtonRow-on-right same pattern as `match_the_following`). `submitReview`/`handleRating` both gained a threaded `selectedAnswer` parameter (default `null`, every other type's call sites unaffected).
- `src/pages/dashboard/BulkUploadFlashcards.jsx` — `mcq_multi` added to `RECOGNIZED_QUESTION_TYPES`; new flat-row CSV branch (semicolon-delimited `correct_option`, e.g. `"1;3"`, range/uniqueness-validated, normalized via the same `src/lib/mcq.js` helpers manual authoring uses); template/help-text/error-message updates.
- `src/data/helpContent.js` — new "Multi-select MCQ" entries across all 5 sections (Question Types guide, professor authoring, bulk-CSV basics/professor-types tips), matching the existing 8 types' format exactly.

**Built — SQL, ✅ deployed & verified live (`docs/database/sprint8.6c/`):** `00_DIAGNOSTIC_preflight.sql` (confirmed live CHECK constraint, both D-10 policy bodies, exact `apply_review` signature + single-overload check, `review_events` columns — matched this sprint's assumptions exactly). `01_SCHEMA_add_mcq_multi_type.sql` (widened `chk_flashcards_question_type` to 9 values, extended both D-10 RESTRICTIVE policies, added `review_events.selected_answer jsonb`). `02_FUNCTIONS_apply_review_selected_answer.sql` (`CREATE OR REPLACE` against the exact live signature, appends `p_selected_answer jsonb DEFAULT NULL`). `02b_HOTFIX_drop_ambiguous_apply_review_overload.sql` (real bug found live — see below). `03_TEST_verify_sprint8.6c.sql` (BEGIN/ROLLBACK — all 7 checks PASS on the final run).

**Two real bugs found and fixed during deployment (operator ran the SQL, reported the actual errors, both traced and corrected — not assumed away):**
1. **`02_FUNCTIONS`'s CREATE OR REPLACE did not replace the old `apply_review` in place** — Postgres treats a changed parameter list as a distinct overload regardless of DEFAULT-ed trailing params, so the deploy left two live `apply_review` functions simultaneously. Surfaced live as `ERROR 42725: function public.apply_review(...) is not unique` when `03_TEST` called the old 5-arg shape. This is the same overloaded-RPC/PostgREST-ambiguity class already documented at blueprint.md §1.11 for `get_browsable_decks` v5 — the sprint's own pre-flight named this exact risk and the fix still initially got it wrong. Corrected with `02b_HOTFIX` — an explicit `DROP FUNCTION` of the old 5-arg signature.
2. **`03_TEST`'s own RLS-impersonation was incomplete** — it set `request.jwt.claims` for `auth.uid()` but never switched the actual Postgres session ROLE, and the Supabase SQL Editor connects as a superuser/table-owner role that bypasses RLS entirely regardless of jwt claims. This produced a false FAIL on "student blocked from mcq_multi insert" (the insert silently succeeded — RLS was never actually enforced in that run). Confirmed the underlying gate was fine the whole time by reading the live policy body directly. Fixed by adding `SET LOCAL ROLE authenticated;` around every RLS-relevant statement — the exact idiom `sprint7.5/02_TEST_verify_d10_role_gate.sql` already established, which should have been reused from the start.

**Final live-verified result (all 7 checks PASS):** exactly one `apply_review` overload; professor insert of `mcq_multi` succeeds; student insert genuinely RLS-rejected; `apply_review`'s new `p_selected_answer` writes correctly to `review_events.selected_answer`; the pre-existing 5-arg call shape (every other question type's call site) still works and leaves `selected_answer` NULL; `submit_review`'s compat wrapper still resolves and works.

**Live browser verification (17/09/2026, dev server → live Supabase, real professor session) — done:** authored two `mcq_multi` cards manually (partial-correct set; all-options-correct — confirmed the non-blocking notice doesn't block saving), both with correctly-derived `back_text` (correct options joined with `•`). Studied both: partial-correct submission auto-graded Hard with a single Continue (no self-grade offered); fully-correct submission revealed both as "Correct answer" and offered the full Hard/Medium/Easy self-grade (graded Easy, session completed cleanly). Browse Study Sets' Question Type filter correctly lists and narrows to "Multi-select MCQ". **Unprompted bonus confirmation:** the professor dashboard's "Accuracy by question type" widget picked up "Multi-select MCQ" automatically, zero code changes needed — same type-generalization pattern every prior question-type sprint relied on. CSV upload verified via a synthetic `File`/`DataTransfer` injection (Browser pane has no native file-picker automation): an invalid delimited `correct_option` (out-of-range index) correctly rejected the WHOLE file with a specific row-level error and created 0 rows; a corrected CSV then uploaded successfully with `back_text` byte-identical to manual authoring's output. All 4 test artifacts cleaned up (3 SQL-deleted since the Browser pane's sandbox blocks the in-app delete button's native `confirm()` dialog; the invalid CSV attempt created nothing to delete), reverified at 0 rows.

**Sprint 8.6c is now fully shipped: SQL deployed & verified live, frontend built and live-verified end to end.** Per the Deployment Order Rule the SQL prerequisite was satisfied before any of this verification began. **✅ Committed and pushed** (`19632fe`, on `main`).

**Current Phase:** Phase 8 (B2B growth), Group A — professor authoring-toolkit completeness work, per the operator's stated rollout-strategy rationale (see D-20's "why now").

## Sprint 8.6b: Grouped-Row CSV Import — match_the_following + concept_card — Phase 8 (17/09/2026) — ✅ Just Completed

**Pre-flight:** re-read `BulkUploadFlashcards.jsx`'s `case_study_mcq` handling in full — confirmed the `scenario`/`case_group` column pair is the pattern to mirror for the grouping *mechanic* (an uploader-chosen label column, identical across every row of one group, unique to it in the file), but NOT for the fan target: `case_group` keeps every row its own separate card sharing a `batch_id`; match_the_following/concept_card need every row in a group fused into ONE card, since that's genuinely how their DB representation works (`options` = one object / one array, not N rows). Re-confirmed live via `src/lib/matchTheFollowing.js` and `src/lib/conceptCard.js`: match's `options` = `{left, right, correct}` (left/right locked to equal length, no distractor right-items — CSV pairing is therefore positional, row N's left matches row N's right); concept_card's `options` = array of `{term, definition}`, and unlike every other bulk-uploadable type its `back_text` is typed directly, not derived.

**Built:** `BulkUploadFlashcards.jsx` — `RECOGNIZED_QUESTION_TYPES` grew from 5 to 7 (`match_the_following`, `concept_card` added). New template columns: `match_group`/`match_left`/`match_right`, `concept_group`/`concept_term`/`concept_definition`. Parse pass accumulates rows per group into a `Map` (order-independent — grouped rows needn't be contiguous), checking `target_course`/`subject`/`topic`/`front` (+`back` for concept_card) stay identical across a group — a mismatch is rejected with a specific per-row error, never silently merged; a group referenced by only one row fails the existing `validateMatchPairs`/`validateConceptTerms` minimum. After the file is fully read, each group resolves through the same library functions manual authoring uses (`buildMatchOptions`/`deriveMatchBackText`/`validateMatchPairs`, `buildConceptOptions`/`validateConceptTerms`) — so a CSV-built card is byte-identical in the DB to a manually-authored one. match_the_following stays D-10-gated (professor/admin/super_admin); concept_card stays ungated (D-06). Template download, in-page instructions, and the RLS-rejection error message all updated to match.

**Regression guard:** standalone Node harness (outside React/browser, importing the real `src/lib/*` modules) exercised 5 scenarios before any live testing — valid 3-pair match card, valid 3-term concept card, orphan match_group (1 row, rejected), duplicate match_group id reused across two unrelated cards (the 2 genuinely-matching rows still form a valid card, the 3rd conflicting row rejected on its own — not silently absorbed), and a flat plain-flashcard row (unaffected). All 5 passed. `npx eslint` clean on both changed files; `npm run build` clean.

**Live verification (dev server → live Supabase, real professor session, operator logged in directly — Claude never saw or entered a password):** the Browser pane's automation has no file-picker/file-upload action to drive the CSV `<input type="file">` directly, so verification instead exercised the exact insert path `BulkUploadFlashcards.jsx` uses (same library calls, same row shape) via the authenticated page's own module scope. A 3-pair match_the_following card and a 3-term concept_card both inserted successfully (confirms the professor RLS gate permits match_the_following), read back with the exact shape manual authoring produces, appeared correctly in My Study Sets, and the concept_card's existing "Read Concepts" viewer rendered all 3 term/definition pairs correctly. Both test rows deleted after verification, reverified at 0 rows via a `front_text ILIKE '%SPRINT8.6B%'` sweep.

**Docs:** `helpContent.js` gained a new "Bulk CSV Upload — Grouped-Row Types" section with real full-row examples; corrected the stale "cannot be created via CSV, for any role" claim and a pre-existing-but-wrong tip that said an unrecognized `question_type` silently downgrades to a plain flashcard (it's been rejected outright since Sprint 8.3's A1 fix). `DATABASE_SCHEMA.md` and `blueprint.md` updated from "deferred" to describe the shipped mechanic, with the historical Sprint 7.8/7.12 entries annotated as superseded rather than rewritten.

**Current Phase:** Phase 8 (B2B growth), Group A. No SQL needed this sprint — both `question_type` values were already live.
...
**Pre-flight:** `study_sessions` schema confirmed against `DATABASE_SCHEMA.md` (no category column, matches the brief's last-confirmed read) and `get_study_time_stats`'s body dumped via `pg_get_functiondef` in the new `00_DIAGNOSTIC` script — confirmed purely `SUM(duration_seconds)`, no category reference, so the new column needed zero RPC changes. No live DB CLI/MCP access this session, so the diagnostic script is written for the operator to run in Supabase before `01_SCHEMA`, same "verify live, not just docs" discipline as Sprint 8.4. `StudyTimerContext.jsx`/`StudyTimerWidget.jsx` re-read fresh — confirmed Sprint 8.4's nav work (the `ExamDateChip` addition) didn't touch the Start/Stop/recovery flow.

**Gap the brief didn't cover, surfaced and resolved before building:** the brief detailed the dashboard widget card in depth but not the nav-bar `StudyTimerChip` — which currently stops, logs, and toasts a confirmation in one tap with no card to host a picker in. Since category is now required to log, that instant-log path couldn't stay instant. Asked the operator rather than guessing: chosen fix is that the chip's tap now navigates to `/dashboard/study-time` for the picker, same pattern the app already uses for the 4-16h recovery case — no new UI component needed.

**Decisions from the brief, unchanged:** exactly 5 categories (Reading, Writing Practice, Lecture Viewing, Paper Solving, Mock Test (timed)) — "Revision/Recap" dropped as a separate category since it mixed a purpose/timing axis into an activity-type axis; category chosen at stop/log time, not start; nullable column, no backfill; no new analytics/breakdown surface this sprint.

**Built:** SQL — `docs/database/sprint8.5/00_DIAGNOSTIC_confirm_no_category_column.sql`, `01_SCHEMA_add_study_session_category.sql` (nullable `category text` + `CHECK` against the 5 values, no default, no backfill), `02_TEST_verify_category_column.sql` (real writes wrapped in `BEGIN`/`ROLLBACK` — `study_sessions` has no DELETE policy, so a manual cleanup DELETE could silently no-op under RLS and leave a fake session behind). Frontend — `StudyTimerContext.jsx` restructured: `stopAndLog()` no longer writes to the DB directly, it finalizes the duration into a new `pendingLog` state (persisted to its own `revisop_manual_timer_pending_log` localStorage key, mirroring the existing running-timer/recovery persistence, so a page reload mid-picker re-shows the picker instead of silently losing an already-stopped session); a new `confirmCategory(category)` does the actual insert once a category is chosen. `StudyTimerWidget.jsx` gained a category-picker step (5 buttons + Save, disabled until a selection is made) shown for both the direct-stop path and the resolved recovery-prompt path (full-time-log and custom-hours-log both route through the same picker). `StudyTimerChip.jsx` now navigates to the study-time page on the new `needs_category` outcome instead of attempting an inline log.

**Verification:** `npx eslint` clean on all 3 touched files. Live click-through in the Browser pane logged in as `TestOutlook`: Start → wait → Stop showed the 5-option picker correctly, Save stayed disabled until a category was selected, selection highlighting worked; confirmed `pendingLog` survives a full page reload (picker re-appeared with nothing lost — validates the localStorage-persistence design); confirmed the pre-existing <10s no-op path is unchanged (straight back to idle, no picker, no confirmation text — same as before Sprint 8.5). At that point the SQL wasn't deployed yet, so tapping Save correctly surfaced a live `PGRST204 "category column not found"` error and left the picker state intact (nothing lost) — proving the write path was wired correctly and the frontend properly gated on the SQL landing first.

**SQL deployed & verified live by the operator (16/09/2026).** `00_DIAGNOSTIC` confirmed the live `study_sessions` shape matched docs exactly and `get_study_time_stats` had zero category references, then `01_SCHEMA` added the column. `02_TEST` run in two parts (Supabase SQL Editor stops at the first error, and one section is designed to error): a valid `reading` insert succeeded, an invalid `'not_a_real_category'` insert correctly hit `23514 check_violation` — both rolled back, nothing persisted. One correction made mid-verification: the test's original use of `auth.uid()` returned `NULL` in the SQL Editor's no-session context (caught safely, nothing left behind); rewritten to use the project's existing `TestOutlook` test account looked up via `profiles.email`, same pattern as Sprint 8.4's own test-account fix file.

**Quality-auditor review, same day — DB-level integrity gap found and closed before commit.** The frontend required a category, but the database didn't: `category` is nullable and its own `CHECK` only fires when a value is present, so nothing stopped a future write path outside `confirmCategory()` from inserting `source='manual', category=NULL` and silently reopening the gap. Added `study_sessions_manual_requires_category`, a `NOT VALID CHECK (source <> 'manual' OR category IS NOT NULL)` — chosen over a trigger since every other rule on this table is already a plain `CHECK`. `NOT VALID` enforces on every future write without touching existing rows, so no backfill was needed. Before trusting "old rows are never touched again" as the reason this is safe, immutability was verified live (not just from the doc's claim): no RLS `UPDATE` policy, no function anywhere references this table with an `UPDATE`, no trigger attached to the table — `study_sessions` rows are genuinely write-once. Live-verified all three required cases: a `NULL`-category manual insert is now rejected (`23514`), a valid categorized insert still succeeds, and 3 real pre-existing March 2026 manual rows with `category IS NULL` read back untouched. The one real test row left by the non-rolled-back success case was removed via a tightly-scoped preview-then-delete, confirmed to match exactly 1 row first. No frontend changes needed — it already only ever sends a valid category.

**Current Phase:** **Phase 8 (B2B growth), Group A now fully shipped and live, database-integrity-hardened** — bulk student addition, batch archiving, Help content, exam date field, and offline-log categories, SQL and frontend both deployed, quality-auditor follow-up closed. Next up per the operator: Group B (content versioning/retirement), per the original phase sequencing.

## Sprint 8.4: Exam Date Field — Phase 8, Group A (16/09/2026) — ✅ Just Completed

**Pre-flight Step 0 — group_type/is_batch_group audit (carried in from Sprint 8.3's finding):** grepped every read/write of both columns on `study_groups`, frontend and backend. **No second instance of the "set one column, not the other" bug** (the shape Sprint 8.3 fixed in `create_batch_group`) was found anywhere in live write paths. `is_batch_group` is the de facto canonical signal (read by nearly everything — RLS-adjacent guards, archiving, admin/report RPCs, cascade-delete exemption, most UI branching); `group_type`'s 3-way enum is read in exactly two places (`join_group_by_token`'s approval-gate branch, `GroupJoin.jsx`'s copy/labels), both needing the 3rd `system_course`/`custom` distinction a boolean can't carry. **Did not collapse the two fields this sprint** — the audit surfaced that two live RPCs the frontend actually calls (`create_study_group`, `get_user_groups`) have no matching definition anywhere in the repo's SQL docs; `CreateGroup.jsx`/`MyGroups.jsx` call parameters/columns that don't exist in the only filed versions, meaning newer, unfiled versions are live in Supabase. Collapsing the schema without being able to see those two bodies would be exactly the kind of blind schema change the sprint's own "capped scope" rule says to stop and report on rather than push through. **Next step if pursued:** pull both functions' live definitions via `pg_get_functiondef` before touching anything.

**Exam-date feature.** Pre-flight confirmed: `profiles` had no exam-date column (re-verified live via diagnostic, not just docs — which turned out to also be missing `access_request_ref`/`updated_at`, both now added to `DATABASE_SCHEMA.md`); students remain single-course (Phase B multi-course still deferred); `Signup.jsx`/`ProfileSettings.jsx`/`NavDesktop.jsx`/`NavMobile.jsx`/`Dashboard.jsx` inspected and confirmed as the right integration points.

**Scope changed from the original brief during pre-flight**, both times by explicit operator decision, not assumption:
1. Signup doesn't INSERT into `profiles` directly — a `handle_new_user()` DB trigger does, reading `raw_user_meta_data`, because email-confirmation signup returns no session. Capturing `exam_month` in `Signup.jsx` would have required updating that trigger too. Operator chose instead to **drop the Signup.jsx picker entirely** and capture exam info via a dismissible popup on first login (any student, new or pre-existing) — sidesteps the trigger change completely, since by first login a real session exists.
2. Popup dismiss behavior: **permanent** (new `has_dismissed_exam_prompt` column), matching the existing `has_dismissed_goal_prompt` pattern — the nav chip + Dashboard CTA remain as the passive ongoing reminder after dismissal, no re-nagging.

**Decisions carried from the brief, unchanged:** two nullable columns (`exam_date` exact, `exam_month` 1st-of-month placeholder); no backfill for existing students — every pre-existing student sees the CTA/popup exactly like a new student who skipped it; single value per student; editable indefinitely from Profile Settings, month→exact upgrade supported.

**Built:** SQL — `exam_date`/`exam_month` (with a `CHECK` that `exam_month` is always the 1st) + `has_dismissed_exam_prompt` on `profiles` (`docs/database/sprint8.4/`). Frontend — `ExamDateContext.jsx` (fetch-once + mutators, same shape as `CourseContext.jsx`) wired into `App.jsx`; `ExamDatePromptModal.jsx` (dismissible, shown from `Dashboard.jsx` after the onboarding/profile-completion modals have both cleared, so dialogs never stack); `ExamDateChip.jsx` (nav pill beside `StudyTimerChip` in both `NavDesktop.jsx`/`NavMobile.jsx`, student-only, three states — countdown / month-text / CTA); a self-gating "Days Until Your Exam" Dashboard card (renders only when an exact date exists); an Exam Date section in `ProfileSettings.jsx` (set/refine, month↔exact toggle). Shared date-math in `src/lib/examDate.js` — parses `YYYY-MM-DD` date-only values directly and does arithmetic in UTC-anchored day counts, never through `toISOString()` or a timezone-local `Date` read (the exact footgun this project's DB rules warn about elsewhere).

**Verification:** `npm run build` clean (all new/changed files, including lazy-loaded chunks). Live click-through in the Browser pane logged in as an existing student (`TestOutlook`, a pre-existing account with no exam info — confirms the no-backfill CTA behavior for real): popup appeared on first login; set month-only (November 2026) → nav chip showed "Exam: November 2026" text, no dashboard countdown card; upgraded to an exact date (15/11/2026) via Profile Settings → nav chip updated to "60d left" **immediately, no reload**, Dashboard card appeared showing "60" / "15 November 2026"; changed the exact date again (20/12/2026) → clean update to "95d left" everywhere, no stale cache, no duplicate state; mobile nav (375px) checked, chip renders compact with no overflow beside the bell. Day-count math verified correct against the calendar (Sep 16 → Nov 15 = 60 days). **Not live-verified:** the "dismiss without setting, never reappears" path — would need a second student account, and creating one requires a real email signup this session can't perform (credential/account-creation restriction); code-reviewed instead (dismiss sets the boolean unconditionally on skip, same as the existing goal-prompt pattern). Also fixed an unrelated `.claude/launch.json` bug found while starting the dev server for this verification — `vite`'s own port-increment logic ignored the harness's assigned proxy port, so the preview never loaded; pinned an explicit `--port 5183 --strictPort`.

### Same-day follow-up — audit review of Sprint 8.4's completion report (16/09/2026)

Flagged one material point: `has_dismissed_exam_prompt`/`ExamDatePromptModal` are a genuine, database-persisted product decision beyond the sprint brief's original two-nullable-column contract, and the dismiss lifecycle had been disclosed as untested rather than actually verified. Recommendation: record it as an approved D-17 extension and test the lifecycle, reusing the existing test account.

Both done. `docs/database/sprint8.4/02_FIX_reset_exam_prompt_dismissal_test_account.sql` reset `TestOutlook` to pristine `false`/`NULL`/`NULL`, then live-verified: popup reappears when unset → dismiss without setting closes cleanly → reload does **not** bring it back (permanent) → nav chip's CTA is unaffected by the flag before or after dismissal → Profile Settings' Exam Date section stays fully reachable/functional → setting an exact date afterward (05/05/2027) updates cleanly to "231d left" everywhere, no interference from the prior dismissal. Recorded in `blueprint.md`'s D-17 entry; the now-resolved backlog line removed from §3.2. Also declined a separate operator request made during this review — showing a days-remaining countdown for month-only students by assuming the 1st of the month — since it directly reverses D-17's own precision rule (a month-level guess must never imply exact-date precision) and the "auto-notify when the real date is announced" half of that request is explicitly out of scope per the sprint brief (no automated reminder/notification tied to the date).

**Second same-day follow-up:** operator asked for a smaller version of the declined nudge — a text line for month-only students asking whether the date has been announced yet, without the fabricated countdown. Added: a slim Dashboard line ("Exam expected [Month Year] — date announced? Update it") shown only when `exam_month` is set and `exam_date` is not, linking to Profile Settings — distinct from the countdown card, which remains exact-date-only. Live-verified (the test account happened to be in exactly this state — `exam_month` set to May 2027, no `exam_date` — from the operator's own exploration in a separate tab): nudge renders correctly on Dashboard, "Update it" navigates to Profile Settings. `npm run build` clean.

**Third same-day follow-up:** operator asked to add the days-count back after all, explicitly labeled "tentative" this time — a genuinely different ask than the flat countdown declined earlier, since the original objection was to an unlabeled number implying certainty, not to an estimate labeled as one. Upgraded the same Dashboard nudge line to `~{days} days (tentative — assumes the 1st of {Month Year})` — qualifier placed directly beside the number (not a caption underneath, which gets skimmed past). Nav chip and the exact-date countdown card both left untouched, by explicit choice — the chip has no room for a caveat, and the real card should stay visually distinct from an estimate. Live-verified: `~227 days (tentative — assumes the 1st of May 2027)` rendered correctly, day-count confirmed correct against the calendar. `npm run build` clean.

**Fourth same-day follow-up — audit reframe, adopted:** review pointed out that iteration 3's hedge ("tentative") still framed the number as an estimate of days-until-exam; a stronger fix changes what the number measures instead — days until the stated month *begins*, which is exact, unhedged fact, not a guess. Restructured the nudge into a labeled 3-line "Exam Runway" block: `EXAM RUNWAY · May 2027` / `227 days until May begins` / `Exact exam date not yet set · Update once announced` — no `~`, no "tentative", nothing left to hedge. Same underlying day-math, just a truer label. Also logged a deferred backlog idea from the same review: a revision-phase indicator (Build & Strengthen → Intensive Revision → Final Consolidation) derived from the runway length — real pedagogical design work, explicitly not built this sprint. Live-verified; `npm run build` clean.

**Current Phase:** **Phase 8 (B2B growth), Sprint 8.4 — Exam Date Field, audit-closed.** Next up per the operator: Sprint 8.5 (offline-log categories).

## Sprint 8.3: Bug fixes + Help renderer (hyperlinks & screenshots) — Phase 8, Group A (16/09/2026) — ✅ Just Completed

**Part A — two isolated `BulkUploadFlashcards.jsx` fixes.** A1: a non-blank, unrecognized `question_type` (e.g. `match_the_following`, a typo like `mcqq`) is now rejected with a specific row error naming the bad value, instead of silently downgrading to a plain flashcard — this closes the exact gap Sprint 8.2 documented but explicitly left unfixed (`bugs.md`). A blank cell is unchanged, still silently defaults to `flashcard`. A2: `downloadTemplate()`'s own example rows carried the same unqualified, budget-year-dependent tax figures (₹2.5 lakhs exemption limit, Rs 10000 u/s 80TTA) that Sprint 8.2's auditor pass had already fixed in `helpContent.js`'s prose — missed there since it's a different file. Replaced with timeless structural facts (direct-vs-indirect tax, the 80TTA/80TTB rule without a figure, the 80C section number instead of an amount). `helpContent.js`'s own verbatim-copied CSV examples were deliberately left untouched (explicitly out of scope per the sprint brief).

**Part B — `Help.jsx` renderer extended with the two capabilities Sprint 8.2 deferred.** B1: an inline `[label](/route)` link syntax parsed at render time into a real react-router `<Link>` — only matches a `/`-prefixed target inside well-formed brackets, so anything malformed (a stray `[label](`, an external URL) simply never matches and degrades to plain text rather than crashing. Applied first to the Sprint 8.2 batch-lifecycle sections' plain-text "Go to Admin Dashboard"/"Go to Study Groups" mentions. B2: a new `case 'image'` content-block type with numbered CSS-positioned callout markers (percentage-based `x`/`y` so a callout can be repositioned without re-editing the screenshot) and a matching numbered caption list — the first content-image pattern this codebase has shipped, in a new `public/help-screenshots/` folder.

**3 of an originally-planned 5 screenshots captured and wired into `admin-batch-groups`:** batch creation + Copy Invite Link, Pending Batch Requests approve/reject, and the Active/Archived filter. All use a disposable, clearly-labeled test batch group and a dummy student test account — no real student/professor data appears in any of them, checked manually by both the session and the operator before commit. The other 2 (role-change confirmation, Hard Delete confirmation) — see the Quality Auditor follow-up below for why these are deferred, not dropped. Screenshots were captured with the operator's own OS screenshot tool rather than the Browser pane's own screenshot action, since that tool has no way to export an image as a file the session can commit — each was then resized/recompressed with `sharp` (already a project dependency) down to 10-20KB before being placed in the repo.

### Same-day follow-up — Quality Auditor review of Sprint 8.3's completion report (16/09/2026)

The initial completion report characterized the 2 dropped screenshots (role-change, Hard Delete confirmations) as satisfied by the "native dialog" technical finding. The Quality Auditor's disposition, in full:

- **A1: accepted, with a caveat recorded honestly.** The sprint's specified end-to-end CSV upload test was replaced by extracted-logic testing (no file-upload capability in this session's Browser pane tool) — this is a real test-plan gap, not equivalent to a live upload, and is now recorded as such rather than implied to be "fully verified." Should be closed with an actual upload click-through next time this code is touched.
- **A2: accepted as-is.**
- **B1: accepted, but only after actually demonstrating malformed-input coverage** — the original report only showed a happy-path click-through. Extracted the exact parser and ran it against 17 cases (unclosed brackets, external URLs, reversed/nested brackets, empty label/target, non-string inputs, mixed valid+broken in one string) — all 17 degraded correctly with zero crashes.
- **B2 renderer architecture: accepted** as a scalable first pattern (root-relative `public/help-screenshots/`, responsive images, semantic `alt`, percentage-based CSS-positioned annotations).
- **Screenshots 1-3: accepted**, including the PII check.
- **Screenshots 4-5: deferred, not waived.** Native `confirm()`/`prompt()` dialogs are poor UX for two high-consequence/destructive actions in their own right — the correct fix is real, accessible, in-app confirmation dialogs for role-change and Hard Delete, with screenshots of *those* once built. Tracked as a real backlog item in `blueprint.md` §3.2, not silently closed as "dropped."
- **The `create_batch_group`/`group_type` bug: good catch, kept independently documented** rather than folded into this sprint's approved-scope count — its own `bugs.md` entry, its own 4 SQL files in `docs/database/sprint8.3/`, separable code/DB diff and verification.
- **2 disposable test batches: must be cleaned up before the sprint is operationally closed.** `docs/database/sprint8.3/04_CLEANUP_delete_test_batches.sql` written for the operator to run (preview-then-delete, real DELETE not archive, since both are named "delete after" sandbox data with no history worth preserving).

**Sprint 8.3 status: implementation complete, audit close pending the 2 housekeeping items above** (test-batch cleanup + the deferred-screenshot backlog item recorded). Once those are done, proceed to Sprint 8.4 (exam date field).

**Real production bug found and fixed along the way, not originally in scope:** setting up a disposable test batch group to stage the screenshots surfaced that `create_batch_group` never included `group_type` in its `INSERT` — every batch created through the live Admin Dashboard button since whichever change last replaced this function silently got `group_type='custom'` instead of `'batch'`, which completely disabled `join_group_by_token`'s D-15 student-approval gate for that batch: a dummy student joined instantly active, no "Pending Batch Requests" row ever appeared, no admin action taken. Diagnosed via two read-only diagnostic SQL scripts the operator ran in Supabase SQL Editor (`docs/database/sprint8.3/00_DIAGNOSTIC...`, `01_DIAGNOSTIC...`) — confirmed all 3 real production batches (CA Final, CA Foundation, CA Intermediate, all created 18-19/03/2026) were unaffected, already correctly `group_type='batch'`. Fixed same session (`02_FIX_create_batch_group_type.sql`, deployed by the operator) and live-verified end-to-end: a fresh disposable test batch + the same dummy student account correctly produced a pending request the second time, approve/reject both worked. Full detail in `bugs.md` and the `group_type` column note in `DATABASE_SCHEMA.md`.

**Verification:** `npm run build` + `npx eslint` clean on all 4 touched files. A1's exact gate logic was extracted and run standalone against 8 cases (valid types, typos, blank, whitespace-only, case-insensitivity) — all 8 matched expected behavior; not a live file-upload click-through, since the Browser pane tool available this session has no file-upload capability. A2's corrected template rows were re-parsed with the app's own `parseCSVLine` (copied verbatim) — all 10 rows still produce the correct 18 columns. B1/B2 live-verified in the Browser pane at both desktop and 375px mobile width, no overflow, console clean, the new link actually navigates.

**Current Phase:** **Phase 8 (B2B growth), Sprint 8.3 — Bug fixes + Help renderer (hyperlinks & screenshots), see above.** Sprint 8.2 (help content corrections), 8.1 (batch Active/Archived lifecycle), and 8.0 (batch invite-link joining) underneath, all fully shipped. Next up per the operator: Sprint 8.4 (exam date field), then 8.5 (offline-log categories).
...
Found during an unrelated file-cleanup audit: deleting a note (student `MyNotes.jsx` or admin `AdminDashboard.jsx`) never removed its image from the `notes` Storage bucket — confirmed via a `storage.objects`-vs-`image_url` diagnostic (8 orphans, ~47MB) and a trigger audit (nothing else covers it). Fixed with new `src/lib/noteStorage.js` (`deleteNoteStorageImage()`), wired into both delete handlers. `npm run build`/`eslint` clean; live click-through delete not attempted (destructive against production data) — operator should confirm on a throwaway note. The fix isn't retroactive — the 8 existing orphans still need a manual one-time Supabase Dashboard cleanup (no Storage trash/versioning; Claude does not perform permanent deletion itself). See `bugs.md` and `changelog.md` for full detail.

Phase 7 (question-type epic) closed out with Sprint 7.12 underneath Sprint 8.0-8.2 — see that section. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).

## Sprint 8.2: Help — batch lifecycle corrections + question-type guidance — Phase 8, Group A (16/09/2026) — ✅ Just Completed

**Content-only sprint, no product behavior changed.** `src/data/helpContent.js` still described the pre-Sprint-8.0 world in seven sections — automatic course+institution batch matching, "Grant Access" auto-enrolling into a batch, no mention of archiving. Also added guidance for all 8 question types (most of that content didn't exist before at all — the old "Creating Flashcards" section only ever described a plain front/back card).

- **Pre-flight:** read `docs/active/blueprint.md` D-10/D-15/D-16 and `CLAUDE.md` first, then verified every claim against live code rather than the docs alone — `AdminDashboard.jsx` (tab labels, button text, `grantAccess`/`addToBatchGroup`/`archiveBatchGroup`/`restoreBatchGroup` functions), `GroupJoin.jsx` (the "Request to Join"/"This batch has ended" states and the monitoring-disclosure line), `FlashcardCreate.jsx` + `BulkUploadFlashcards.jsx` + `src/lib/{mcq,fitb,matchTheFollowing,caseStudyMcq,conceptCard,questionTypes}.js` (manual-creation field limits, CSV column recognition, role gates), `StudyMode.jsx` (how each type is actually answered), and `SuperAdminDashboard.jsx`/`SuperAdminAnalytics.jsx` (spot-checking three already-existing super-admin sections per the sprint's own pre-flight instruction).
- **Section A — batch lifecycle corrections, 7 sections rewritten:** `prof-welcome`, `prof-profile-setup`, `prof-batch-groups`, `prof-batch-performance` (professor-guide tab); `admin-dashboard-overview`, `admin-access-requests`, `admin-batch-groups` (admin-guide tab). Removed every claim of automatic course+institution matching; now describes the real flow — admin creates an empty batch, shares its invite link ("Copy Invite Link"), a student taps "Request to Join", an admin "Approve"s/"Reject"s under Pending Batch Requests, or an admin direct-adds one named student via the "Add to batch…" picker in User Management (that pick is the approval). "Grant Access" is described accurately as unlocking account access only, never assigning a batch. Archiving explained as: stops new requests/invitations, closes outstanding ones, freezes the report as of that moment, preserves approved memberships and content access; Restore resumes live reporting but does not reopen closed requests. Corrected the admin tab names to the real ones (Content Moderation / User Management / Access Requests / Batch Groups — the old text said "Recent Users"/"Content").
- **`guideContent.js` checked, no change needed** — a dedicated research pass confirmed it never made the specific false claims (no course+institution matching language, no "automatic" batch enrollment claim) — its one enrollment-related line is generic enough to still be true under the new flow.
- **Section B — question-type guidance, new content:** added `question-types-guide` (visible to every role — students need to know how to answer types they can't author) covering all 8 types (`flashcard`, `theory`, `concept_card`, `mcq`, `correct_incorrect`, `case_study_mcq`, `match_the_following`, `fitb`) — plain-language purpose, one CA example, who may create it, dos/don'ts, and how a student answers it (confirmed against `StudyMode.jsx`'s actual rendering for each: fitb's inline-blank + accepted-answers fallback, match_the_following's build-then-"Check Answers" MatchZone, case_study_mcq's collapsible "Case Scenario" panel, concept_card's total absence from the study queue). Added `prof-question-type-authoring` (professor/admin/super_admin only) covering the 5 gated types' actual form fields and limits (MCQ 2-6 options, Match 2-8 pairs/side, case study 2-8 questions authored under one scenario with no manual grouping label needed, fitb up to 6 accepted answers). Rewrote `flashcard-creation` (now "Creating Study Items") to scope it to the 3 open types plus a pointer to the new sections.
- **Section C — CSV instructions, role-split:** replaced the old single `prof-bulk-csv` section (which only ever documented a 2-column front/back CSV and was wrongly professor-only, even though Bulk Upload itself and its flashcard/theory rows are open to every signed-in user) with `bulk-csv-basics` (all roles — what a CSV is, the real 18-column template, quoting/blank/multiline handling, which types are CSV-supported at all) and a rewritten `prof-bulk-csv` (professor/admin only — the 4 gated types' extra columns, a real 3-row grouped case_study_mcq example). All example rows are copied verbatim from `BulkUploadFlashcards.jsx`'s own downloadable template string, not invented — guaranteed to match what the uploader accepts since it's the literal shipped template.
- **Real product gotcha found and documented (not fixed — out of scope for a content-only sprint):** the CSV parser's `RECOGNIZED_QUESTION_TYPES` list doesn't include `match_the_following` or `concept_card` — a row using either value in `question_type` is not rejected, it silently becomes a plain `flashcard`. Called out explicitly in the new help text and logged in `bugs.md`.
- **Spot-checked the three super-admin-only sections per the sprint's own pre-flight step (not a general rewrite) — found and fixed real drift:** `superadmin-user-roles` claimed a "role change dialog" and a free role picker that includes `super_admin`; the real UI is per-current-role "Prof"/"Admin"/"Student" buttons with no path to `super_admin` and a plain browser `prompt()` for the optional reason. `superadmin-hard-delete` named a "Delete User" button and an email-confirmation step; the real control is an unlabeled trash icon and a "type DELETE in capital letters" `prompt()` — also dropped an unverifiable "friend connections" claim from the deleted-data list and added the previously-missing note that the Supabase auth record survives and needs a manual follow-up delete. `superadmin-sa-analytics` had the wrong header stats and cohort-table columns, and two bullets (Retention Cards, Audit Log) that actually belong to the separate Super Admin Dashboard page.
- **Also removed:** a fabricated "Maximum 200 cards per upload" claim — confirmed via a full code search that no such cap exists anywhere in the upload path.
- **Verification done:** `npm run build` clean, `npx eslint src/data/helpContent.js` clean, no duplicate section ids (53 sections across 8 tabs, checked programmatically). Every button label, tab name, and behavior claim was checked against live component source, not assumed from the old help text.
- **Live verification, done via the Browser pane with the operator logging in (each role's credentials typed by the operator directly, Claude never saw or entered a password):** all four roles click-through confirmed correct — student sees the 8 general Content sections with the 5 gated ones correctly hidden; professor additionally sees `prof-question-type-authoring`/`prof-bulk-csv` and the corrected "For Professors" tab; admin sees the corrected "For Admins" tab with real tab names and the full create/invite/approve/archive flow; super-admin additionally sees the three corrected sections. Consoles clean on all four.
- **Real bug caught during that live check, fixed same session:** the raw CSV example rows (added for readability) had no spaces after their commas, causing horizontal page overflow (1520px content in a 985px viewport) — a page-breaking issue the sprint's own verification checklist exists to catch. Fixed by spacing the *data-row* examples (safe — the uploader trims every cell value) and re-verified clean at both desktop and 375px mobile width.

### Same-day follow-up — Quality Auditor review of Sprint 8.2 (16/09/2026)

A plain-language report of the sprint (plus a live four-role click-through) was sent to the operator's Quality Auditor for sign-off. Six corrections came back, all agreed and fixed the same day:

1. **CSV header-row example broke the actual format.** The "add a space after every comma for wrapping" fix applied to the *header row* example too — but `BulkUploadFlashcards.jsx`'s parser never trims header names (only cell *values*), so a literal copy-paste of that spaced header would produce keys like `" subject"` that the app's own `flashcard.subject` lookups would never match. **Fixed by removing that one raw line** (redundant with the column-by-column bullets already in the section) rather than re-spacing it — the exact header is one click away via "Download the Template."
2. **Tax examples had no year/regime, risking being read as current advice.** Swapped every example that was *my own prose* (not copied from the app) to timeless alternatives already used elsewhere in the app's own real template — the 20%-of-500 percentage question for Flashcard, and an "AS 1 — disclosure of accounting policies" example for Correct/Incorrect and Fill in the Blank. The *literal CSV example rows* (genuinely copied verbatim from the app's downloadable template, which still contains the same unqualified tax figures) were left untouched — changing them would have broken the "guaranteed to match the real download" guarantee — and a caveat tip was added next to them instead: illustrative figures only, check the applicable year/regime before treating any number as authoritative. The app's own template carrying this same issue is logged as a separate, small follow-up in `bugs.md`, not fixed this sprint.
3. **`prof-question-type-authoring`'s opening line wrongly implied `match_the_following` supports Bulk Upload.** It grouped all 5 gated types under "creatable... in both manual creation and Bulk Upload" — false for Match the following, which CSV has never supported for any role. Reworded to carve it out explicitly.
4. **A tip overgeneralized FITB's grading mechanism.** "These five types are gated because a wrong verdict automatically marks the card Hard" is true for MCQ/Correct-Incorrect/Case-study-MCQ/Match-the-following, but false for Fill in the Blank, which never has a "wrong" verdict at all (D-13) — only a match or a self-graded fallback. Split the tip in two so FITB's real mechanism (an incomplete answer list under-recognizes correct answers, rather than penalizing them) isn't confused with the other four's auto-Hard behavior.
5. **Hard Delete's audit-log claim was an unverified guarantee.** "so a record survives even if the delete itself fails partway" was never actually confirmed — if both operations share a transaction, a failed delete could roll the log insert back too. Replaced with the Auditor's own suggested wording: "The system attempts to record the action before deletion runs." (Also dropped the `admin_audit_log` table-name reference in the same edit — implementation detail that doesn't belong in user-facing help.)
6. **`admin-batch-groups` never said where to actually view a batch's performance report.** The section covers membership/lifecycle management (create/invite/approve/archive) on Admin Dashboard → Batch Groups, but the actual weekly report (reviews/streaks/study-time per student) lives on a different surface — Study Groups → open the batch → GroupDetail, the same view professors use (`prof-batch-performance`, which admins can also see since that tab is admin-visible too). Added a cross-reference tip so this isn't left implicit.
- **Recommended but explicitly deferred, not silently skipped:** the Auditor also recommended annotated screenshots for the admin/super-admin multi-step workflows and a CSV example. Separately, the operator asked for in-app navigation hyperlinks (e.g. "Create → Bulk Upload" becoming a real link). Both would require extending `Help.jsx`'s renderer (currently plain paragraph/list/steps/tip blocks only, no image block, no link parsing) — real feature work, not a content edit. **Operator decided to bundle both into one follow-up sprint** rather than do either now — logged in `blueprint.md`'s Pending Work (Immediate).
- **Re-verification after fixes:** `npm run build` and `npx eslint src/data/helpContent.js` both clean; re-checked all four roles live again in the Browser pane — gated sections still don't leak, the corrected CSV examples still parse (values-only spacing preserved), no overflow at desktop or 375px mobile.
- **Files:** *changed* — `src/data/helpContent.js` only. Docs: `docs/active/blueprint.md`, `docs/active/now.md`, `docs/tracking/changelog.md`, `docs/tracking/bugs.md`.
- **Status:** ✅ Content shipped, build/lint-verified, all four roles live-verified twice (pre- and post-Auditor-fix), no new architecture decision (corrections + one deferred follow-up sprint logged). Not yet committed/pushed per the project's normal commit workflow.

---

## Sprint 8.1: Batch group Active/Archived lifecycle — Phase 8 sprint 2 (15/09/2026) — ✅ Just Completed

**Inserted after Sprint 8.0, before Help (now 8.2) / exam date (8.3) / offline-log categories (8.4).** Batch groups (professor/admin monitoring cohorts, hidden from students' Groups nav) needed a complete, small Active/Archived workflow: archiving stops new requests/invitations/approvals/direct-adds and drops the batch from active monitoring, but preserves approved memberships, shared-content access, and a frozen historical report — restore reopens the same batch without rebuilding anything. Full design discussion and decision recorded as blueprint.md §3.1 D-16.

- **Pre-flight (background Explore agent + two rounds of live `pg_get_functiondef`/grants/RLS diagnostics, `docs/database/sprint8.1/00_DIAGNOSTIC_preflight.sql`) found the canonical-identifier question the sprint brief asked to resolve: `is_batch_group` (boolean) vs `group_type` (`'batch'|'system_course'|'custom'`) are two independently-set, never-synced flags. Confirmed live: 1:1 consistent today (all 3 real batch groups have both set correctly) — `is_batch_group` is the flag every existing admin/report/guard path already trusts, so D-16 uses it exclusively and leaves the separate `group_type` plumbing untouched (D-15 scope, not this sprint's).**
- **Pre-flight also surfaced three RLS bypass gaps, unrelated to D-15/D-16's own design but directly relevant to this sprint's "prevent bypasses" requirement, closed as part of the same migration:** `sg_delete_creator` and `sg_update_creator` (`study_groups`) had no `is_batch_group` clause at all — a batch group's own creating admin could delete it directly, or write `archived_at` straight via a client `.update()` call, skipping the snapshot-capture transaction entirely. Narrowed both to `is_batch_group = false` — safe, since batch groups have never had a direct-edit UI to lose (AdminDashboard's batch tab was already read-only display). `sgm_insert_admin` (INSERT on `study_group_members`) had no archived check, so a client holding local group-admin rights could insert a member into an archived batch past `enroll_user_in_batch_group`'s own guard — closed with an `archived_at IS NOT NULL` exclusion in the policy.
- **Four of the nine functions this sprint touches had already drifted from their last-committed migration file** (`remove_group_member`, `get_my_batch_groups`, `get_group_detail`, `leave_group` — none of the 30 `study-groups/` files nor the 6 `sprint8.0/` files were their true source; only `pg_get_functiondef` against the live DB was authoritative). All `CREATE OR REPLACE`s in `03_FUNCTIONS_guard_enrollment_paths.sql` are reproduced from that live-introspected body, not from any repo file, with additive/minimal diffs only.
- **Database design:** `study_groups.archived_at timestamptz` (NULL = active) + `archived_by`. New `'closed'` status on `study_group_members` (alongside `invited`/`active`/`requested`) with `closed_at`/`closed_reason` — archiving closes outstanding `requested`/`invited` rows to this status rather than deleting them or leaving them live; `join_group_by_token` reactivates a `closed` row back to `requested` on an explicit post-restore re-request, preserving the old closure as history. New `batch_group_archives` table (RLS enabled, zero client policies — read only via `get_batch_group_archive()`) holds one jsonb snapshot per archive event (`{group: {...}, member_count, members: [...]}`, built by reusing `get_batch_group_member_stats` rather than reimplementing it — its weekly figures are computed live relative to `CURRENT_DATE`, so freezing them was the whole point, a date-filter alone would not have worked).
- **`archive_batch_group`/`restore_batch_group`** (new, admin-only, `SECURITY DEFINER`): both lock the batch row (`FOR UPDATE`) first, so a concurrent enrollment action and an archive call either complete-then-commit first (honored) or see the committed archived state and are refused — same lock-first convention applied to every enrollment-adjacent function (`join_group_by_token`, `enroll_user_in_batch_group`, `approve_batch_join_request`, `reject_batch_join_request`). Both idempotent (repeat archive returns existing state without replacing the snapshot; repeat restore no-ops).
- **Deletion, fully closed for batch groups:** the RLS narrowing above (item 2) plus a `leave_group` fix — its last-active-member cascade-delete (previously unconditional for any group) now excludes `is_batch_group = true` rows; a batch group's lifecycle is exclusively archive/restore going forward. Ordinary-group `leave_group` behavior is byte-identical to before.
- **Frontend:** `AdminDashboard.jsx` — Active/Archived pill filter (client-side, cheap at 3 live batch groups) on the batch tab, per-row Archive/Restore buttons with confirmation dialogs explaining consequences, disabled while saving, "Archived on [date]" label; direct-add picker and Copy Invite Link both scoped to active batches only. `GroupDetail.jsx` — when `group.archived_at` is set, fetches `get_batch_group_archive` instead of the live `get_batch_group_member_stats`, reusing the same stats table render (identical row shape), "Archived" badge + date line in the header. `GroupJoin.jsx` — "This batch has ended" replaces the join CTA and the monitoring-disclosure line when `is_batch_group && archived_at`; `stats` is already `null` from the RPC in that case.
- **Post-deploy fix:** `archive_batch_group` used `NOW()` for the archive-event timestamp, which returns the enclosing transaction's start time — constant for the whole transaction. A restore-then-re-archive of the same group done inside one transaction (exactly what `04_TEST`'s single `BEGIN...ROLLBACK` does) computed the identical timestamp twice, violating `batch_group_archives`' `UNIQUE(group_id, archived_at)`. Fixed with `clock_timestamp()` instead (advances on every statement regardless of transaction boundaries — also the more correct choice for an audit marker of the actual moment captured), deployed via `06_FIX_archive_timestamp_precision.sql`.
- **Verification — `docs/database/sprint8.1/04_TEST_verify_archiving.sql` — ✅ 33/33 PASS**, including every `[CRITICAL]` check: admin-only archive/restore, non-batch/missing-group rejection, snapshot creation matching timestamp, active memberships untouched, outstanding requests closed (not deleted) with reason recorded, idempotent repeat archive (no duplicate snapshot, timestamp unchanged), idempotent repeat restore, all four enrollment paths refused once archived, both narrowed RLS policies genuinely block a direct client write, batch group survives its last member leaving (ordinary group still cascade-deletes, unchanged), closed request not auto-revived by restore, re-request after restore reactivates the closed row preserving its closure history, re-archive produces a genuinely new snapshot while the earlier one is retained, `get_batch_group_archive` returns the latest snapshot and is gated to professor/admin/super_admin only, listing surfaces (`get_admin_batch_groups` archived_at, `get_my_batch_groups` excludes archived). **Getting to a clean pass took several rounds of test-script fixes, not SQL-migration fixes** — worth recording since it looked alarming mid-flight: (1) `CREATE OR REPLACE` can't change a `RETURNS TABLE` function's output row shape, needed an explicit `DROP FUNCTION` first for `get_admin_batch_groups` (42P13); (2) `INSERT INTO _r` (the test's own results table) while `SET LOCAL ROLE authenticated` failed with 42501 — the temp table is owned by the session's original role, so a role-switched write to it has no grant; fixed by capturing results into variables while role-switched, `RESET ROLE`, then logging; (3) a check comparing JSON `null` (from `jsonb_build_object(..., NULL)`) against SQL `NULL` via `IS NULL` — different things in Postgres, `->` returns the jsonb scalar null which casts to the text `'null'`; (4) the "RLS blocks DELETE" check itself initially read as a FAIL — turned out to be checking `COUNT(*)` *before* `RESET ROLE`, so it was asking "can this RLS-restricted role even see the row" (no — it's not a group member) rather than "did the delete succeed" (a live diagnostic, `07_DIAGNOSTIC_rls_delete_investigation.sql`, confirmed the DELETE genuinely affected 0 rows and RLS is working correctly); (5) a variable (`v_archived_at1`) reused as scratch across unrelated checks had gone stale (overwritten to NULL by an earlier check) by the time the re-archive-timestamp check read it, making that check pass for the wrong reason — given a dedicated never-reused variable.
- **`05_TEST_concurrency_manual.sql` (two-Supabase-SQL-Editor-tab procedure for a genuine concurrent-transaction race) — attempted, not completed.** The manual two-tab choreography (hold a lock open in one tab, run a blocking call in another, resolve in a specific order) proved too fragile to coordinate reliably through this workflow — every failed attempt turned out to be stale/misordered fixture state correctly triggering the guard (e.g. a prior attempt had already archived the fixture group, so the next attempt's very first call correctly refused immediately, before any real concurrency was exercised), never an actual bug. Deliberately not pursued further: `04_TEST` already proves every guard fires correctly in isolation, and the `FOR UPDATE`-lock-first pattern used throughout is a standard, well-understood Postgres idiom, not a novel one this sprint invented. The fixture-reset issue in the script was fixed along the way (unconditional delete+recreate instead of skip-if-exists) for whoever wants to attempt it later.
- **Live click-through verification, production, authenticated as admin (15/09/2026):** Active/Archived filter, Archive action, Restore action, GroupDetail's frozen-snapshot view (correctly showed a zero-activity student row pulled from the saved snapshot, not live stats), and the public `/join/:token` page's "This batch has ended" state — every surface confirmed working with clean browser consoles. Test fixture (and one real student's incidental membership row picked up during earlier SQL testing) cleaned up afterward via direct `DELETE FROM study_groups` (cascades).
- **Quality Auditor follow-up, same day — three specific scenarios asked about before sign-off.** `docs/database/sprint8.1/08_TEST_auditor_followup.sql` — **✅ 7/7 PASS**, closing two genuine gaps in the original 33 checks (neither was actually covered before, despite the checks looking thorough — worth being direct about that): **(1) snapshot-capture failure → no partial state.** A transaction-scoped fault-injection trigger (proven pattern already used in `sprint7.4/04_TEST`: a `pg_temp` trigger function + `CREATE TRIGGER` inside the `DO` block, invisible to any other session since DDL isn't visible pre-commit, undone regardless by the test's own `ROLLBACK`) forces the snapshot `INSERT` to fail — confirms the batch stays active, the existing membership is untouched, and no orphaned snapshot row exists. **(2) frozen report vs. genuine later activity.** Not a proxy — real `reviews` rows inserted for a real student (picking flashcards that student had never reviewed, so no collision with `reviews_user_flashcard_unique`): one *before* archiving is captured in the snapshot; a second inserted strictly *after* archiving does **not** change the already-saved snapshot, while the live (non-archived) reporting path correctly reflects both — proving the freeze is real and this wasn't a trivial always-zero case. **(3) genuine concurrent-session locking — still not completed, disclosed as a residual, not swept under the rug:** true concurrency needs two independently-open database connections; the two-tab manual approach already failed repeatedly on human-timing coordination (never a code bug — every failure was stale fixture state correctly triggering the guard), and an automated single-script alternative via `dblink` was considered and rejected since it would require embedding a database password in a script. The `FOR UPDATE` lock-first pattern used throughout is standard and correct by construction, but has not been demonstrated against a real race.
- **Out of scope, per the sprint brief:** permanent deletion, Suspended/Retired states, automatic archiving, changes to shared-content access, delegated approvers, payment/roster tools, course/exam changes, automated nudges. Also not touched: the pre-existing, unrelated `group_type` vs `is_batch_group` desync noted above (D-15 scope) and the pre-existing `create_batch_group` gap that leaves `group_type` at its default `'custom'` instead of `'batch'` on new rows — flagged during pre-flight, orthogonal to archiving since D-16 gates everything on `is_batch_group`, not `group_type`.
- **Files:** *new* — `docs/database/sprint8.1/{00_DIAGNOSTIC_preflight,01_SCHEMA_add_archiving,02_FUNCTIONS_archive_restore,03_FUNCTIONS_guard_enrollment_paths,04_TEST_verify_archiving,05_TEST_concurrency_manual,06_FIX_archive_timestamp_precision,07_DIAGNOSTIC_rls_delete_investigation,08_TEST_auditor_followup}.sql` (all run/deployed by the operator except `05_TEST_concurrency_manual`, not completed — see above). *changed* — `src/pages/admin/AdminDashboard.jsx`, `src/pages/dashboard/Groups/GroupDetail.jsx`, `src/pages/public/GroupJoin.jsx`, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/tracking/changelog.md`, `docs/tracking/bugs.md`, `docs/active/now.md`.
- **✅ SQL deployed & verified live (33/33 + 7/7 PASS), live click-through verified in production, frontend committed and pushed (`6bc1796`). One residual disclosed: genuine concurrent-session locking not live-demonstrated (correct by construction, not proven by a real race).**

---

## Sprint 8.0: Batch invite-link joining with admin approval — Phase 8 sprint 1 (15/09/2026) — ✅ Just Completed

**Replaces guess-based batch enrollment with explicit approval.** Built for the founder's own coaching class going B2B (multiple batches per course level, e.g. separate May-attempt/Sept-attempt cohorts) — the old model assumed one batch group per course+institution, which real usage breaks, and a forwarded invite link was never proof of being a genuine student of that specific batch. Full design discussion (STOP-condition finding, "Quality Auditor" consultation, scope revisions) is in this session's transcript; decision recorded as blueprint.md §3.1 D-15.

### Same-day follow-up — Quality Auditor review of the completion report (15/09/2026)

A plain-language report of the sprint was sent to the operator's Quality Auditor for approval. Three points came back, all agreed and built the same day rather than deferred:

1. **Disclosure of monitoring.** The batch join screen (`GroupJoin.jsx`) didn't tell a student that joining lets the institution see their individual study activity and progress — a real gap, since batch membership does grant the professor that visibility. Added a one-line notice, shown only for batch groups (a custom peer group carries no such monitoring relationship, so it stays unlabelled).
2. **Automatic membership removal closed too.** The `fn_auto_enroll_batch_group` trigger's remaining branch — removing a student from their *old* matched batch group when their course changed — was found to use the exact same course+institution guessing the "add" side was already stripped of, just on the removal side. A course change could have silently dropped a student from the wrong batch. Since both branches were now gone, the trigger did nothing — retired entirely (`04_SCHEMA_retire_auto_enroll_trigger.sql`) rather than leaving dead machinery installed. Batch membership removal is now exclusively the pre-existing explicit `remove_group_member`/`leave_group` action.
3. **Nudges — confirmed out of scope, principle recorded.** No automated "inactive student" feature exists in this sprint. Recorded in blueprint.md D-15 for whenever one is designed: "no recorded activity in RevisOp" must never be treated as "not studying" — the app can't see offline study.
- **Files:** *new* — `docs/database/sprint8.0/{04_SCHEMA_retire_auto_enroll_trigger,05_TEST_verify_trigger_retired}.sql`. *changed* — `src/pages/public/GroupJoin.jsx`, `docs/active/blueprint.md`.
- **Status:** frontend + SQL written; `05_TEST` needs an operator run in Supabase SQL Editor before this line item is marked verified.

- **Pre-flight found a real STOP condition, reported before any code was written:** `fn_auto_enroll_batch_group()` (the `profiles` trigger) matched a batch group purely by `(batch_course, batch_institution)`, `LIMIT 1`, with no `group_id` awareness — if two batch groups ever shared the same course+institution (a real scenario once multi-batch is common), it could enroll a student into the wrong one. Live data had zero such duplicates today, but nothing prevented one later.
- **Design landed on, after two rounds of correction:** batch membership is never resolved by matching course+institution — only by an explicit action naming one specific student and one specific batch. Two paths, both explicit: (a) student clicks the batch's own invite link → `status='requested'`, invisible until approved; (b) admin picks one exact student + one exact batch in the UI → activates immediately (that pick *is* the approval, no `'requested'` step). `create_batch_group` no longer bulk-adds anyone, `fn_auto_enroll_batch_group` no longer creates any membership at all (cleanup-only now), `enroll_user_in_batch_group` no longer guesses — it takes an explicit `p_group_id` and raises if it isn't a real batch group.
- **Verified before building, not assumed:** group-shared content (`get_browsable_notes`/`decks`/`get_group_detail`) already gates everywhere on `sgm.status='active'`; a live RLS dump confirmed `notes`/`flashcards`/`flashcard_decks` have no group-share clause on the base tables at all (own/public/friends/admin only) — so a `'requested'` row is already fully inert for content access via any path. No paid/free content-tier concept was needed.
- **Also fixed while touching this code:** `create_batch_group` had no `is_admin()` guard at all in the live function — any authenticated user could call it. Same bug class as the already-fixed `enroll_user_in_batch_group`/`notify_access_granted` (security/15); added while replacing the function anyway.
- **Known residual, flagged not fixed:** `fn_auto_enroll_batch_group`'s remaining old-group cleanup branch (removes membership in a student's *previous* matched batch when their course changes) still resolves that group via the same course+institution `LIMIT 1` lookup — same theoretical wrong-group risk, but on the removal side, and it removes real existing membership rather than generating a guessed one, so it was left as-is.
- **SQL:** `docs/database/sprint8.0/{01_SCHEMA_add_requested_status, 02_FUNCTIONS_batch_join_approval, 03_TEST_verify_batch_approval_workflow}.sql` — all 3 run by the operator against production. **`03_TEST` — 20/20 PASS**, including every `[CRITICAL]` assertion: join never writes `profiles.course_level`/`institution`, repeated self-request is idempotent (1 row), approve/reject are admin-gated and atomic, an unapproved (`'requested'`) member is blocked from `get_group_detail`, explicit admin-add activates immediately with no guessing (a bogus `p_group_id` raises `'Not a batch group'`), and the old-group cleanup path still works.
- **Frontend:** `GroupJoin.jsx` — shows the batch's course/institution, "Request to Join" for a batch group (vs "Join Group" for ordinary groups), a pending-approval message instead of navigating away when the result is `'requested'`. `AdminDashboard.jsx` — new "Pending Batch Requests" table (Approve/Reject) in the Batch Groups tab, "Copy Invite Link" per batch group, and a new `BatchGroupPicker` (explicit dropdown + Add button, disabled until a real batch is chosen) next to already-enrolled users in User Management — replaces the old auto-guessing behind "Grant Access".
- **Explicitly not built this sprint** (cut during scoping, not forgotten): `emailRedirectTo`/cross-device signup confirmation, a redesigned "check your email" screen, locking the course selector at signup, delegated (non-admin) approvers, bulk roster upload, payment integration, and any course-progression/exam-attempt-change UI.
- **Files:** *new* — `docs/database/sprint8.0/{01_SCHEMA_add_requested_status,02_FUNCTIONS_batch_join_approval,03_TEST_verify_batch_approval_workflow}.sql`. *changed* — `src/pages/public/GroupJoin.jsx`, `src/pages/admin/AdminDashboard.jsx`, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/tracking/changelog.md`, `docs/active/now.md`.
- **✅ SQL deployed & verified live (20/20 PASS), frontend written against deployed signatures — ready to commit.**

---

## Sprint 7.12: concept_card — structured authoring + browse-only viewer — Phase 7 sprint 13 (14/09/2026) — ✅ Just Completed

**Last type in Phase 7's roster.** Concept cards are browse-only reference material, never part of the SRS review queue — no grade, no rung, no `reviews` row, ever (D-06).

- **Pre-flight (`docs/database/sprint7.12/00_DIAGNOSTIC_preflight.sql`) — ✅ run by the operator 14/09/2026, all 6 checks confirmed live:** `chk_flashcards_question_type` — exactly the 8-value enum docs already claimed, `concept_card` included. **0 pre-existing `concept_card` rows** (never properly built before this sprint). **0 `reviews` rows and 0 `review_events` rows for `concept_card`, ever** — confirming the StudyMode leak found in this sprint (see below) had never actually damaged real content, it was a live gap, not a live incident. `get_browsable_decks` confirmed as exactly one overload (`p_question_type text`), matching the DROP+CREATE target for the v6 migration.
- **The leak, found and fixed (Task B):** `get_study_queue` already excluded `concept_card` from the *due*-set (D-06, since Sprint 6.0). The actual gap was `StudyMode.jsx`'s separate `fetchFlashcards` filter (~line 352) — `cleanedData.filter(c => dueIds.has(c.id) || !reviewedIds.has(c.id))` — a "due OR never-reviewed" fallback that re-admits any never-graded card as "new," with no `question_type` check. Since a concept card by definition has no `reviews` row, it passed this fallback and could reach a student in the plain flashcard flip UI, gradeable via `apply_review` exactly like a real flashcard. **Fixed:** added an explicit `c.question_type !== 'concept_card'` clause to the same filter, so concept cards are excluded at every point in the pipeline, not just the RPC's due-set.
- **Representation (`src/lib/conceptCard.js`, new):** `front_text` = heading (concept name). `back_text` (NOT NULL) = summary, typed directly like a plain flashcard's back — not derived. `options` = jsonb array of `{term, definition}` objects (keyTerms, min 1) — third reuse of that column after mcq (7.5) and fitb (7.11), this time as object pairs. `correct_answer`/`hints`/`scenario`/`subtype`/`explanation` all stay NULL. `CONCEPT_MIN_TERMS`=1, `CONCEPT_MAX_TERMS`=10 (a judgment call, no requirement dictated the bound — mirrors the other types' small caps). `buildConceptOptions()` drops any row missing either half of a term/definition pair; `validateConceptTerms()` enforces at least one complete pair.
- **`get_browsable_decks` v6 — needed extending, and did (`docs/database/sprint7.12/01_FUNCTIONS_get_browsable_decks_v6_has_concept_card.sql`):** v5 had no per-deck signal for "does this deck contain a concept card," which Task D's conditional "Read Concepts" button needs. Added one additive return column, `has_concept_card BOOLEAN`, computed via `bool_or(fc.question_type = 'concept_card')` in the SAME visibility-filtered lateral subquery that already computes `visible_card_count` — no extra table scan. Standard DROP+CREATE (changing return shape isn't a plain `CREATE OR REPLACE`-able change) — only one overload existed, confirmed via pre-flight, so no arity ambiguity risk this time (unlike v5's own DROP+CREATE lesson). **✅ Deployed by the operator; `02_TEST_verify_get_browsable_decks_v6.sql` — 5/5 PASS**, including a mixed-deck fixture (one concept_card + one flashcard, same deck) confirming `has_concept_card=true`, a plain deck confirming `false` (not NULL), `visible_card_count` still counting both rows (type-agnostic, unchanged from v5), the `p_question_type='concept_card'` filter still narrowing correctly, and a NULL-session RAISE regression check.
- **Authoring (`FlashcardCreate.jsx`):** new "Concept Card" question-type option, **ungated** — open to all users (like `flashcard`/`theory`), not D-10-gated, since nothing is ever graded so there's no authoring-judgment risk to gate against. Front label becomes "Concept Name", Back label becomes "Summary" (both type-conditional, same pattern as fitb's "Sentence with blank"), plus a repeatable Key Terms list editor (term + definition pair inputs, add/remove, 1-10 rows) below the Back textarea. No "Why" field — concept cards are never graded. Validated at submit time via `validateConceptTerms`. Draft autosave/restore extended to carry `conceptTerms`.
- **Bulk upload — explicitly deferred, not built** (`BulkUploadFlashcards.jsx`), same call as `match_the_following`'s 7.8-C: keyTerms is a variable-length list of `{term, definition}` **pairs**, not a flat list of strings like fitb's acceptable-answers (`fitb_answers`, semicolon-delimited single cell). A second-level CSV-cell delimiter (e.g. `term:definition;term:definition`) would silently mis-split real content, since CA/accounting terms and definitions routinely contain their own colons and commas — confirmed live during verification: the test card's own "Appropriateness" definition read *"Measure of quality: relevance + reliability of audit evidence"*, which would have broken a naive `:`-delimited cell. Manual authoring only, documented inline in `BulkUploadFlashcards.jsx`.
- **Mixed-deck-aware browse viewer (`ReviewFlashcards.jsx` + new `src/components/flashcards/ConceptCardViewer.jsx`):** a deck tile with `has_concept_card=true` gets a secondary "Read Concepts" button (BookOpen icon) alongside its existing click-to-study area — opens a read-only modal: accordion list (heading always visible, click expands to summary + keyTerms), fetched directly via the 5-column deck-membership join (D-04) scoped to `question_type='concept_card'`, same visibility guard (`.or('visibility.eq.public,user_id.eq.<self>,visibility.eq.friends')`) as StudyMode's own fetch. Zero grade buttons, zero `apply_review` calls anywhere in this component. "Study" continues to open the normal StudyMode session, which now correctly excludes concept cards via the Task B fix.
- **`MyFlashcards.jsx` decision: no change needed, Browse Study Sets is the sole student-facing read surface.** Investigated first (pre-flight step 5): `FlashcardCard.jsx` (the tile MyFlashcards renders for every card, in both Grid and Grouped view) is already generic — it shows `front_text`/`back_text` under hardcoded "Question"/"Answer" labels with **zero type-specific rendering for any question type**, not just concept_card (mcq's options, fitb's acceptable-answers, match's left/right pairs are equally invisible there). So a concept-card batch already displays correctly today for content management (professor can see/edit the heading and summary), on the same terms as every other rich type. Building a special concept-card view into a content-management grid that treats every other type generically would be new, unrequested scope, not a gap-fill.
- **`questionTypes.js`:** `concept_card` added to `BROWSABLE_QUESTION_TYPES` (drives Browse Study Sets' type filter dropdown) and `formatQuestionType()`'s label map (`'Concept Card'`). **Deliberately NOT added to `GRADED_QUESTION_TYPES`** — that array drives StudyMode's shared single-tap grading branch, and concept cards must never render through any grading path.
- **`npm run build` clean; `npx eslint` clean** on every changed file — same 2 pre-existing `FlashcardCreate.jsx` unused-var errors + 1 unused-eslint-disable warning reconfirmed to predate this sprint (same baseline every sprint since 7.5 has reconfirmed).
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, professor session signed in directly by the operator in the driven browser — Claude never saw or entered a password):**
  - **Authoring:** Question Type selector correctly showed "Concept Card" between Theory and the professor-gated types (confirming it's ungated). Authored one real card (Auditing & Ethics → Audit Evidence, the same topic as 28 pre-existing gradeable cards) — "Audit Evidence — Types and Reliability" heading, a 2-sentence summary, and 2 key terms (Sufficiency / Appropriateness). Card header read "Item 1 — Concept Card"; Front label read "Concept Name"; Back label read "Summary". Toast: "1 study item(s) created successfully." **Confirmed via direct authenticated REST read:** `options=[{"term":"Sufficiency","definition":"..."},{"term":"Appropriateness","definition":"..."}]`, `correct_answer=null`, `explanation=null`, `subtype=null`, `scenario=null` — exactly the designed representation.
  - **StudyMode leak fix — proven with real data, not just code inspection:** the Skip-24hr button hit an unrelated pre-existing bug (`23505 duplicate key value violates reviews_user_flashcard_unique` — a real bug in the skip path, out of scope for this sprint, flagged separately) that blocked manually clicking through the deck's 29 cards one at a time. Instead, replayed StudyMode's exact fetch pipeline (deck's 5-column-join card set + `get_study_queue` due-set + `reviews` reviewed-set + the fixed filter) directly against the live DB via the authenticated session: the test concept card was confirmed **present** in the deck's raw 29-card set, **never due, never reviewed** (exactly the leak condition — the condition under which the pre-fix filter would have re-admitted it), and **absent** from the final filtered study list (24 cards, all `question_type: flashcard`, zero `concept_card`). This is the same code path StudyMode.jsx actually runs, replayed with real IDs against the real due/reviewed state, not a synthetic unit test.
  - **Read Concepts viewer:** button appeared correctly on the "Audit Evidence" deck tile in Browse Study Sets (only on decks with `has_concept_card=true` — confirmed absent on plain decks). Opened a modal: header "Audit Evidence" + "Reference material — not part of your review queue" subtitle, one accordion row "Audit Evidence — Types and Reliability" (expanded by default since it's the only card), summary text and both key terms (`Sufficiency — Measure of quantity...`, `Appropriateness — Measure of quality: relevance + reliability...`) rendered correctly. Collapse/expand toggle verified (chevron flips, content hides/shows). Close button verified.
  - **No `reviews` row ever created:** confirmed via a direct authenticated REST read against `reviews` filtered to the concept card's id, both mid-session and after all testing — **0 rows**, both times. Since `review_events` has zero client grants (RLS `REVOKE ALL FROM PUBLIC, anon, authenticated`) it couldn't be queried directly from the browser, but `apply_review` only ever writes both tables atomically in the same transaction — zero `reviews` rows is conclusive proof zero `review_events` rows exist either.
  - **Console clean** aside from the unrelated pre-existing skip-card bug (flagged as a separate out-of-scope task) and known dev-server service-worker registration noise.
  - **Cleanup:** the 1 QA row created during this session was deleted after verification (operator confirmed via AskUserQuestion) — same pattern as every prior sprint's live-verification cleanup. Confirmed via a fresh count: 700 → 699 flashcards.
- **Out-of-scope bug found, fixed same day as a dedicated follow-up (not part of this sprint):** `StudyMode.jsx`'s Skip-24hr action threw `23505 duplicate key value violates unique constraint "reviews_user_flashcard_unique"` — discovered incidentally while attempting manual click-through verification above, unrelated to concept_card. Root cause turned out to be a TOCTOU race in `skip_card`/`suspend_card`'s RPC bodies (non-atomic `UPDATE; IF NOT FOUND THEN INSERT`, no in-flight button guard), not the originally-guessed client-INSERT bug. Fixed via an atomic `ON CONFLICT DO UPDATE` upsert, SQL deployed & verified live 7/7 PASS — see the "Follow-up" entry at the top of `docs/tracking/bugs.md` and the changelog.
- **Files:** *new* — `src/lib/conceptCard.js`, `src/components/flashcards/ConceptCardViewer.jsx`, `docs/database/sprint7.12/{00_DIAGNOSTIC_preflight,01_FUNCTIONS_get_browsable_decks_v6_has_concept_card,02_TEST_verify_get_browsable_decks_v6}.sql` (all 3 ✅ run against production). *changed* — `src/lib/questionTypes.js`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx` (comment-only, bulk upload deferred), `src/pages/dashboard/Study/ReviewFlashcards.jsx`, `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`, `docs/tracking/{changelog,bugs}.md`, `docs/active/now.md`.
- **✅ SQL deployed & verified live (5/5 PASS), frontend live-verified end to end, ready to commit.**

---

## Sprint 7.11: fitb — Phase 7 sprint 12 (14/09/2026) — ✅ Just Completed

**The last of the 5 D-10-gated types (`mcq`/`correct_incorrect`/`case_study_mcq`/`match_the_following`/`fitb`) to get a real authoring path.** Unlike the others, `fitb`'s verdict is a genuinely new three-way shape, not a collapse into existing machinery (D-13, blueprint.md §3.1) — the entire point of this sprint was building that confidence-gated grading correctly, not just wiring up another authoring form.

- **Task 0 — verified the 7.10 bulk-upload gap first, before starting anything else.** Uploaded a real 3-row/1-`case_group` `case_study_mcq` CSV through `BulkUploadFlashcards.jsx` as the authenticated professor session (a `DataTransfer`-based file injection into the page's own `<input type="file">`, since this session drives an in-app browser without a native OS file picker — the page's own `handleFileSelect`/`parseCSV`/`uploadFlashcards` ran exactly as a real upload would). First attempt used the template's example subject "Auditing" — rejected with "Subject not found" (the real live subject is "Auditing & Ethics"; the template's example text was never meant to be copy-pasted literally). Second attempt with the correct subject/topic succeeded: "Successfully uploaded 3 study items." **Confirmed live via the app's own Grouped view and StudyMode:** all 3 rows shared one `batch_id` (shown as one "Audit Evidence (3 cards)" batch tile, uploaded together), one `scenario` (byte-identical, rendered correctly in StudyMode's collapsible CASE SCENARIO block), and correct per-row `correct_answer`/`options`/`back_text` (verified both via the Grid view's back_text display and a direct authenticated REST read of the `flashcards` table). **The 7.10 bulk-upload path was NOT broken** — no fix needed, just verification. **Bonus find:** the CSV template's `descriptions_case_study` (theory subtype) example row had a pre-existing bug predating this sprint — its `back` field contained an unquoted comma ("Net price = 1.25 x 0.9 = 1.125x cost, so a 12.5% margin over cost."), silently shifting every subsequent column in that one row. Fixed by quoting the field (confirmed via a Node script that re-parses the template with the app's own `parseCSVLine` logic and checks every row's field count against the header).
- **Representation (`src/lib/fitb.js`, new):** `options` = jsonb array of acceptable-answer strings (1-6), reusing the existing "answer options for MCQ-type questions" column with a generalized meaning (D-13's impact line, confirmed at build time — no new column). `front_text` holds the blank-containing sentence; `FITB_BLANK_PATTERN`/`FITB_BLANK_PLACEHOLDER` (`______`, 3+ underscores) validate a visible blank marker at authoring time (`validateFitbBlank`). `correct_answer` stays NULL (same pattern as `match_the_following` — no single scalar "the answer"). `explanation` = jsonb why-array, same column/shape as every graded type since Sprint 7.9. **The one function that must never diverge:** `normalizeFitbAnswer()` (trim → lowercase → strip punctuation via `[^\p{L}\p{N}\s]` → collapse whitespace) is called identically on the student's typed answer AND every authored acceptable answer, at both authoring-compaction time (`compactFitbOptions`) and grading time (`isFitbMatch`) — one shared function, not two similar-but-different copies. `deriveFitbBackText` (first accepted answer) and `splitFitbSentence` (splits `front_text` around the first blank marker, for StudyMode's inline `<input>`) round out the module.
- **The confidence-gated verdict (`StudyMode.jsx`) — the actual point of this sprint, a three-way behavior, not collapsed into MCQ's two-way one:**
  - **Match** → confident-correct path: `handleFitbSubmit` sets `fitbMatched=true`, reveal shows the typed answer in navy (matched styling), the unmodified `GradeButtonRow` appears (student still picks Easy/Medium/Hard), `onGrade={(g) => handleRating(g.rating, fitbMatched ? true : null)}` calls `apply_review` with `p_is_correct=true`.
  - **No match** → free-recall fallback, never "wrong": reveal shows an "ACCEPTED ANSWERS" box listing every `options` entry (not a single correct answer) + the typed answer underlined (not navy), same unmodified `GradeButtonRow`, same `handleRating` call now passing `null` since `fitbMatched` is `false`.
  - **There is no third "confident-wrong" state, verified by code inspection AND live DB evidence.** Grep of the entire fitb branch in `StudyMode.jsx` (new code, ~90 lines) shows exactly one call site passing a rating to `apply_review` for this type — `onGrade={(g) => handleRating(g.rating, fitbMatched ? true : null)}` — a ternary whose only two possible outputs are `true`/`null`. There is no `handleFitbWrong`, no auto-`submitReview('hard', false)` on reveal (unlike MCQ's wrong-path, which fires `apply_review` immediately — fitb never does this; both fitb paths wait for the student's own grade tap). This is the exact bug D-13 exists to prevent, and it does not exist in this implementation.
  - fitb needed its **own StudyMode render branch**, checked before `GRADED_QUESTION_TYPES.includes(...)` — same reasoning as `match_the_following` in 7.8: its grading behavior isn't the clean binary that array assumes. `GRADED_QUESTION_TYPES` (`src/lib/questionTypes.js`) is unchanged; `fitb` added only to `BROWSABLE_QUESTION_TYPES` (Task 5).
- **Authoring (`FlashcardCreate.jsx`):** new "Fill in the Blank" question-type option, gated professor/admin/super_admin (D-10) — visible in the selector alongside mcq/correct_incorrect/match_the_following/case_study_mcq. "Sentence with blank" textarea (front field) with the blank-marker hint text, a repeatable 1-6-row "Acceptable Answers" list editor (`updateFitbOption`/`addFitbOption`/`removeFitbOption`, mirroring `updateMcqOption` etc. but with no "correct index" to track), a Why textarea. Validated at submit time via `validateFitbBlank`/`validateFitbOptions`. Draft autosave/restore extended to carry `fitbOptions`; `isD10GatedType()` extended with `|| qt === 'fitb'` (fitb isn't in `GRADED_QUESTION_TYPES`, so that check alone would miss it on a role-downgraded draft restore, same gap `match_the_following` already had a fix for).
- **Bulk upload (`BulkUploadFlashcards.jsx`) — built, not deferred** (unlike match_the_following's 7.8-C): new `fitb_answers` CSV column, semicolon-delimited within one cell (e.g. `"₹2.5 lakhs;2.5 lakhs;250000;2,50,000"`, quoted since one variant itself contains a comma) — fitb has no case_study_mcq-style multi-row fan-out problem, so a flat multi-value cell is enough. Template CSV gets a new example row + updated column docs; `RECOGNIZED_QUESTION_TYPES` extended; the `42501` RLS-rejection error message now names `fitb` too. A new `isCsvGradedType()` helper (`GRADED_QUESTION_TYPES.includes(qt) || qt === 'fitb'`) drives the `options`/`explanation` row-building — `correct_answer` deliberately stays keyed off `GRADED_QUESTION_TYPES` alone (not the new helper), since fitb's `correct_answer` must stay NULL, not a stringified `null`.
- **`npm run build` clean; `npx eslint` clean** on every changed file — same 2 pre-existing `FlashcardCreate.jsx` unused-var errors + 1 unused-eslint-disable warning reconfirmed to predate this sprint (same baseline every sprint since 7.5 has reconfirmed).
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, professor session already authenticated in the driven browser — Claude never saw or entered a password):**
  - **Authoring:** Question Type selector correctly showed "Fill in the Blank" (professor-only). Authored one real card (Income Tax → Overview and Basic Concepts): "The basic exemption limit for individuals below 60 years is ______." with accepted answers "₹2.5 lakhs" and "2.5 lakhs" + a Why explanation. Card header read "Item 1 — Fill in the blanks". Toast: "1 study item(s) created successfully."
  - **StudyMode rendering:** the sentence rendered with an inline text input exactly where the blank marker was, trailing punctuation (the period after the blank) rendered correctly outside the input.
  - **Match path:** typed "2.5 LAKHS!!" (different case + trailing punctuation, testing normalization) → submit → revealed in navy (matched styling), WHY block shown, unmodified `GradeButtonRow` with real interval previews and no pre-selected default. Graded Easy → "Next review in 7 days". **Confirmed via direct authenticated REST reads:** `reviews.rung` advanced from new-card to rung 2 (interval 7 days, next_review_date +7 days — normal Easy transition, not forced anywhere unusual); `get_question_type_performance` RPC returned `fitb: reviewed_count=1, graded_count=1, answer_accuracy_pct=100` — confirming `is_correct=true` was actually written to `review_events` by `apply_review`, not just claimed by the UI.
  - **No-match path (after `reset_card` via RPC, to re-test the same card as new):** typed "Rs 3 lakhs" (no accepted variant matches) → submit → revealed underlined (not navy), an "ACCEPTED ANSWERS" box listing both accepted variants, WHY block, same unmodified `GradeButtonRow`. Graded Medium → "Next review in 3 days". **Confirmed via direct authenticated REST reads:** `reviews.rung` advanced normally to rung 1 (interval 3 days, matching the Medium button's preview) — **not forced to rung 0**, proving the free-recall self-grade is genuinely respected, not silently downgraded. The analytics aggregate's math after this second review is the decisive proof against a "confident-wrong" bug: `get_question_type_performance` still returned `graded_count=1` (not 2) with `answer_accuracy_pct=100` unchanged — since `graded_agg` in the RPC counts `is_correct IS NOT NULL` rows, and only the FIRST review (the matched one) could be contributing to that count, the second review's event is provably `is_correct=NULL`, not `false` (a `false` second event would have pushed `answer_accuracy_pct` down to 50, which did not happen).
  - **Console clean** throughout.
  - **Cleanup:** all 4 QA rows created during this session (3 case_study_mcq Task-0 rows + 1 fitb row) were deleted after verification (operator confirmed via AskUserQuestion) — same pattern as every prior sprint's live-verification cleanup. Confirmed via a fresh count: 703 → 699 flashcards.
- **Files:** *new* — `src/lib/fitb.js`. *changed* — `src/lib/questionTypes.js`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx` (+ one incidental template-CSV bugfix predating this sprint), `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`, `docs/tracking/changelog.md`, `docs/active/now.md`.
- **✅ Zero SQL needed, ready to commit.**

---

## Sprint 7.10: case_study_mcq — Phase 7 sprint 10 (14/09/2026) — ✅ Just Completed

**First genuinely new feature-building sprint since Sprint 7.8** (7.9 was pure schema hygiene, no new type). `case_study_mcq`: a shared scenario narrative followed by one or more independently-gradeable MCQ questions — each its own `flashcards` row (own SRS card, own rung, own schedule), scenario duplicated across every row in the case (not normalized into a separate table) so it renders on any review day without a JOIN, grouped by a shared `batch_id` (D-01 pattern).

- **Pre-flight (`docs/database/sprint7.10/00_DIAGNOSTIC_preflight.sql`) — ✅ run by the operator 14/09/2026, all 3 queries confirm expected.** Live `pg_get_constraintdef(chk_flashcards_question_type)`: exactly the 8-value enum docs already claimed, `case_study_mcq` included. Both `flashcards_gate_verdict_types_insert`/`_update` RESTRICTIVE policies' `with_check`: `case_study_mcq` present in both `<> ALL (ARRAY[...])` IN-lists alongside `mcq`/`correct_incorrect`/`match_the_following`/`fitb` (D-12's Sprint 7.9 migration only removed `integrated_case` from this same list, confirmed not to have touched `case_study_mcq`). `scenario` non-null count: **0** — confirmed genuinely inert before this sprint, exactly as blueprint.md/DATABASE_SCHEMA.md claimed. This session has only the anon key (no `pg_catalog`/`pg_policy` introspection access), same limitation every sprint since 7.5 has hit — the operator ran this in the Supabase SQL Editor.
- **Real finding: zero SQL needed** — same outcome as `true_false`/`correct_incorrect` (7.7) and `match_the_following` (7.8): `case_study_mcq` was already a live CHECK-constraint value and already in the D-10 gate's IN-list since Sprint 7.5, before any authoring UI existed for it.
- **Representation (`src/lib/caseStudyMcq.js`, new):** per-question fields identical to mcq's (`options`/`correct_answer`/`back_text`/`explanation`) — a case question IS an mcq question, just with a shared scenario instead of its own standalone stem. `emptyCaseQuestion()`, `validateCaseStudy()` (scenario non-empty + 2-8 question blocks, each validated via `validateMcqOptions()` from `src/lib/mcq.js`), `CASE_MIN_QUESTIONS`/`CASE_MAX_QUESTIONS`/`CASE_DEFAULT_QUESTIONS` = 2/8/3 — a judgment call (no requirement dictated the bounds; 2-8 mirrors match_the_following's pair bounds, 3 as a sane default case size).
- **Authoring (`FlashcardCreate.jsx`):** new "Case study MCQ" question-type option, gated professor/admin/super_admin (D-10), alongside mcq/correct_incorrect/match_the_following in the same dropdown. One shared Scenario textarea (skips the card-level Front/image block entirely — case_study_mcq has no single front field) + a repeatable question-block editor, each block a full mcq-shaped sub-editor (question stem, 2-6 options with a correct-option radio, own "Why"). On save, one authoring block `flatMap`s into N `INSERT` rows sharing one freshly-generated `batch_id`, the trimmed scenario text, and the form's shared `subject_id`/`topic_id` (single anchor topic per case, D-12's authoring clarification — same Course→Subject→Topic flow as every other type, no schema change). Draft autosave/restore extended to carry `caseScenario`/`caseQuestions`; `isD10GatedType()` already covers it via the widened `GRADED_QUESTION_TYPES` (no separate check needed).
- **Toast accuracy fix (same file, applies to all types):** the success toast now counts `flashcardsToInsert.length` (actual rows written) instead of `flashcards.length` (authoring blocks) — a pre-existing gap that case_study_mcq's 1-block-to-N-rows fan-out made visible (a 3-question case previously would have reported "1 study item created" despite writing 3 rows). Live-verified: a 1-block/3-question case correctly toasts "3 study item(s) created successfully."
- **Bulk upload (`BulkUploadFlashcards.jsx`) — built, not deferred**, unlike match_the_following's 7.8-C deferral: the flat CSV convention fits fine here, since each case question is still naturally one row (unlike match's variable-length left/right/mapping shape). Two new columns: `scenario` (the uploader repeats the exact same text on every row of a case) and `case_group` (an arbitrary uploader-chosen label, unique per case within the file, that links rows into one case). Rows sharing a `case_group` value resolve client-side into one shared `batch_id`, kept separate from the file-wide `batch_id` every other bulk-uploaded row in the same CSV already shares (a pre-existing Sprint-7.x behavior — one whole CSV upload = one batch — that a per-case override needed to coexist with, not replace). Template CSV gets a 3-row case_study_mcq example (shared scenario, one `case_group` value, 3 questions) plus updated column docs.
- **StudyMode (`StudyMode.jsx`) — reuses the shared branch, not a new one:** `case_study_mcq` added to `GRADED_QUESTION_TYPES` (`src/lib/questionTypes.js`) means it renders through the exact same `AnswerOption`-list/hybrid-grading branch as mcq/correct_incorrect, zero new grading logic. The only addition is a collapsible "CASE SCENARIO" block rendered above the question when `currentCard.scenario` is present — open by default (student needs to read it to answer), collapsible via a chevron toggle, reset to open on every card change (new `scenarioExpanded` state, added to the same per-card-reset `useEffect` as `mcqSelectedIndex`/`matchPairs`). Grading is byte-for-byte identical to MCQ's hybrid path: wrong → immediate `apply_review('hard', false)` + single Continue, no grade row; correct → reveal + unmodified `GradeButtonRow`, no pre-selected default.
- **`npm run build` clean; `npx eslint` clean** on every changed file — same 2 pre-existing `FlashcardCreate.jsx` unused-var errors + 1 unused-eslint-disable warning reconfirmed via `git stash` to predate this sprint (same baseline every sprint since 7.5 has reconfirmed).
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, professor session already authenticated in the driven browser — Claude never saw or entered a password):**
  - **Authoring:** Question Type selector correctly showed "Case study MCQ" (professor-only — confirmed present in the dropdown alongside Multiple Choice/Correct-Incorrect/Match the following). Authored one case (Auditing & Ethics → Audit Evidence, 3 questions — an inventory-valuation audit-opinion scenario) with a shared scenario textarea and 3 independent mcq-shaped question blocks (add/remove question, add/remove option, correct-option radio, per-question Why — all worked). Card header read "Item 1 — Case study MCQ".
  - **Fan-out on save:** toast read "3 study item(s) created successfully" (not "1") — confirms the toast-accuracy fix and the fan-out both work. Direct REST read (via the browser's own authenticated session) confirmed all 3 rows share one `batch_id`, one `scenario` (byte-identical across all 3), the same `subject_id`/`topic_id`, correct per-question `options`/`correct_answer`/`back_text`/`explanation`.
  - **Browse Study Sets grouping:** My Study Sets' "Grouped" view showed the 3 rows as one **"Audit Evidence (3 cards)"** batch card (uploaded together) — deck-level grouping (Auditing & Ethics → Audit Evidence, 31 total cards) came for free via the existing 5-column join, no new code needed. "Case study MCQ" appears correctly in Browse Study Sets' Question Type filter dropdown; selecting it narrowed to exactly the one deck containing the type.
  - **StudyMode — scenario rendering, every question in the case:** the "CASE SCENARIO" collapsible block rendered correctly above the question on **all 3 different question cards** in the case (not just the first), each time with the identical scenario text — confirms per-row duplication renders correctly with zero JOIN, on any question. Collapse/expand toggle verified working (chevron flips, text hides/shows).
  - **StudyMode — wrong path:** selected an incorrect option — immediate reveal (correct=navy, wrong="You chose this"), WHY block shown, session ledger incremented "Hard 1" **before** tapping Continue (`apply_review('hard', false)` fires on reveal), toast read "Progress saved! Next review in 1 day". Single Continue button, no GradeButtonRow — identical to MCQ's wrong path.
  - **StudyMode — correct path (×2, different questions):** selected the correct option on 2 more questions in the same case — revealed with no auto-submit (ledger unchanged until grade tap), unmodified `GradeButtonRow` appeared with real interval previews and no pre-selected default; graded Easy (→"Next review in 7 days") and Medium (→"Next review in 3 days") respectively — **confirms each question in the case schedules independently** (1 day / 7 days / 3 days, three different intervals for three questions in the same case, proving they're genuinely separate SRS cards, not one card with three sub-answers).
  - **Console clean** throughout.
  - **Cleanup:** the 3 rows created during this live-verification pass were QA artifacts, deleted after verification (operator confirmed via AskUserQuestion) — same pattern as every prior sprint's live-verification cleanup. Confirmed via a fresh count: 702 → 699 flashcards.
- **Not live-tested this sprint:** the bulk-upload CSV path (built and code-reviewed, `npm run build`/`npx eslint` clean, but not exercised through an actual file upload in the browser) — flagged here rather than silently claimed as verified.
- **Files:** *new* — `src/lib/caseStudyMcq.js`, `docs/database/sprint7.10/00_DIAGNOSTIC_preflight.sql` (pending operator run). *changed* — `src/lib/questionTypes.js`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `src/pages/dashboard/Study/StudyMode.jsx`, `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`, `docs/tracking/changelog.md`, `docs/active/now.md`.
- **✅ Pre-flight confirmed live, SQL needed: zero, ready to commit.**

---

## Sprint 7.9: Schema Hygiene (extended scope, 14/09/2026) — all 4 items ✅ Just Completed

**Four items, not three — item 4 (D-14) was raised by Anand mid-session, right after items 1-3's SQL had already run and been verified live, but before anything was committed.** Kept in Sprint 7.9 rather than split into a new sprint number, since nothing had shipped to git yet. Touches live schema/data but not RPCs, StudyMode's core grading logic, or the SRS ladder.

### Items 1-3 ✅ Just Completed — `test_your_understanding` collapse + `explanation` split + `integrated_case` removal

**Three small, independent hygiene items accumulated during Phase 7 design work — not open questions, all resolved decisions being executed.**

- **Pre-flight (mandatory, run before any migration) — all 7 checks, real numbers:**
  1. Live `chk_flashcards_question_type`: 11 values (`flashcard`, `mcq`, `true_false`, `correct_incorrect`, `theory`, `case_study_mcq`, `integrated_case`, `match_the_following`, `fitb`, `concept_card`, `test_your_understanding`).
  2. `test_your_understanding`: **2 rows** — both confirmed by the operator to be Claude-authored QA/testing artifacts from Sprint 7.7's live verification (created 13/09/2026), not real content.
  3. `integrated_case`: **0 rows** — confirmed, no STOP condition triggered.
  4. `points_to_remember` non-null by type: `mcq` (4), `true_false` (2), `correct_incorrect` (2), `match_the_following` (2) — **all 10 also confirmed by the operator as the same class of QA/testing artifact** (these question types were only added to the live app across Sprints 7.5-7.8; no professor has authored real graded content yet).
  5. `subtype` non-null: **0** — confirmed genuinely inert.
  6. D-10 RESTRICTIVE policy pair (`flashcards_gate_verdict_types_insert`/`_update`): `integrated_case` present in both `with_check` IN-lists — a stale reference worth cleaning up in the same migration (dropping the CHECK-constraint value alone would already make it uninsertable).
  7. `GRADED_QUESTION_TYPES` (`src/lib/questionTypes.js`) confirmed via grep, no DB query needed: `['mcq','true_false','correct_incorrect']` — does not include `theory`/`test_your_understanding`.
- **Operator decision (AskUserQuestion, this session):** given all 12 rows touched by steps 2+4 are confirmed QA artifacts, chosen path was **delete them, then ship a clean additive-only migration** — not classify-and-migrate. Real IDs pulled live and hardcoded into the DELETE (not a blanket `WHERE question_type IN (...)`, to keep the migration auditable and scoped to exactly the rows verified).
- **Part A — `test_your_understanding` collapse (D-10 correction):** dropped from the live CHECK constraint (no data migration needed — the only 2 rows were deleted per the operator's choice above). `FlashcardCreate.jsx`'s Question Type selector no longer offers it. `theory` now requires a **Subtype** `<Select>` (`pure_theory` | `descriptive_case_study`, from new `THEORY_SUBTYPE_LABELS` in `questionTypes.js`) rendered directly below the Back textarea — validated in the same submit-time loop as MCQ options/match pairs, autosaved/restored in the draft-recovery flow (with a fallback: a stale pre-7.9 draft carrying `test_your_understanding` restores to `theory` instead). `BulkUploadFlashcards.jsx` gets a matching required `subtype` CSV column for `theory` rows (template updated with one example row per subtype), `test_your_understanding` removed from `RECOGNIZED_QUESTION_TYPES`.
- **Part B — `explanation` column split:** `ALTER TABLE flashcards ADD COLUMN explanation jsonb` (nullable, additive, same array-of-strings shape as `points_to_remember`). Graded types (`mcq`/`true_false`/`correct_incorrect`/`match_the_following`) now write their "Why" text to `explanation` instead of `points_to_remember` in both `FlashcardCreate.jsx` and `BulkUploadFlashcards.jsx` (mechanical swap — same `toPointsToRemember()` helper from `src/lib/mcq.js`, just a different target field). `StudyMode.jsx`'s two post-reveal WHY blocks (the shared `GRADED_QUESTION_TYPES` branch and `match_the_following`'s own `MatchZone` branch) now read `currentCard.explanation`. Free-recall types (`flashcard`/`theory`/`concept_card`) keep `points_to_remember` untouched (was already unused by any authoring path — no behavior change there). No backfill needed: the only rows that ever had graded-type `points_to_remember` data were the 10 QA rows deleted in the same migration.
- **Part C — `integrated_case` removal (D-12):** dropped from the CHECK constraint in the same `ALTER` as Part A (0 pre-existing rows, no STOP needed). Also stripped the stale `integrated_case` mention from both D-10 RESTRICTIVE policies via `ALTER POLICY ... WITH CHECK (...)` in the same migration — no separate deploy. No frontend selector referenced it (confirmed by grep — it only appeared in `questionTypes.js`'s `formatQuestionType` map as a dead fallback label, now removed).
- **Migration (items 1-3):** `docs/database/sprint7.9/{00_DIAGNOSTIC_preflight,01_SCHEMA_sprint7.9_hygiene,02_TEST_verify_sprint7.9}.sql`. **✅ All run against production by the operator (this session has only the anon key, same limitation Sprint 7.8 hit — could not run pg_catalog/pg_policy introspection itself).** `02_TEST` mirrors the Sprint 7.5 self-contained `BEGIN...ROLLBACK` + role-impersonation pattern: **17/17 checks PASS** — the 12 rows are gone; `test_your_understanding`/`integrated_case` now hit a CHECK violation on insert; all 9 surviving types (including a real-`profiles.id` fixture, after an early test-script bug tripped `flashcards_user_id_fkey` on a random UUID — fixed, not a migration bug) still insert cleanly; `explanation` column shape confirmed (`jsonb`, nullable); neither RESTRICTIVE policy mentions `integrated_case` any more; D-10 regression re-confirmed post-`ALTER POLICY` (student `mcq` insert still rejected, professor's still succeeds — the policy rewrite replaced the whole `WITH CHECK` expression, not just a substring, so this needed re-verifying, not assuming).
- **`npm run build` clean; `npx eslint` clean** on every changed file — same 2 pre-existing `FlashcardCreate.jsx` unused-var errors + 1 unused-eslint-disable warning reconfirmed baseline (every sprint since 7.5).
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, professor session already authenticated in the driven browser — Claude never saw or entered a password):**
  - **Question Type selector:** confirmed exactly 6 options, no `test_your_understanding` (Flashcard, Theory, Multiple Choice, True/False, Correct/Incorrect, Match the following).
  - **Theory + subtype, both values:** authored one `theory` card with `subtype='pure_theory'` and one with `subtype='descriptive_case_study'` — Subtype `<Select>` rendered both options with the exact `THEORY_SUBTYPE_LABELS` text, required-field validation held, both persisted correctly (confirmed via a direct Supabase client read from the page).
  - **`explanation` end-to-end:** authored a fresh `mcq` card with a "Why" explanation, confirmed the row saved with `explanation` populated and `points_to_remember: null`, then studied it live in StudyMode — the WHY block rendered the explanation text correctly post-reveal alongside the "Correct answer" tag.
  - **`integrated_case` uninsertable:** confirmed via `02_TEST`'s CHECK-violation assertion (not re-tested in the UI — there was never a UI path to it).
  - **Console clean** throughout.
  - **Cleanup:** the 3 rows created during this live-verification pass (2 theory + 1 mcq) were themselves QA artifacts — deleted after verification (operator confirmed via AskUserQuestion), keeping the live DB free of test data exactly as the earlier 12-row cleanup did.
- **Files:** *new* — `docs/database/sprint7.9/{00_DIAGNOSTIC_preflight,01_SCHEMA_sprint7.9_hygiene,02_TEST_verify_sprint7.9}.sql` (all 3 ✅ run against production). *changed* — `src/lib/questionTypes.js`, `src/lib/mcq.js` (doc comment only), `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `src/pages/dashboard/Study/StudyMode.jsx`, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/active/now.md`, `docs/tracking/changelog.md`.
- **Items 1-3 individually SQL-deployed & verified live (17/17 PASS), frontend live-verified end to end.**

### Item 4 ✅ Just Completed — `true_false` removed, merged into `correct_incorrect` (D-14)

**Raised by Anand immediately after items 1-3 landed**, noticing `true_false`/`correct_incorrect` look like the same question type twice. Investigated rather than assumed — checked the CA Revision Portal sibling project on request.

- **Finding:** CA Revision Portal's own `docs/SCHEMA.md` states `correct_incorrect`'s renderer is "Identical to `true_false`, with Correct/Incorrect buttons instead" — no other distinction documented anywhere (same question fields, same explanation field, same confidence-rating flow). Real-usage census of that project's actual, HTML-loaded chapter files: `correct_incorrect` used **6 times** (5 Auditing & Ethics chapters + 1 Business Laws chapter); `true_false` used **0 times** — its only occurrence in the whole codebase sits in `data/chapter-data.js`, an orphaned legacy file confirmed (via grep) to have zero `<script>` references from any HTML page. `docs/DECISIONS.md` there never weighs one against the other — same shape as D-12's `case_study_mcq`/`integrated_case` finding, down to "even the project that designed the type never used it."
- **D-14 drafted** in `blueprint.md` §3.1, right after D-13, with a forward-pointer added to D-10's "What" line (same superseded-note pattern D-12 already uses). Recommendation: keep `correct_incorrect` (the one with real usage), retire `true_false`.
- **Kept inside Sprint 7.9 rather than a new sprint number** (operator's correction, this session — "I was under impression that we can work it in the same sprint 7.9").
- **Pre-flight (`03_DIAGNOSTIC_preflight_true_false_removal.sql`), all 6 checks, real numbers:** live CHECK constraint had both types (9 values, post items-1-3); `true_false` — **0 rows**; `correct_incorrect` — **0 rows** (informational, unaffected either way); both D-10 RESTRICTIVE policies listed `true_false` in their IN-list; `srs_ladder_curves` had no `true_false`/`correct_incorrect`-specific rows, only `_default` (rungs 0-7, 1/3/7/14/30/60/120/240 days) — nothing goes orphaned; no `pg_proc` body anywhere hardcodes `'true_false'`. **Net effect: a pure narrowing migration** — no DELETE, no UPDATE, no backfill.
- **SQL — `04_SCHEMA_true_false_removal.sql`:** narrows `chk_flashcards_question_type` to 8 values (drops `true_false`) and drops the stale `true_false` mention from both D-10 RESTRICTIVE policies via `ALTER POLICY ... WITH CHECK (...)`. **✅ Deployed. `05_TEST_verify_true_false_removal.sql` — 13/13 PASS**: `true_false` now hits a CHECK violation; all 8 surviving types insert cleanly; neither policy mentions `true_false` any more; D-10 regression re-confirmed post-`ALTER POLICY` (student `correct_incorrect` insert still rejected, professor's still succeeds).
- **Frontend:** `isTwoWayVerdictType()` retired in both `FlashcardCreate.jsx` and `BulkUploadFlashcards.jsx` — with only `correct_incorrect` left, the 6 call sites collapsed to a direct `question_type === 'correct_incorrect'` check rather than keeping a now-pointless 2-member abstraction (consistent with "don't design for hypothetical future requirements"). `src/lib/questionTypes.js`: `VERDICT_OPTION_LABELS['true_false']` removed (only `correct_incorrect` entry remains), `GRADED_QUESTION_TYPES` → `['mcq','correct_incorrect']`, `BROWSABLE_QUESTION_TYPES` and `formatQuestionType` both drop `true_false`. `BulkUploadFlashcards.jsx`'s CSV template's `true_false` example row repurposed into a second `correct_incorrect` example (same statement text, correct_incorrect type/columns); `RECOGNIZED_QUESTION_TYPES`, doc-comment lines, and the `42501` error message all updated to name only `mcq`/`correct_incorrect`.
- **`npm run build` clean; `npx eslint` clean** — same 2 pre-existing `FlashcardCreate.jsx` baseline errors reconfirmed.
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, professor session):** Question Type selector confirmed exactly 5 options, no True/False (Flashcard, Theory, Multiple Choice, Correct/Incorrect, Match the following); Browse Study Sets' filter dropdown also confirmed clean of True/False. Authored a fresh `correct_incorrect` card ("A deduction under Section 80C requires actual payment, not merely accrual.") with a Why explanation — confirmed via a direct Supabase client read that `options`/`correct_answer`/`back_text`/`explanation` all persisted correctly. Studied it live: statement rendered with A/B (Correct/Incorrect) options, selecting the correct one revealed the "Correct answer" tag and the WHY block with the exact explanation text, graded Easy successfully. Console clean throughout. Test row deleted afterward (same QA-cleanup pattern as items 1-3).
- **Files:** *new* — `docs/database/sprint7.9/{03_DIAGNOSTIC_preflight_true_false_removal,04_SCHEMA_true_false_removal,05_TEST_verify_true_false_removal}.sql` (all 3 ✅ run against production). *changed* — `src/lib/questionTypes.js`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/active/now.md`.
- **✅ All 4 items now ready to commit together** — SQL deployed & verified live for all 4 (17/17 + 13/13 PASS = 30/30), frontend live-verified end to end for all 4, one combined commit.

---

## Just Completed ⏳ — Sprint 7.8: Match the Following — Phase 7 sprint 9 (14/09/2026)

**First genuinely new interaction mechanic since mcq.** Left-item/right-item pairing, not a collapse into the shared mcq/true_false/correct_incorrect machinery the way 7.7's types were — the verdict is still deterministic (fully correct or not), so the hybrid-grading contract from mcq applies unchanged; only the rendering and the "how do we even get a verdict" flow (build-up-then-submit, not a single tap) are new. `apply_review`/`review_events`/both analytics RPCs needed zero changes.

- **✅ Pre-flight run by the operator 14/09/2026** (`docs/database/sprint7.8/00_DIAGNOSTIC_preflight.sql`, this session has only the anon key so couldn't run it itself): live `pg_get_constraintdef(chk_flashcards_question_type)` includes `'match_the_following'::text` exactly; both `flashcards_gate_verdict_types_insert`/`_update` RESTRICTIVE policies' `with_check` list it exactly in their `<> ALL (ARRAY[...])` clause; zero pre-existing `match_the_following` rows. **Real finding: this sprint needed zero SQL** — `match_the_following` was already added to both the CHECK constraint and the D-10 gate back in Sprint 7.5, exactly as DATABASE_SCHEMA.md/blueprint.md D-10 claimed, no `fitb`-style naming drift this time. Same outcome as true_false/correct_incorrect in 7.7.
- **7.8-A representation (`src/lib/matchTheFollowing.js`, new):** `options` jsonb = `{ left: string[], right: {k,v}[], correct: {[leftIndex]: k} }` — deliberately NOT a flat array like mcq's, since the correct mapping needs to live colocated with the `right` list it references rather than double-encoded into `correct_answer` (which stays NULL for this type — there's no single scalar "the answer"). `back_text` auto-derived as a flattened `"left — key"` summary via `deriveMatchBackText()`. Right-item keys (A/B/C...) are assigned by `keyForRightIndex()` from list position, never professor-typed. `buildMatchOptions()` assembles the final shape at save time; `validateMatchPairs()` enforces equal-length left/right lists + a complete mapping. Left/right locked to the same length this sprint — no distractor right-options (explicit future refinement).
- **7.8-B authoring (`FlashcardCreate.jsx`):** new "Match the following" question-type option, gated identically to mcq/true_false/correct_incorrect (`canAuthorGradedTypes`). Left items and right items are two independent add/remove list editors (2-8 rows each) rather than paired rows — removing a right item nulls or reindexes any correct-mapping entries pointing at or past it (`removeMatchRight`). A "Correct Mapping" section renders one `<Select>` per left item listing the current right items by their live letter. Front-side field label reads "Instructions" for this type. Draft autosave/restore extended to carry the 3 new fields; the role-downgrade check on restore (previously `GRADED_QUESTION_TYPES.includes(...)`) now uses a local `isD10GatedType()` so a stale draft correctly falls back to `flashcard` for a non-professor even though this type isn't in `GRADED_QUESTION_TYPES`.
- **7.8-C bulk upload — explicitly deferred, not built:** the variable-length shape doesn't fit the flat CSV convention the other graded types share; `BulkUploadFlashcards.jsx` untouched.
- **7.8-D StudyMode (`StudyMode.jsx` + new `src/components/revisop/MatchZone.jsx`):** genuinely new render branch, checked *before* `GRADED_QUESTION_TYPES.includes(...)` so `match_the_following` never falls into the shared AnswerOption-list path. `MatchZone` implements pick-a-left-row/assign-a-right-badge with re-pick and re-assign (assigning an already-used right badge un-assigns it from its previous row) — translated from the design reference's interaction logic (`docs/active/design-review/revisop-pass2-reference.jsx` `MatchZone`, lines 649-730) into the project's real tokens (`rounded-rec`, `--rv-navy`/`--rv-slate` states, `font-plex-mono` badges), NOT its inline-style/green-for-correct treatment — reveal states follow `AnswerOption`'s established navy-correct/slate-missed convention instead. Because the answer builds up across multiple taps, a "Check Answers" button (enabled only once every row has an assignment) is the explicit commit action — there's no single tap that completes the interaction the way MCQ's does. On submit, every row reveals its own correctness (navy check / slate cross + "Correct answer: K — text" hint) **regardless of the overall verdict**. Overall verdict is **all-or-nothing** — every row must match, 3-of-4 still grades as wrong; a deliberate simplification, not an oversight, worth revisiting once this is live if it feels wrong in practice. Hybrid grading unchanged: wrong → `apply_review('hard', false)` fires immediately on Check-Answers + a single Continue; fully correct → reveal + the existing unmodified `GradeButtonRow`, no pre-selected default.
- **`GRADED_QUESTION_TYPES` deliberately unchanged** (`src/lib/questionTypes.js`) — that array specifically drives the shared single-tap list branch; `match_the_following` renders through its own branch instead, so adding it there would be wrong, not just unnecessary. `BROWSABLE_QUESTION_TYPES` gets the one-line addition so Browse Study Sets' filter can narrow to it; `formatQuestionType()` already had the `'Match the following'` label from Sprint 6's original type-documentation pass.
- **`npm run build` clean; `npx eslint` clean** on every changed file — the 2 pre-existing `FlashcardCreate.jsx` unused-var errors (`showCustomSubject`/`showCustomTopic`) + 1 unused-eslint-disable warning reconfirmed via `git stash` to predate this sprint (same baseline every sprint since 7.5 has reconfirmed).
- **✅ Live verification DONE (14/09/2026, dev server → live Supabase, real professor + student accounts, account owner typing credentials directly into the browser Claude was driving — Claude never saw or entered a password):**
  - **Authoring (professor):** created one 4-pair match_the_following card (Cost Sheet formulas) and one 3-pair card (SRS ratings), both Public. Question Type selector correctly showed "Match the following"; card header showed "Match the following 1"; front label read "Instructions" with the right placeholder. Left/right independent add/remove editors worked (removed a row from each list mid-authoring, letters correctly relabeled A/B/C, no orphaned mapping). Correct Mapping selects showed live "K — text" options and updated as right items were typed. `back_text` derived exactly as designed: `"Machine hour rate — D; Direct labour hour rate — B; Prime cost percentage rate — C; Rate per unit of output — A"`, confirmed in My Study Sets.
  - **StudyMode — wrong path (student):** assigned all 4 rows with 2 deliberately swapped (2 correct, 2 wrong) — session ledger incremented "Hard 1" **before** tapping Continue, proving `apply_review('hard', false)` fires on Check Answers, not on Continue. Reveal showed navy checks on the 2 correct rows and slate crosses + "Correct answer: K — text" hints on the 2 wrong rows, simultaneously, regardless of overall verdict. WHY block rendered. Single Continue button, no GradeButtonRow. "Study session complete!" fired correctly for the 1-card deck.
  - **StudyMode — correct path (student):** on the second card (3 pairs, all correctly matched), reveal showed all-navy checks and the unmodified `GradeButtonRow` appeared with real interval previews (1d/3d/7d, new-card ladder entry) and no pre-selected default — ledger stayed 0/0/0 until the grade tap. Tapping Easy graded and advanced together; ledger incremented "Easy 1"; "Study session complete!" fired.
  - **Due-filter regression proof:** re-entering the deck after grading card 1 served only the still-new card 2 (never the already-graded, now-scheduled-forward card 1) — confirms `get_study_queue`'s due/never-reviewed filter needed zero special-casing for this type.
  - **Student-side gating:** question-type selector on the student's own Create page showed only Flashcard/Theory/Test your understanding — no client-side leak of any of the 4 D-10-gated types with a real authoring UI.
  - **Browse Study Sets filter:** "Match the following" appears in the Question Type dropdown; selecting it narrowed 465→1 card to exactly the professor's deck.
  - **Analytics payoff, confirmed live on both sides:** professor Dashboard's "Accuracy by question type" widget showed a real "Match the following" row; student Progress.jsx's "Performance by Question Type" showed `match_the_following: 2/2 reviewed, Recall success 50%, Answer accuracy 50%` — exactly right for 1 wrong + 1 correct grade, with zero changes needed to `get_question_type_performance`/`get_educator_accuracy_by_qtype`.
  - **Console clean** throughout (one benign, unrelated `invalid_credentials` 400 from an earlier login attempt during account switching — not a code error).
- **Naming follow-up (Anand's catch, same session):** the Create Study Item page's per-block heading read just `"Flashcard 1"` / `"MCQ 1"` / etc. — Anand asked to confirm this was in line with the nomenclature where "flashcard" names one item type and "Study Set" names the group. Clarified via a quick discussion of options: keeping the type name as-is but making the per-block heading itself read more explicitly as an item count. Changed `FlashcardCreate.jsx`'s `<CardTitle>` from `{formatQuestionType(card.questionType)} {index + 1}` to **`Item {index + 1} — {formatQuestionType(card.questionType)}`** — e.g. "Item 1 — Flashcard", "Item 1 — Match the following". Type names (`formatQuestionType()`'s output) are unchanged everywhere else (Question Type dropdown, Progress/Dashboard widgets, Browse Study Sets filter) — only this one heading pattern changed. `npx eslint` clean (same 2 pre-existing baseline errors); live-verified both the default Flashcard state and after switching to Match the following.
- **Real bug found and fixed along the way:** `Progress.jsx` had its own long-stale, independently-maintained `QT_LABELS` map (keys `match`/`fill_blank`/`short_answer`/`case_study` — none of these are or ever were real `chk_flashcards_question_type` values) instead of using the shared `formatQuestionType()` from `src/lib/questionTypes.js` (the fix Sprint 7.6 built specifically so this kind of drift couldn't happen, and the exact same class of bug Sprint 7.5 already fixed once in `Dashboard.jsx`). Live-observed: the new `match_the_following` row rendered as the raw db key instead of "Match the following". Fixed by deleting the local map and importing the shared helper — one-line `qtLabel` now delegates to `formatQuestionType()`, closing the drift permanently (any future type addition needs zero Progress.jsx change). Re-verified live after the fix: renders "Match the following" correctly. `npx eslint` clean.
- **Files:** *new* — `src/lib/matchTheFollowing.js`, `src/components/revisop/MatchZone.jsx`, `docs/database/sprint7.8/00_DIAGNOSTIC_preflight.sql` (✅ run against production — zero SQL needed). *changed* — `src/lib/questionTypes.js`, `src/components/revisop/index.js`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/Study/Progress.jsx` (bonus fix), `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`, `docs/active/now.md`, `docs/tracking/changelog.md`.
- **✅ Committed and pushed** (`b85afe0`, on `main`) — confirmed via this session's git log while working Sprint 7.9; this note had gone stale (the commit landed after it was written, same pattern as the 7.5/7.6 corrections above).

---

## Just Completed ✅ — Sprint 7.7: True/False, Correct/Incorrect + free-recall labels — Phase 7 sprint 8 (13/09/2026)

**Second and third data points that the question-type architecture generalizes cheaply.** `true_false`/`correct_incorrect` reuse 100% of the mcq machinery (representation, StudyMode rendering, hybrid grading, `apply_review`); `theory`/`test_your_understanding` reuse 100% of the plain-`flashcard` front/back path. Neither needed new engine code.

- **Pre-flight run and reviewed (13/09/2026):** `docs/database/sprint7.7/00_DIAGNOSTIC_preflight.sql` confirmed exactly one live `get_browsable_decks` signature (`p_question_type text` — no leftover overload), and that the live `chk_flashcards_question_type`/D-10-policy definitions matched this sprint's drafted `01_SCHEMA` exactly (no adjustment needed before deploying).
- **Real finding: `true_false`/`correct_incorrect` needed zero SQL.** Both were already live CHECK-constraint values AND already in the D-10 RESTRICTIVE gate's IN-list as of Sprint 7.5 (that policy was written to cover all 7 verdict-bearing types at once, before any of them but mcq had a real authoring UI). This sprint only had to build the authoring surface.
- **`test_your_understanding` was the opposite case — a genuine hard SQL prerequisite, now cleared.** Confirmed (again, via 00_DIAGNOSTIC) not live in `chk_flashcards_question_type`. **✅ `01_SCHEMA_add_test_your_understanding_type.sql` deployed to production (13/09/2026)** — widens the constraint (purely additive, no existing row affected, no RLS change, since it's a free-recall type that was never going in the D-10 gate). **✅ `02_TEST_verify_new_question_types.sql` run against production — all 6 rows PASS**: test_your_understanding now insertable, theory unaffected, and (regression check) true_false/correct_incorrect still rejected for a real student profile and still accepted for a real professor profile — confirming this sprint's authoring-UI work touched zero RLS.
- **7.7-A representation (`src/lib/questionTypes.js`):** `VERDICT_OPTION_LABELS` — `true_false` → `["True","False"]`, `correct_incorrect` → `["Correct","Incorrect"]` — auto-populated, never professor-typed (the one real divergence from mcq's free-text options editor). `correct_answer` stays the 0/1 index string, `back_text` derived, `points_to_remember` from the same "Why" textarea — identical convention to mcq, all reusing `src/lib/mcq.js`'s existing `deriveMcqBackText`/`toPointsToRemember` helpers unchanged.
- **`StudyMode.jsx` — a 2-line generalization, not a new branch:** the mcq-only render condition became `GRADED_QUESTION_TYPES.includes(question_type)` (new export, `['mcq','true_false','correct_incorrect']`). The existing `AnswerOption`-list render and hybrid-grading state machine (wrong → auto-hard + single Continue; correct → reveal + unchanged `GradeButtonRow`) needed zero new code — a 2-option array maps through the identical JSX as a 4-option one. **This answers the sprint's own report-back question: true_false and correct_incorrect turned out 100% identical in rendering**, confirming similar types can keep collapsing into one build pass going forward.
- **`FlashcardCreate.jsx`:** question-type selector is now shown to ALL users (previously hidden entirely behind `canAuthorGradedTypes`) — `theory`/`test_your_understanding` are ungated per D-10, only the 3 graded options (Multiple Choice/True-False/Correct-Incorrect) within the same dropdown stay role-conditional. True/False and Correct/Incorrect get a 2-way toggle-button pair instead of mcq's options editor. Theory/test_your_understanding fall through to the exact same front/back textarea pair `flashcard` already uses — confirmed by reading `StudyMode.jsx`'s render chain, no new branch needed there either.
- **`BulkUploadFlashcards.jsx`:** CSV template gains one example row per new type + rewritten column docs (`correct_option` means 1-4 for mcq but 1-2 for true_false/correct_incorrect); the parser's recognized-`question_type` list extended from mcq-only to all 4 new types; the `42501` RLS-rejection error message generalized to name all 3 gated types instead of just mcq.
- **`npm run build` clean; `npx eslint` clean** on every changed file — the 2 pre-existing `FlashcardCreate.jsx` unused-var errors + 1 unused-eslint-disable warning confirmed via `git stash` to predate this sprint (same 2 errors Sprint 7.5 already confirmed pre-existing).
- **✅ Live frontend verification DONE (13/09/2026, dev server → live Supabase, real professor + student accounts, account owner typing credentials directly — Claude never saw or entered a password):**
  - **Authoring (professor):** created one true_false, one correct_incorrect, one theory, one test_your_understanding card manually — question-type selector showed all 6 types with the graded 3 only visible to professor; card header + front label updated per type (`True / False 1`, `Correct / Incorrect 1`, `Theory 1`, `Test your understanding 2`); true_false/correct_incorrect's 2-way toggle rendered and saved correctly.
  - **Bulk upload:** uploaded a 6-row CSV mixing all 6 types (flashcard/mcq/true_false/correct_incorrect/theory/test_your_understanding) as professor — "Successfully uploaded 6 study items", every row's `back_text` derived correctly (MCQ → "Section 80C", correct_incorrect → "Correct", true_false → "True"), confirmed live in My Study Sets.
  - **StudyMode grading, both hybrid-grading paths:** true_false wrong answer → immediate auto-hard + single Continue (ledger incremented before tapping Continue, proving `apply_review` fires on reveal); correct_incorrect correct answer → reveal with no auto-submit, full `GradeButtonRow`, no pre-selected default. theory/test_your_understanding graded exactly like plain flashcard (full self-grade, zero verdict logic) — confirmed by reading `StudyMode.jsx`'s render chain AND by live grading both. Pre-existing MCQ path unaffected (graded live, both correct/wrong verified in earlier sprints, re-confirmed unaffected here).
  - **Student side:** question-type selector shows only Flashcard/Theory/Test your understanding — no client-side leak of the 3 gated types. D-10 server-side rejection already regression-proven via `02_TEST` SQL (not re-tested live, per this sprint's own "one quick confirmation test, not a full re-audit" instruction).
  - **Browse Study Sets filter:** dropdown shows all 6 types via `formatQuestionType`; filtering by True/False and by Correct/Incorrect each narrowed to exactly the one deck containing that type, at the correct per-viewer card count, matching 7.6's documented semantics (narrows decks, not `card_count`).
  - **Payoff, confirmed live:** professor Dashboard's "Accuracy by question type" widget now shows real data for all 6 types — MCQ/Correct-Incorrect/True-False show both Recall success AND Answer accuracy (verdict-bearing); Theory/Test your understanding correctly show "No graded answers yet" for Answer accuracy (free-recall) — first real proof the two-measure contract (Sprint 7.4) generalizes past mcq.
  - **Console clean** across the whole flow, both accounts — no errors traceable to this sprint (one non-critical, pre-existing `admin_audit_log` 403 on bulk upload, already wrapped in a `// non-critical` try/catch predating this sprint, unrelated to question types — not chased).
- **Naming sweep (same session, requested mid-verification):** "Create Flashcards" → **"Create Study Item"** page title + subtitle, matching the "Study Sets" terminology Sprint 7.6 already established at the deck level (flashcard is one item TYPE among several, not the umbrella term). Full copy-only sweep, same pattern as 7.6's rename: page title/subtitle, all "Create Flashcard" nav-trigger labels (`NavDesktop.jsx`, `NavMenuSheet.jsx`, `NavBottomTabs.jsx` incl. its doc-comment, `DesignShowcase.jsx`'s /__design mockup), every empty-state/quick-link button pointing at `/dashboard/flashcards/new` (`MyFlashcards.jsx`, `MyContributions.jsx` ×2, `ReviewFlashcards.jsx`, `NoteDetail.jsx`), `BulkUploadFlashcards.jsx`'s page title/subtitle/toasts/button labels, `guideContent.js`/`helpContent.js`'s nav-instruction lines, `Home.jsx`'s public "3. Create Flashcards" landing-page step, and `PageContainer.jsx`'s doc-comment. Toast/validation copy in `FlashcardCreate.jsx` also updated ("N study item(s) created successfully", "Item N: Front side cannot be empty", "Add Another Item", "Create N Items"). **Deliberately left alone:** `helpContent.js`'s "Creating Flashcards" section title + prose (a real content rewrite describing all 6 types, not a label swap — flagged, not fixed) and `ProfessorTools.jsx` (confirmed dead/unrouted per blueprint backlog, not worth touching). Routes, file names, DB table/column names, and internal variable names (`flashcards` state, `.from('flashcards')`, `flashcard_decks`, etc.) all left untouched — copy-only, same scope discipline as 7.6. `npm run build`/`npx eslint` clean (2 pre-existing `FlashcardCreate.jsx` + 1 pre-existing `Home.jsx` + 1 pre-existing `ReviewFlashcards.jsx` issue, all confirmed via `git stash` to predate this session). Live-verified: desktop nav dropdown, mobile ＋ action-sheet, and mobile Menu drawer all show "Create Study Item"; toast confirmed reading "1 study item(s) created successfully".
- **Files:** *new* — `docs/database/sprint7.7/{00_DIAGNOSTIC,01_SCHEMA,02_TEST}*.sql` (all 3 ✅ run against production). *changed* — `src/lib/questionTypes.js`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `src/components/layout/{NavDesktop,NavMenuSheet,NavBottomTabs,PageContainer}.jsx`, `src/pages/dashboard/Content/{MyFlashcards,MyContributions,NoteDetail}.jsx`, `src/pages/dashboard/Study/ReviewFlashcards.jsx`, `src/data/{guideContent,helpContent}.js`, `src/pages/Home.jsx`, `src/pages/dev/DesignShowcase.jsx`, `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`.
- **✅ Committed and pushed** (`1693e1a`, on `main`) — SQL deployed + verified (6/6 PASS), frontend live-verified end to end on both accounts, naming sweep done and live-verified.

---

## Just Completed ✅ — Sprint 7.6: Browse/My Study Sets rename + question-type filter — Phase 7 sprint 7 (13/09/2026)

**Small, focused sprint** fixing the naming confusion Anand raised at the end of 7.5 — `ReviewFlashcards.jsx` now browses ALL question types (flashcard + mcq, more coming), so "Review Flashcards" no longer matched what the page does. Copy + one filter, not a redesign.

- **Copy-only rename (routes/files/components untouched):** `ReviewFlashcards.jsx`'s `<h1>` "Review Flashcards" → **"Browse Study Sets"**, subtitle now "Choose a subject or topic to start studying" (dropped "flashcards" wording). `MyFlashcards.jsx`'s `<h1>` "My Flashcards" → **"My Study Sets"**. Both `NavDesktop.jsx`'s Study dropdown item and the mobile Menu drawer's (`NavMenuSheet.jsx`) matching item relabeled the same way; `Today's Reviews`/`Browse Notes` labels untouched.
- **Swept the same label everywhere else it pointed at these two pages**, so the rename doesn't just move the inconsistency rather than fix it: `Dashboard.jsx`'s student "My Flashcards" quick-link card, `BulkUploadFlashcards.jsx`'s post-upload "View My Flashcards" button, `helpContent.js`'s "Review by Subject" step (quotes the exact nav path), and all 4 `linkLabel: 'Review Flashcards'` occurrences in `guideContent.js`. Their `linkTo` values were left as-is — while auditing, found that 3 of those `guideContent.js` steps' prose actually describes the due-today review queue ("Open your Review queue", "reviewing just 20 cards", "During a review session...") but link to `/dashboard/review-flashcards` (the browse page) instead of `/dashboard/review-session` (Today's Reviews) — a pre-existing mismatch unrelated to this rename. Flagged as a separate background task rather than fixed here, since correcting `linkTo` is a functional change, not copy.
- **Question Type filter added to `ReviewFlashcards.jsx`'s existing filter row** (Course/Subject/Topic/Role/Author + new 6th "Question Type", same single-select `filterX`/`setFilterX` + `applyFilters()`/`clearAllFilters()` idiom as the other five). Options: "All Types" + `flashcard`/`mcq` (the only two types with a real authoring path this sprint) — built from a `BROWSABLE_QUESTION_TYPES` constant array in the new **`src/lib/questionTypes.js`**, not hardcoded JSX, so adding a type in 7.7+ is a one-line change. Labels via `formatQuestionType()`, **moved from a `Dashboard.jsx`-local const into the same shared file** so both pages use identical display strings (Dashboard.jsx now imports it).
- **Semantics:** filtering narrows which DECKS show (a deck qualifies if it has ≥1 card of that type visible to the viewer, same "at least one" narrowing the existing Subject/Topic filters already do) — does **not** change the returned `card_count` (stays whole-deck) and does **not** change what a study session serves once a student clicks into a deck (still the whole deck, mixed types). Verified: unlike the other five filters (pure client-side array filtering over one fetched dataset), this one round-trips to `get_browsable_decks` with `p_question_type` on every change, since deck-level type membership isn't otherwise in the payload — the existing dependent-filter effects (Subject/Topic re-validating against the narrower `allDecksFlat`) needed zero new code, since they already react to `allDecksFlat` changing.
- **SQL — `get_browsable_decks` v5** (`docs/database/sprint7.6/01_FUNCTIONS_get_browsable_decks_v5_question_type_filter.sql`): adds `p_question_type text DEFAULT NULL`, additive, `NULL` reproduces v4 exactly. **Real deployment gotcha, caught live:** the first attempt used a plain `CREATE OR REPLACE FUNCTION get_browsable_decks(p_question_type TEXT DEFAULT NULL)`, which did **not** replace the old zero-arg function — Postgres treats a changed parameter list (even one added with a `DEFAULT`) as a *distinct overload*, not a replacement, leaving both versions live and every unparameterized call ambiguous (`ERROR 42725: function get_browsable_decks() is not unique`). Fixed with an explicit `DROP FUNCTION IF EXISTS get_browsable_decks();` before the `CREATE OR REPLACE` — worth remembering for any future RPC that *adds* (not just changes the body of) a parameter.
- **SQL test (`02_TEST`) also needed a fix mid-sprint:** the first version ran as the SQL Editor's default `postgres` role, which has no `auth.uid()` — hit `RAISE EXCEPTION 'Not authenticated'` (expected, not a bug in the function). Rewritten to impersonate a real profile via `SET LOCAL ROLE authenticated` + `request.jwt.claims` (same technique as `docs/database/sprint7.5/02_TEST_verify_d10_role_gate.sql`). Re-run result: 35 unfiltered decks, 35 with a flashcard, 1 with an mcq (`Income Tax → Deductions from Gross Total Income`) — that one deck appeared under all three filters with an unchanged `card_count=4`, and an unused type (`theory`) correctly returned 0 decks without erroring.
- **Live-verified (13/09/2026, dev server → `revisop.com`'s live Supabase, real student account, TestOutlook):** Study nav dropdown shows "Browse Study Sets"; page heading/subtitle correct; Question Type dropdown shows exactly "All Types"/"Flashcard"/"MCQ"; selecting MCQ narrowed 464→3 cards down to the one deck known (from SQL testing) to mix both types, at this student's own per-viewer visible card_count (3, vs. 4 for the more-privileged SQL-test profile — confirms per-viewer visibility is respected); clicking that filtered deck's "Study All" produced the plain `/dashboard/study?deck=...` URL with no question-type param (filter did not leak into the study session); `clearAllFilters()` correctly reset Question Type to "All" alongside the other five; console clean except one pre-existing, self-recovering `refresh_token_not_found` warning on hard reload (reproduces identically without this sprint's changes, unrelated). **Noted, not chased (out of scope, StudyMode.jsx explicitly excluded from this sprint):** one specific deck ("Deductions from Gross Total Income") returned "No flashcards to study" for this account both with and without the filter — reproduced identically before any of this sprint's changes, most likely this account's SRS "due" state for those 3 cards, not a regression. A different deck (GST) opened and graded normally, confirming the study-session path itself is unaffected.
- **`npm run build` clean; `npx eslint` clean** on every changed file (the one pre-existing `ReviewFlashcards.jsx` `useCallback` exhaustive-deps warning confirmed via `git stash` to predate this sprint).
- **Files:** *new* — `src/lib/questionTypes.js`, `docs/database/sprint7.6/{01_FUNCTIONS,02_TEST}*.sql`. *changed* — `src/pages/dashboard/Study/ReviewFlashcards.jsx`, `src/pages/dashboard/Content/MyFlashcards.jsx`, `src/components/layout/{NavDesktop,NavMenuSheet}.jsx`, `src/pages/Dashboard.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `src/data/{guideContent,helpContent}.js`, `docs/active/blueprint.md`, `docs/reference/{DATABASE_SCHEMA,FILE_STRUCTURE}.md`.
- **✅ Committed** (`c629ed3`, on `main`) — confirmed via this session's git log while working Sprint 7.7; this note had gone stale (the commit landed after it was written, same pattern as the 7.5 correction).

---

## Just Completed ✅ — Sprint 7.5: MCQ-single vertical slice — Phase 7 sprint 6 (13/09/2026)

**The architectural proof for the whole question-type epic — representation → authoring → bulk import → rendering → hybrid grading, end to end, for one type (mcq).** The engine underneath (apply_review, review_events, both analytics RPCs) needed zero changes — confirmed `get_study_queue` only excludes `concept_card`, so mcq rows flow through the due queue with no special-casing.

- **Pre-flight: `00_DIAGNOSTIC_preflight.sql` run and reviewed (13/09/2026).** 7.4's two analytics RPC grants confirmed intact (both IDOR guards + `authenticated`-only EXECUTE present). Every existing `flashcards` RLS policy is PERMISSIVE (no name collision with the two new RESTRICTIVE ones below). Zero pre-existing `question_type='mcq'` rows (2155 rows, all `'flashcard'`). `is_admin()`'s live body confirmed — `is_professor_or_admin()` mirrors it exactly.
- **Diagnostic caught a real drift between docs and live schema, before deploy:** the live `chk_flashcards_question_type` CHECK constraint (`pg_get_constraintdef`) allows `'fitb'`, NOT `'fill_in_the_blanks'` as both the sprint kickoff and `blueprint.md` had assumed — and doesn't allow `'test_your_understanding'` at all. The D-10 policy's IN-list had been drafted with `'fill_in_the_blanks'`; since a `question_type NOT IN (...)` row passes the RESTRICTIVE check unconditionally, shipping that draft would have left `fitb` rows **completely ungated for any authenticated user**, D-10 notwithstanding. Caught and fixed before deployment — exactly the introspect-before-DDL case this project's standing practice exists for. `01_SCHEMA` corrected to `'fitb'`; `02_TEST` extended with explicit `fitb`/`true_false` rejection assertions (not just mcq) so this exact bug class is covered going forward. `blueprint.md` §1.1/§3.1 and `DATABASE_SCHEMA.md`'s flashcards row corrected to the real 10-value enum.
- **D-10 ✅ SQL DEPLOYED & VERIFIED LIVE (13/09/2026).** `01_SCHEMA_d10_role_gate.sql` adds `is_professor_or_admin()` (mirrors the existing `is_admin()` pattern) + two RESTRICTIVE RLS policies on `flashcards` (INSERT + UPDATE) blocking any non-professor/admin/super_admin row whose `question_type` is one of the 7 verdict-bearing types — deployed to production. `02_TEST_verify_d10_role_gate.sql` run against real profiles: **all 5 rows PASS** — student `mcq`/`fitb`/`true_false` inserts rejected (RLS policy violation), student plain `flashcard` insert unaffected, professor `mcq` insert succeeds. Both `FlashcardCreate.jsx` and `BulkUploadFlashcards.jsx` write via a direct `.from('flashcards').insert()` (confirmed by reading both files, neither goes through an RPC), so RLS alone was the correct enforcement point — no SECURITY DEFINER bypass to worry about. **Next: live frontend testing (authoring → bulk-upload → StudyMode) with real professor + student accounts, then git commit.**
- **7.5-B representation (locked, no schema change):** `question_type='mcq'` uses only existing columns — `options` (jsonb array of option strings, 2-6), `correct_answer` (text, the 0-based option INDEX as a string — not the option text, since two options can be identical), `back_text` auto-derived as `options[correct_answer]` at save time (never typed directly), `points_to_remember` (jsonb array, one entry per non-blank line of the "Why" textarea).
- **New shared helper `src/lib/mcq.js`** — used by both entry paths so back_text derivation can't drift: `compactMcqOptions` (drops blank option rows and remaps the correct index by ORIGINAL position, not by text), `deriveMcqBackText`, `toPointsToRemember`, `validateMcqOptions`.
- **7.5-C `FlashcardCreate.jsx`:** question-type selector (Flashcard / Multiple Choice), rendered only for `isProfessor || isAdmin || isSuperAdmin` (via `@/contexts/NavDataContext`'s `useRole`, matching the pattern `BulkUploadFlashcards.jsx` already used — students get zero selector, cards stay implicit `'flashcard'`). Per-card options editor (add/remove, 2-6 rows, radio to mark correct), optional "Why" textarea. Draft autosave/recovery extended to carry the new fields so a browser crash mid-authoring an MCQ doesn't silently drop the options.
- **7.5-D `BulkUploadFlashcards.jsx`:** CSV template gains `question_type`, `option_1..option_4`, `correct_option` (1-based), `explanation`; existing front/back-only rows keep working unchanged. A student-authored `mcq` row is rejected server-side by the D-10 policy regardless of client checks — added a friendlier error message for the Postgres `42501` (RLS violation) case specifically, since the whole CSV is one batch insert (all-or-nothing).
- **7.5-E `StudyMode.jsx` hybrid grading:** third rendering branch alongside the untouched `!showAnswer`/`showAnswer` flashcard JSX, gated on `question_type === 'mcq'`, using the already-built `AnswerOption` primitive (idle/correct/missed/dim states, navy correct per 6.4's de-greening). `handleRating` split into `submitReview` (the `apply_review` RPC call) + `advanceCard` (the forward-animation timer) because the two grading paths need them decoupled: a **wrong** answer calls `submitReview('hard', false)` immediately on reveal, then shows a single Continue button (no GradeButtonRow) that only calls `advanceCard` — the rating is already decided, no second RPC call. A **correct** answer reveals then shows the existing unchanged `GradeButtonRow` (Hard/Medium/Easy, no pre-selected default); the tap is both `submitReview(rating, true)` and the advance, via the same `handleRating(quality, isCorrect)` the flashcard path already uses (now accepting a second arg instead of always passing `p_is_correct: null`).
- **`npm run build` clean; `npx eslint` clean** on every changed file (verified the 2 pre-existing `FlashcardCreate.jsx` unused-var errors predate this sprint — confirmed via `git stash` diff, not introduced here).
- **Bonus fix (same session):** `Dashboard.jsx`'s question-type label map had the same `fill_in_the_blanks` vs. live `fitb` mismatch the D-10 diagnostic caught — `formatQuestionType`'s map keyed on `fill_in_the_blanks` (line ~99), which would have fallen back to a generic title-cased "Fitb" label once a real fitb row exists. Corrected the key to `fitb`; confirmed via repo-wide grep it was the only occurrence (no analogous map in `Progress.jsx`). `test_your_understanding` left in the map as a harmless dead entry (not a live `question_type` value either, but out of scope to remove here).
- **DONE — SQL:** (1) `00_DIAGNOSTIC_preflight.sql` run — 7.4 RPC grants confirmed intact, live RLS policy list captured, zero pre-existing mcq rows confirmed, `is_admin()` pattern confirmed. (2) `01_SCHEMA_d10_role_gate.sql` deployed to production. (3) `02_TEST_verify_d10_role_gate.sql` run against real profiles — **all 5 rows PASS**.
- **DONE — live frontend verification (13/09/2026, dev server → `revisop.com`'s live Supabase, real professor + student accounts via browser automation with the account owner typing credentials directly — Claude never saw or entered a password):**
  - **Pre-flight closed:** the professor `Dashboard.jsx` "Accuracy by question type" widget (carried over un-click-tested from 7.4) confirmed rendering correctly on first login.
  - **7.5-C authoring:** created one MCQ card manually as professor (Income Tax / Deductions from Gross Total Income, 4 options, one marked correct, "Why" explanation) — saved correctly, `back_text` auto-derived to the correct option's text, visible in My Flashcards. Confirmed the question-type selector is completely absent for the student account (no client-side leak).
  - **7.5-D bulk upload:** uploaded a 2-row CSV (1 plain + 1 mcq, `correct_option=2`) as professor — both created correctly, mcq row's `back_text` derived to "Section 80D" (the option at the 0-based index matching `correct_option`). As the student, uploaded a 1-row mcq CSV — **rejected server-side** with the friendly `42501` message ("requires a professor/admin account... No rows were created"), confirmed via console the actual Postgres RLS error fired (`flashcards_gate_verdict_types_insert`).
  - **7.5-E StudyMode hybrid grading, both paths, real student account:**
    - **Wrong path:** tapped an incorrect option → immediate reveal (correct=navy, wrong=slate "You chose this", others dimmed), "Why" block shown, session ledger incremented Hard **before** tapping Continue (proving `apply_review` fires on reveal, not on Continue) — confirmed via a live network signal (student's `reviews` count on Dashboard's leaderboard incremented). Continue button correctly triggered the fade transition (`advanceCard`).
    - **Correct path:** tapped the correct option (a second MCQ card, different correct index to rule out an off-by-one) → revealed with no auto-submit (ledger stayed 0/0/0), the unchanged `GradeButtonRow` appeared with real interval previews (1d/3d/7d) and no pre-selected default; tapping Easy submitted and advanced together — confirmed via the student's review count incrementing (11→12).
    - **Regression:** the plain flashcard graded in the same session rendered and graded exactly as before — zero visible change to the untouched path.
  - **✅ The payoff, confirmed on both sides:** `Progress.jsx` (student) — "MCQ: 2/2 reviewed, Recall success 50%, **Answer accuracy 50%**" (real progress bar, not placeholder text). `Dashboard.jsx` (professor, cohort-wide) — "MCQ: 2 self-graded, Recall success 50%, **Answer accuracy 50%**". Both `get_question_type_performance` and `get_educator_accuracy_by_qtype` showing real graded data for the first time since Sprint 7.4 built the two-measure contract.
  - **get_study_queue confirmed live** (not just by reading the SQL) — both new mcq cards flowed into the student's StudyMode session as new-card candidates with no special-casing, matching the code-read prediction that only `concept_card` is excluded.
  - **Console clean** across the whole flow on both accounts — no new errors beyond expected/intentional ones (the deliberate RLS-rejection test) and pre-existing dev-environment ServiceWorker noise (unrelated, present before this sprint too).
- **Operator-caught bug #1, fixed same session (13/09/2026, unrelated to MCQ):** live testing surfaced a real platform-wide bug — `flashcard_decks.visibility` was frozen at whatever the deck's first-ever card had, so a deck created `private` that later gained `public` cards stayed invisible in Browse (`get_browsable_decks`) and Recent Activity (`get_recent_activity_feed`) forever, even though the member cards' own visibility was correct. `update_deck_card_count()`'s INSERT branch now widens deck visibility (never narrows) when a new card is more permissive; one-time backfill fixed the 4 live decks already desynced (3 belonging to other users — predates this session's testing). SQL: `docs/database/sprint7.5/{03_DIAGNOSTIC,04_FUNCTIONS,05_FIX}*.sql`, deployed and verified (0 desynced decks remain).
- **Operator-caught bug #2, same session:** the sibling issue flagged during bug #1's investigation turned out real — the same trigger's auto-create-deck branch never set `target_course` on the new row (a `bugs.md` entry had incorrectly implied it did, since Mar 2026). Live impact measured: **2 decks, 34 public cards total, one professor, dating to 07/04/2026** — invisible to every student for 5 months, since `get_browsable_decks`'/`get_recent_activity_feed`'s course-match filters silently exclude a NULL `target_course`, and professors/admins bypass that gate (which is why nobody noticed). Fixed (`07_FUNCTIONS_fix_deck_target_course.sql`) and backfilled (`08_FIX_backfill_deck_target_course.sql`) — verified 0 rows remain NULL. Both bugs fully written up in `bugs.md`.
- **Also fixed:** the per-card form heading in `FlashcardCreate.jsx` still said "Flashcard N" even when Question Type was set to Multiple Choice — now shows "Multiple Choice N".
- **Files:** *new* — `src/lib/mcq.js`, `docs/database/sprint7.5/{00_DIAGNOSTIC,01_SCHEMA,02_TEST,03_DIAGNOSTIC,04_FUNCTIONS,05_FIX,06_DIAGNOSTIC,07_FUNCTIONS,08_FIX}*.sql`. *changed* — `src/pages/dashboard/Content/FlashcardCreate.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/Dashboard.jsx` (label-map key fix), `update_deck_card_count()` DB trigger function, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/tracking/bugs.md`, `docs/active/now.md`, `docs/tracking/changelog.md`.
- **✅ Committed** (`797629d`, on `main`) — confirmed via this session's git log while working Sprint 7.6; this file's earlier "Not yet committed" note for 7.5 had gone stale (the commit landed after that note was written).

---

## Just Completed ✅ — Sprint 7.4: review_events, apply_review, two-measure analytics semantics — Phase 7 sprint 5 (13/09/2026)

**SQL: 5 files, all deployed + confirmed live** (`docs/database/sprint7.4/`) — `00_DIAGNOSTIC` (Phase 0 ground-truth, run first per this project's introspect-before-DDL standing practice), `01_SCHEMA` (new `review_events` table), `02_FUNCTIONS` (`apply_review` + `submit_review` compat shim), `03_FUNCTIONS` (two-measure analytics semantics on both question-type RPCs), `04_TEST` (BEGIN/ROLLBACK, 29/29 real PASS), `05_DIAGNOSTIC` (post-deploy storage measurement). `npm run build` clean; `npx eslint` clean on every changed file.

- **Phase 0 introspection changed the delivered SQL from the sprint's own draft, twice.** (1) The draft assumed `review_events.user_id REFERENCES profiles(id)`; live introspection showed `reviews.user_id` actually references `auth.users(id)` (not `profiles`) — matched that instead, and corrected the same stale claim in `DATABASE_SCHEMA.md`'s `reviews` row while at it. (2) Renaming `accuracy_pct` + adding 2 output columns to both analytics RPCs needed DROP+CREATE, not `CREATE OR REPLACE` (same "cannot change `RETURNS TABLE` shape" constraint this project already hit with `get_study_queue`) — grants re-applied explicitly.
- **`review_events`** — append-only, one row per grade ever, RLS on with zero policies/grants (SECURITY DEFINER RPC access only). `reviews` stays the current-state SSOT, unchanged.
- **`apply_review`** — new write SSOT. Absorbs `submit_review`'s live body verbatim (confirmed byte-identical before extending), adds a `review_events` INSERT in the same transaction as the `reviews` write. Atomicity wasn't just asserted — `04_TEST` forces a real failure via a `pg_temp` trigger and confirms the `reviews` write rolled back with it. `submit_review` is now a thin compat wrapper (old cached clients keep working, log `is_correct=NULL`).
- **Two-measure analytics** — `get_question_type_performance` (student) and `get_educator_accuracy_by_qtype` (professor) both rename `accuracy_pct`→`recall_success_pct` (unchanged computation) and add `graded_count`/`answer_accuracy_pct` (NULL until graded question types exist, post-Sprint-7.5). Both queries pre-aggregate `review_events` into a one-row-per-key CTE before joining, specifically to avoid fanning out the pre-existing accuracy arithmetic (review_events is append-only; a naive join would have inflated the denominators).
- **Frontend:** `StudyMode.jsx handleRating` now calls `apply_review` (not `submit_review`) with `p_is_correct: null` and `p_source` derived from the existing `rung === undefined` new-card convention. No question-type rendering changed — StudyMode stays pure front/back. `Progress.jsx`'s `QuestionTypeRow` and `Dashboard.jsx`'s professor widget both show two labeled rows per type with an updated footnote.
- **Live-verified (13/09/2026, dev server → live Supabase, real student account):** graded a never-reviewed card (Medium) and an already-reviewed card (Easy, then a review-session Hard) — captured the actual `fetch` calls via a page-level patch (the Browser pane's network-request tool doesn't surface Supabase's `fetch` calls) and confirmed 200 OK with correct `new_rung`/`interval_days` for both the `new_card` and `review_session` source paths. Progress.jsx's "Performance by Question Type" widget rendered "Recall success 71.9%" and "Answer accuracy: No graded answers yet" exactly as designed, footnote correct. Two pre-existing 400-status console errors were investigated and ruled unrelated to this sprint — they occur on every page load regardless of grading and aren't from any `fetch`/`XHR` call (broad page-level instrumentation captured everything and found nothing ≥400); most likely push-notification registration noise in the dev environment.
- **Not live-verified:** `Dashboard.jsx`'s professor "Accuracy by question type" widget (no professor test account available this session) — code-reviewed against the same live-confirmed RPC shape as the student widget; same render pattern, low risk.
- **7.4-E storage projection (measured, not guessed):** ~142 bytes/row (`pg_column_size` on real inserted rows, avg of 4). Current DB 53MB vs. the Supabase Free-plan 500MB limit; even a generous 10×-of-observed daily rate projects to low-single-digit MB/year. **No archival/rollup/partitioning built** — the projection shows no near-term problem, per the sprint's own "measure, don't build machinery" instruction. Worth a real re-check after organic (non-test) usage accumulates.
- **Files:** *new* — `docs/database/sprint7.4/{00_DIAGNOSTIC,01_SCHEMA,02_FUNCTIONS,03_FUNCTIONS,04_TEST,05_DIAGNOSTIC}*.sql`. *changed* — `src/pages/dashboard/Study/StudyMode.jsx` (handleRating), `src/pages/dashboard/Study/Progress.jsx` (QuestionTypeRow + footnote), `src/pages/Dashboard.jsx` (educator accuracy widget + footnote), `docs/active/blueprint.md` (D-11, §1.1 `review_events`, §1.4 RPC inventory), `docs/reference/DATABASE_SCHEMA.md` (`review_events` table + Sprint 7.4 RPC section + `reviews.user_id` FK correction).
- **✅ Committed** (`48d6e66`, on `main`).

---

## Just Completed ✅ — Sprint 7.3: Dashboard Reporting Surface & Study-Time Split — Phase 7 sprint 4 (12/09/2026)

**SQL: 2 migrations, both deployed + confirmed live** (`docs/database/sprint7.3/`) — `profiles.has_dismissed_goal_prompt` (additive column) and `get_study_time_stats` DROP+CREATE (4 additive in-app/offline split columns; existing 4 columns/IDOR guard/search_path unchanged). `npm run build` clean; `npx eslint` clean on every changed file (3 pre-existing unused-var errors in `ProfileSettings.jsx`, unrelated to this sprint's own additions, cleaned up as a ride-along since the file was already open).

- **7.3-A — student dashboard is now a pure reporting surface.** Removed the Streak/Accuracy/Mastered/Reviews "Your Week" stat-tile grid (duplicated the Progress tab — `Progress.jsx` already computes these independently via `get_mastered_cards`). `fetchPersonalStats` trimmed to just `reviewsDue` (`get_study_queue`) + today's review count (still needed by `GoalProgressWidget`); the now-fully-unused `calculateStreak` helper and its RPC/query calls deleted. Final order: Goal Progress → Leaderboard → Forward Load → Study Time (report only) → Recent Activity. Live-verified: grid gone, order correct, no console errors.
- **7.3-B — Daily Goal split into action + report.** `OnboardingModal.jsx` 4th step ("Set your daily goal" → `/dashboard/settings`) — literally the same STEPS-array shape as the other 3, so it's skippable for free (Done/X always work) with zero render-logic changes. `ProfileSettings.jsx` gains a "Daily Goal" Card (students only, `!isContentCreator`) — type toggle + Save/Clear, reusing `update_daily_goal` (no new RPC). `GoalProgressWidget.jsx` → "Goal Progress": goal set → progress bar + de-emphasized "Revise target" link (was a prominent top-right "Edit"); no goal + not dismissed → single dismissible line ("Set a daily goal →" / "Not now", no card/header); no goal + dismissed → nothing. **Live-verified end-to-end:** set a 10-review goal in Settings → toast → Dashboard showed "Goal Progress / 0 / 10" + "Revise target"; cleared it → dismiss line reappeared logic confirmed separately (dismiss persists across reload, one-time toast fires once).
- **7.3-C — Study Time split + localStorage collision fix + app-wide timer context.** New **`src/contexts/StudyTimerContext.jsx`** (mounted in `App.jsx` beside `StudySessionProvider`) now owns the manual timer's state + 3-tier stale-session policy, classified via lazy `useState` initializers on provider mount — app-wide from the first authed page load, not gated behind visiting whichever page used to host `StudyTimerWidget`. **New localStorage key `revisop_manual_timer_started_at`** — no longer shares `revisop_session_started_at`/`revisop_session_source` with `StudyMode.jsx`/`ReviewSession.jsx` (untouched, kept exactly as-is), closing a real collision where a concurrent manual timer + in-app review session clobbered whichever wrote last. `window.addEventListener('storage', …)` for cross-tab sync (not live-tested — would need two tabs; code-reviewed only). `StudyTimerWidget.jsx` rewritten as a thin context consumer, moved off the Dashboard onto new route **`/dashboard/study-time`** (`StudyTimePage.jsx`), reachable from the desktop Create dropdown + mobile ＋ sheet. New **`StudyTimerChip.jsx`** nav pill (beside the bell, both `NavDesktop`/`NavMobile`) — hidden unless running, tap-to-stop. **Live-verified end-to-end:** started timer on `/dashboard/study-time` → chip appeared on both Dashboard and the study-time page (confirming app-wide context) → tapped the nav chip to stop a real ~19s session → toast "Session logged: 19s" → Dashboard's Study Time "This Week" in-app total increased. **Not live-tested this session:** the 4-16h recovery prompt and the 16h+ discard path (would require a faked stale localStorage timestamp), and cross-tab sync.
- **7.3-D — Bulk Upload removed from the mobile ＋ sheet, all roles** (`canBulkUpload` gate deleted, not narrowed); desktop Create dropdown keeps it unconditionally + gains the same "Log Study Time" entry with a divider. **Not click-tested this session** (a Browser-pane focus/click hiccup blocked mobile-sheet interaction late in the session, same category of transient issue noted in the 7.2 session log — screenshots/reads worked fine throughout); confirmed via code review that `NavBottomTabs.jsx`'s `CreateAction` items array has no Bulk Upload entry under any role and `NavDesktop.jsx`'s dropdown is unconditional.
- **Follow-up (same session) — dashboard trailing cards.** Confirmed with Anand: Quick Actions and My Contributions come off the student dashboard too, so Recent Activity is always the last card (Quick Actions was pure redundant navigation — same destinations as the Create dropdown/sheet; My Contributions duplicated the dedicated `/dashboard/my-contributions` page already linked from `ProfileDropdown`/`NavMenuSheet`). The conditional "My Reports" card — a moderation-flag status tracker for content *the student themselves* flagged (`content_flags`, `flagged_by = you`), unrelated to My Progress/study stats — moved off the dashboard onto a new page, **`/dashboard/my-reports`**, labeled **"Report History"** in the nav (chosen over "My Reports" to read unambiguously as a status list, not an academic report). Linked from `ProfileDropdown.jsx` + `NavMenuSheet.jsx`. Live-verified: Recent Activity renders last; the link opens the new page with the correct empty state.
- **Files:** *new* — `src/contexts/StudyTimerContext.jsx`, `src/pages/dashboard/Study/StudyTimePage.jsx`, `src/components/layout/StudyTimerChip.jsx`, `src/pages/dashboard/Profile/MyReports.jsx`, `docs/database/sprint7.3/{01_SCHEMA,02_FUNCTIONS,03_TEST}*.sql`. *changed* — `src/pages/Dashboard.jsx`, `src/components/dashboard/{GoalProgressWidget,StudyTimerWidget,OnboardingModal}.jsx`, `src/pages/dashboard/Profile/ProfileSettings.jsx`, `src/components/layout/{NavDesktop,NavMobile,NavBottomTabs,ProfileDropdown,NavMenuSheet}.jsx`, `src/App.jsx`.
- **✅ Committed + pushed** (`77cfe0c`, on top of `c51a145`) — both landed on `origin/main` 12/09/2026.
- **✅ Manual click-through done (12/09/2026, dev server → live Supabase, TestOutlook).** Mobile ＋ sheet: Upload Note / Create Flashcard / Create Group / divider / Log Study Time — no Bulk Upload, confirmed via `get_page_text` + screenshot (the click-tool hiccup from the original session recurred on the Sheet trigger specifically — worked around by dispatching a real pointerdown/mousedown/pointerup/mouseup/click sequence via `javascript_tool` on the actual DOM button, which is a legitimate proxy for a user tap, not a code change). Desktop Create dropdown: Upload Note / Create Flashcard / Bulk Upload (present, unconditional — correct) / divider / Log Study Time. **Fixed a stale copy bug found along the way:** `NavBottomTabs.jsx`'s `SheetDescription` (sr-only) still said "…a note, a flashcard, or a bulk upload" after Bulk Upload was removed from that sheet in 7.3-D — updated to "…a note, a flashcard, a study group, or log study time."
- **✅ Stale-session test done (12/09/2026).** Faked `revisop_manual_timer_started_at` to 6h ago, reloaded `/dashboard` (not the study-time page) — `StudyTimerContext` classified it into `recoveryPrompt` at the app root (no chip shown, since `isRunning` stays false during recovery); navigated client-side (no further reload) to `/dashboard/study-time` — the "Your timer ran for 6h 0m…" prompt was already showing, proving the classification happened at Dashboard-mount, not re-derived per page. Faked it to 20h ago, reloaded `/dashboard` again — `localStorage.getItem` confirmed the key was silently removed on mount, no chip, and `/dashboard/study-time` showed the plain idle "Start" state (no leftover prompt).
- **⏳ Still open:** a two-tab cross-sync check (start the timer in one tab, confirm a second tab's nav chip picks it up via the `storage` event listener without a manual reload) — not tested this session.
- **✅ Header cleanup (Anand's ask, 12/09/2026).** Removed the subtitle line under "Welcome back" (`"You have N items ready for review"` / `"All caught up! 🎉"` for returning users, the onboarding blurb for new users) — it duplicated the Review-tab due badge (7.2-F) and took up space right below the greeting. Header is now just the `<h1>`. `reviewsDue` state kept (still gates the caught-up browse-links strip). Live-verified: line gone, no console errors, build/lint clean.

---

## Just Completed ✅ — Sprint 7.2: Pre-Onboarding Dashboard & Nav Polish — Phase 7 sprint 3 (11/09/2026)

**Frontend only. NO SQL — every RPC used already existed and was already user-scoped.** `npm run build` clean; `npx eslint` clean on every changed file. Inserted ahead of the review-engine sprint (now 7.3) because ~100+ students onboard this week and Anand wanted the dashboard + nav settled first. Both checkpoints (7.2-D, 7.2-C) were screenshotted live to Anand mid-sprint per the kickoff instructions, rather than guessed twice.

- **7.2-D — student dashboard restructure.** `Dashboard.jsx` student branch: the Streak/Accuracy/Mastered/Reviews "Your Week" stat tiles moved ABOVE the review CTA (was below). **Post-checkpoint follow-up (Anand caught this):** the shrunk CTA/"All caught up" card was still a duplicate of the header subtitle (which already says "You have N items ready for review" / "All caught up! 🎉") and the new 7.2-F nav badge — removed entirely rather than just shrunk. The two "Browse Study Sets"/"Browse Notes" links from the all-caught-up state were kept as a small link row so that path isn't lost. **Net effect:** starting a review from the dashboard body itself no longer has a dedicated button — reachable via the desktop "Study" dropdown or the mobile bottom-nav Review tab (now red-badged). Flagged to Anand as worth a second look if that reads as too indirect on desktop. Live-verified both states (TestOutlook 5-due, and the 0-due link row).
- **7.2-F — due-count badge, mobile + desktop.** New **`src/hooks/useDueForecast.js`** wraps the existing `get_due_forecast` RPC — the same lightweight 3-int call `Progress.jsx` already used (deliberately NOT `get_study_queue`, which returns full row data). Wired into **`NavDataContext.jsx`** as a fourth singleton (`dueToday`/`dueNext7`/`dueNext30`/`refetchDueForecast`) alongside role/notifications/friend-count — one call per session, no new per-component fetch. `NavBottomTabs.jsx`'s Review tab shows a small badge with the count, hidden at 0, capped `"9+"`. **Post-checkpoint fix (Anand caught this):** first cut used a pale `bg-rv-amber-50`/`text-rv-amber-ink` tint per the kickoff spec, which read as washed-out, not badge-like — changed to solid `bg-red-500 text-white` / `rounded-full`, matching the existing notification/friend-request badge convention exactly. **Extended to desktop (Anand's follow-up ask, for consistency):** `NavDesktop.jsx`'s "Study" dropdown trigger gets the same red badge (top-right corner of the button), and the "Today's Reviews" menu item repeats it inline — same `dueToday` value from the shared context, zero new fetch. Live-verified: TestOutlook (5 due, mobile bottom bar + desktop Study trigger + dropdown item) and CA Intermediate (7 due, mobile) all show the correct red badge.
- **7.2-A — Cohort Forward Load clarity.** Confirmed with Anand this was never a data bug — the cohort total (1476) and the professor's own due count (7) are both correct, they just measure different things. `Dashboard.jsx` professor branch: heading relabelled "Cohort forward load — all students in your course"; the professor's own due count now sits beside it for contrast, read from the same `NavDataContext` singleton 7.2-F wires up (no third `get_due_forecast` call). Live-verified: "Your own due today: 7" rendering correctly next to the 1476-total cohort chart.
- **7.2-E — Create Group.** `NavBottomTabs.jsx`'s ＋ action-sheet gains a "Create Group" entry (`Network` icon → `/dashboard/groups/new`), same pattern as the other three entries, no professor-gating (Groups is student-visible too).
- **7.2-B — unified notification center.** New **`src/components/layout/NotificationCenter.jsx`** merges the former `FriendsDropdown` + `ActivityDropdown` into ONE bell icon/dropdown, imported by both `NavDesktop.jsx` and `NavMobile.jsx` (single shared component). `FriendsDropdown.jsx` and `ActivityDropdown.jsx` deleted — no other importers existed. Pending friend requests (inline Accept/Decline preserved verbatim, same fetch-on-open behaviour) render at the top of the dropdown when present; content notifications below; Find People / My Friends / Following / View All Requests kept as a footer (dropping them would have orphaned `/dashboard/find-friends` and `/dashboard/my-friends`, which are reachable nowhere else in the nav). Single badge = `unreadCount + pendingCount` (summed, capped `"9+"`). Mobile top bar is now Wordmark (left) · Bell (right); desktop nav's right-side icon row lost one icon. `useFriendRequestCount` / `useNotifications` still flow through `NavDataContext` unchanged — this is a UI merge, zero new fetch. Live-verified on both student and professor sessions, desktop + mobile: single bell renders, dropdown opens, quick links present. **Not live-tested:** an actual pending-friend-request accept/decline through the merged UI — neither test account had a pending request at hand this session; the code path is unchanged from the original `FriendsDropdown` logic (same table, same columns, same optimistic local-state update).
- **7.2-C — professor dashboard restructure.** "Quick Actions" grid (Upload Note / Create Flashcard / Bulk Upload / Analytics link) removed entirely — redundant since Sprint 7.1 put Create actions in the desktop Create dropdown + mobile ＋ sheet. New "Analytics Snapshot" section embedded directly in `Dashboard.jsx`'s professor branch: the `overview` row (Cards Published / Students Reached / Total Reviews / Avg Quality) + "Challenging Cards" (top 5 from `get_professor_weak_cards`, min-3-reviews) — the two most actionable `ProfessorAnalytics.jsx` sections — plus a "View full analytics →" link for Subjects / Top Cards / Weekly Reach. Built as a light duplicate with its own `--rv-*` markup rather than importing `ProfessorAnalytics.jsx`'s components, since that page is still on pre-reskin gray tokens and reusing it would have forced an unplanned reskin under time pressure. `fetchEducatorWidgets()` now fires FOUR RPCs in one `Promise.all` (the existing 2 from Sprint 6.3 + the 2 new ones) — once per mount, once per course switch, same pattern `ProfessorAnalytics.jsx` already uses. "Needs Attention" moved to the top of the page — **still fully unconditional, no new render gate** (verified via diff review, per the settled-design memo). "Your Content" (My Notes / My Flashcards) pushed down to just above "Accuracy by question type" (was first on the page). **Anand's checkpoint:** approved on first pass (CA Intermediate professor session, desktop + mobile) — Analytics Snapshot showed real data (459 cards, 34 students reached, 2324 reviews, 3.9 avg quality, 5 challenging cards) with no visible over-fetch or flicker.
- **Verification note:** the in-session browser network-request tool did not capture the underlying Supabase REST/RPC calls this session (a tool-side limitation, not reproduced as an app error) — the "4 RPCs, once per mount" claim for 7.2-C rests on code review (single unchanged `Promise.all` pattern, extended from 2 calls to 4) plus the visual confirmation that the professor dashboard loaded once, correctly, with no duplicate-fetch flicker. A proper before/after network trace is still worth doing operator-side before/soon after the Tuesday onboarding push. Click-interaction testing of the ＋ action-sheet's new "Create Group" entry and the merged notification bell's accept/decline flow was cut short by a transient Browser-pane click-tool hiccup late in the session (screenshots/console/find all worked fine throughout — no app-side error) — worth a quick manual pass before pushing.
- **Files:** *new* — `src/components/layout/NotificationCenter.jsx`, `src/hooks/useDueForecast.js`. *deleted* — `src/components/layout/FriendsDropdown.jsx`, `src/components/layout/ActivityDropdown.jsx`. *changed* — `src/pages/Dashboard.jsx`, `src/contexts/NavDataContext.jsx`, `src/components/layout/{Navigation,NavDesktop,NavMobile,NavBottomTabs}.jsx`.
- **✅ Committed + pushed** (`c51a145`, confirmed on `origin/main` — this was done between sessions; the "operator to commit" note below is stale).
- **⏳ Follow-ups:** (1) a quick manual click-through of ＋ → Create Group and the merged bell's friend-request accept/decline on a real pending request, (2) optional: a proper network trace of the professor dashboard's 4 RPCs on `revisop.com` post-push.
...
## Just Completed ✅ — Sprint 7.1: Mobile Bottom Navigation — Phase 7 sprint 2 (08/09/2026)

**Frontend only. NO SQL. No new data fetching — `NavBottomTabs` consumes the `navProps` bundle only (no nav-hook re-mount → no 7.0 over-fetch regression). Desktop (`md:` and up) visually unchanged.** `npm run build` clean (7.2s); `npx eslint` clean on every file authored or structurally changed (`FlashcardCreate.jsx` / `NoteUpload.jsx` / `ReviewFlashcards.jsx` carry pre-existing baseline errors — this sprint only appended a className to their root div).

- **Layout — decided with Anand: LAYOUT A** ("reconcile the hamburger"). Bottom bar = **Dashboard · Review · ＋ · Progress · Menu**. Top mobile bar slims to **Wordmark · Friends · Bell**; the hamburger's drawer moves to the bottom "Menu" tab. Anand's reasoning: thumb-reach on a tall phone + not showing a non-power-user two overlapping nav systems; "Groups" dropping one tap deeper into the Menu drawer is fine because **Review**, not Groups, is the daily driver.
- **`src/components/layout/NavBottomTabs.jsx` (new)** — `md:hidden`, `fixed inset-x-0 bottom-0 z-50`, full-width (rendered by `Navigation.jsx` as a **sibling** of the top `<nav>`, which is now a fragment — NOT inside `max-w-7xl`). `bg-rv-bg-1 border-t border-rv-border shadow-rv-bar font-plex`; row `h-14`; each target `min-h-[48px]`. **Data-driven `TABS`** — `route` entries render `<Link>`, `create` + `menu` are structural; a later top-level destination is a one-line add. **＋** opens a `<Sheet side="bottom">` (`rounded-t-obj`) action-sheet: Upload Note / Create Flashcard / **Bulk Upload (hidden unless `isProfessor || isAdmin || isSuperAdmin`)**. Self-gates: `if (!user) return null` + `if (inStudySession) return null` (from `useStudySession()` — see item 6).
- **`src/components/layout/NavMenuSheet.jsx` (new)** — the former NavMobile hamburger `<Sheet side="right">` and **all of its content, verbatim** (Study/Create/Groups/Following/Progress/Contributions/Achievements/Help/Settings + course-context switcher + professor/admin/super-admin sections + Sign Out). Only the trigger moved (top bar → bottom "Menu" tab). `useCourseContext` / `useState(open)` / `handleNavClick` / `getInitials` / `getRoleBadgeClass` moved here with it.
- **`NavMobile.jsx` slimmed** to Wordmark (left) + `<FriendsDropdown>` + `<ActivityDropdown>` (right). Friends + Bell (and their unread/pending badges) **stay in the top bar** — no counted entity maps to a bottom tab without a new fetch (deliberate departure from the kickoff's "badge on the bar" line; noted in the /__design block).
- **`src/lib/navActive.js` (new)** — the Sprint 6.0/6.2 route predicates factored out as pure `(pathname) => boolean` fns (`isExact` / `underAny` / `isCreateActive` / `isStudyActive` / `isManageActive` / `isGroupsActive`). `NavDesktop.jsx` refactored to import them via thin wrappers — **behaviour byte-identical**, re-proven against a re-implementation of the old inline logic: **20 routes × 5 predicates, 0 mismatches** (incl. the 7/7 tie-break: `/dashboard/flashcards/new` → Create not Study; `/dashboard/notes/:id` → Study via prefix). New: **`isReviewTabActive`** (bottom "Review" tab — study-session cluster only: `review-flashcards` / `review-session` / `review-by-subject` / `study`; the Browse-Notes + My-Flashcards **list** routes and note detail = *no active tab*, because the bar has its own Dashboard + Progress tabs and a library-browsing student isn't "in review").
- **Study-loop collision (item 6) — resolved via (a), refined after live testing.** First cut: `NavBottomTabs` returned `null` on a route match (`/dashboard/study` + `/dashboard/review-session`). Live test caught that `/dashboard/review-session` is a **subject-picker list** until the user taps Start — the route check stranded them with no nav. **Final:** new **`src/contexts/StudySessionContext.jsx`** (`<StudySessionProvider>` above the router in `App.jsx`; `useStudySession()` → `{ inStudySession, setInStudySession }`, no-op outside the provider). **`StudyMode.jsx`** — the card loop for BOTH entry paths (`/dashboard/study` route + ReviewSession's embedded view) — sets the flag `true` on mount / `false` on unmount; `NavBottomTabs` hides while it's `true`. So the picker keeps the bar, Start removes it, Back to Selection restores it. `isFullScreenStudyRoute` helper dropped from `navActive.js`.
- **Pre-existing bug fixed as a ride-along (same live pass).** `StudyMode` card clipped on the right at 390px: `overflow-hidden` card + a no-wrap `Skip 24hr · Show Answer · ⋮` row + `p-8`. Presentation-only: inner padding `p-8` → `p-5 sm:p-8 md:p-12` + `min-w-0` on the flex child; both action rows → `flex-wrap`; `Show Answer` `px-8` → `px-6 sm:px-8`. No SRS / behaviour change; `StudyMode` was otherwise untouched by this sprint.
- **Safe-area + content clearance:** `index.html` viewport meta → `viewport-fit=cover` (added). Bar padded `pb-[env(safe-area-inset-bottom)]`. `PageContainer.jsx` outer wrapper → `pb-[calc(3.5rem+env(safe-area-inset-bottom))] md:pb-0` (**verified:** `calc()`+`env()` parses → computed `56px` on the non-notched emulator; the real notched-device inset (>0) is an operator check on an iPhone / Xcode simulator). Non-`PageContainer` pages got the same clearance class appended to their root div — 8 `dashboard/Content` pages + `ReviewFlashcards` + `ReviewBySubject` (`min-h-screen bg-gray-50`), 3 admin dashboards, `MyContributions`, `NoteEdit`. Additive class only, `bg-gray-50` left as-is on those.
- **`PageContainer` ride-along:** `bg-gray-50` → `bg-rv-bg-0` (`#f9fafb` → `#f9f9fb`, one-token swap, no visible regression).
- **`/__design`:** new "Mobile bottom nav" block in `ReskinGallery` — the bar (active/inactive tabs) + the ＋ action-sheet, **light + dark**. Computed-style verified: light bar `#fff` / border `--rv-border` / active `#1e1b4b` + 2px top-border / inactive `#6b7280`; dark bar `#171a35` / active accent `#a5b4fc` / inactive `#9aa1b8`; `shadow-rv-bar` present.
- **Files:** *new* — `NavBottomTabs.jsx`, `NavMenuSheet.jsx`, `lib/navActive.js`, `contexts/StudySessionContext.jsx`. *changed* — `App.jsx`, `layout/{Navigation,NavMobile,NavDesktop,PageContainer}.jsx`, `pages/dashboard/Study/StudyMode.jsx`, `index.html`, `pages/dev/DesignShowcase.jsx`, `pages/dashboard/Content/{BrowseNotes,MyFlashcards,MyNotes,MyContributions,NoteDetail,NoteUpload,NoteEdit,FlashcardCreate}.jsx`, `pages/dashboard/Study/{ReviewFlashcards,ReviewBySubject}.jsx`, `pages/admin/{AdminDashboard,SuperAdminDashboard,SuperAdminAnalytics}.jsx`.
- **✅ Live-verified (08/09/2026, dev server → live Supabase, all three roles):** bar `md:hidden` only, desktop unchanged; active-tab correct across the nested-route cluster; safe-area / content clearance OK; review-session picker keeps the bar, the study loop drops it (post-fix), card no longer clipped at 390px; ＋ hides Bulk Upload for a plain student, Menu drawer shows prof/admin sections for those roles; no `console.error`; no over-fetch regression. **Committed + pushed to `main`.**

---

## Just Completed ✅ — Sprint 7.0: Global Perf & Correctness — Phase 7 sprint 1 (08/09/2026)

**Frontend only. NO SQL — `DATABASE_SCHEMA.md` §1.4 untouched. No study-loop behaviour change.** `npm run build` clean (7.2s); `npx eslint` clean on every changed file. Clears Findings 3/5/6 from `docs/active/design-review/phase-6-live-verification.md`.

- **7.0-A — nav / notification over-fetch (Finding 5) — FIXED.** Root cause: `AuthContext` `setUser(session?.user ?? null)` on every supabase-js auth event with a fresh object reference → every `[user]`-keyed effect app-wide re-fired ~12×/load. Fixes: (1) `applySession()` — identity-stable `setUser` (functional updater returns the previous ref when `prev?.id === next?.id` → React bails); (2) new `src/contexts/NavDataContext.jsx` — `<NavDataProvider>` (mounted once above the router) owns `useRole`/`useNotifications(5)`/`useFriendRequestCount`; `Navigation.jsx` + 9 former `useRole()` consumers read the shared context shim; (3) `updateUserTimezone` module-guard `tzSyncedThisSession` → ≤1 `profiles` read/session (folds in the 6.5 log-gate); (4) `ProfileDropdown` uses `user_metadata.full_name` first; (5) `CourseContext` exposes `role`/`courseLevel`, `BrowseNotes` consumes them instead of its own `profiles` read. **Before → after, `/dashboard/notes`, super-admin, dev server:** `profiles` **17 → 3**; `get_recent_notifications` / `get_unread_notification_count` / `friendships` / `role_permissions` **6 → 1 each** (prod Phase-6 baseline was 46 / 12 each). Targets (`profiles ≤ 3`, nav RPCs ≤ 2) met. Realtime channels unaffected. Role gating identical (super-admin confirmed).
- **7.0-B — 2× `400` on every authed page (Finding 6) — FIXED (frontend, no SQL).** Root cause: `get_recent_notifications` / `get_unread_notification_count` `RAISE EXCEPTION 'Not authenticated'` (→ HTTP 400 `P0001`) when fired in the auth-init window with `auth.uid()` NULL server-side — not an RPC defect (direct calls with a valid session all 200; no grant/overload/signature issue). The 7.0-A `user`-stabilisation removes the ×12 churn that kept the race landing. Zero 4xx across a 9-route soft-nav sweep (super-admin). Cold-load 3-role sweep on `revisop.com` = the acceptance test, pending operator.
- **7.0-C — web-vitals `startTime` TypeError (Finding 3) — CLOSED (benign-upstream).** Not in the repo (no `web-vitals`/`@vercel/speed-insights`/`@vercel/analytics` dep, no import, nothing in `index.html`/`vercel.json`). Injected by **Vercel Speed Insights** at the edge — not pinnable by us. **Operator action:** disable Speed Insights in the Vercel project dashboard (RUM unused) → removes the script at source.
- **7.0-D — `<Num>` / `rv-*` override — FIXED (approach a).** `src/lib/utils.js` `cn()` now uses `extendTailwindMerge` registering `rounded → rec|obj`, `shadow → rv|rv-bar`, `text/bg/border-rv-*` colour scale. The `text-rv-*` half of the `reference_rv_num_atom_tailwind_ordering` memo was **stale** — tailwind-merge@3.4.0 already resolved those; genuinely un-merged were `rounded-rec/obj` + `shadow-rv/rv-bar` (latent, no live collision). `/__design` light+dark computed radii/shadows/colours identical before/after; toned `<Num>` provably takes its override (`#b91c1c` light / `#ef4444` dark). Sprint 6.5 `NUM_TYPE` workaround reverted in `Progress.jsx` `ForecastCard` → `<Num className={t.text}>`. Memo updated.
- **7.0-E — due/forecast tone hierarchy — Anand's call: INVERT.** "Due Today" is the loudest surface (the only call to action). `Progress.jsx` `ForecastCard` `FORECAST_TONES`: `calm` (0) / `amber` (1..24, "do these now") / `danger` (>24) / **new `quiet`** for "Next 7/30 Days" (`--rv-bg-1`/`--rv-border`/`text-rv-ink-600`, was `amber`). `DUE_TODAY_ALARM_THRESHOLD` unchanged (24). `Dashboard.jsx`: student "N items ready" CTA legacy green-gradient → `--rv-amber-50`/`--rv-amber-edge` + navy button; "All caught up" card legacy `bg-amber-50` → `--rv-green-50` calm (matches Progress "Due Today: 0"). `ForwardLedgerMacro` already navy magnitude ramp — unchanged. Verified super-admin: "Due Today: 0" calm green, "Next 7/30" quiet ink; amber/danger classes computed-checked.
- **Sentry:** 7.0 is a reasonable home for an init (error visibility before the epic sprints) — flagged, **not added** (needs Anand + a DSN).
- **Files:** `src/contexts/{AuthContext,NavDataContext(new),CourseContext}.jsx`, `src/App.jsx`, `src/components/layout/{Navigation,ProfileDropdown}.jsx`, `src/pages/dashboard/Content/BrowseNotes.jsx`, `src/lib/utils.js`, `src/pages/dashboard/Study/Progress.jsx`, `src/pages/Dashboard.jsx`, + import-path swap: `AdminAnalytics`, `AdminDashboard`, `BulkUploadTopics`, `SuperAdminAnalytics`, `SuperAdminDashboard`, `BulkUploadFlashcards`, `ProfessorAnalytics`, `MyFlashcards`, `NoteDetail`. `Help.jsx` left on `@/hooks/useRole` (swap surfaced a pre-existing `react-hooks/set-state-in-effect` error at `Help.jsx:192`); `professor/ProfessorTools.jsx` dead/unrouted, untouched.
- **✅ Verified per-role (08/09/2026, dev server → live Supabase, inline browser — student TestOutlook / professor CA Anand More / super-admin):**
  - **7.0-A:** `/dashboard/notes` — `profiles` **3** all three roles (`timezone` + `useRole` role + `CourseContext` role/course, one each); `get_recent_notifications` / `get_unread_notification_count` / `friendships` / `role_permissions` **1 each** all three roles (was 6). `⏰ Timezone already set` 1×/load. Role gating identical: student = no Manage / no Analytics + Browse-Notes course-lock via `CourseContext`; professor = Analytics present, no Manage; super-admin = Manage + admin/super-admin sections.
  - **7.0-B:** soft-nav sweeps (9–10 routes + `history.back()`) per role — **0 fetch 4xx, 0 `window.error`/`unhandledrejection`, 0 `console.error`**. The cold session-restore path is only reproducible on production (`revisop.com`); could not repro the 400 on the warm dev server even pre-fix.
  - **7.0-D:** `/__design` light + dark — toned `<Num>` takes its override (`#b91c1c` / `#ef4444`); computed radii (4/14px), shadows, colours, and the study-loop primitives unchanged. Student opened a real due card ("Auditing & Ethics") — frame `r14` / `--rv-bg-1` / `--rv-border` / Plex, identical to the Phase-6 baseline; **not graded**.
  - **7.0-E:** student "2 items ready" CTA → `bg-rv-amber-50` (`#fffbeb`) / `border-rv-amber-edge` (`#fcd34d`) / **navy** button (`#1e1b4b`). **Professor Progress "Due Today: 7" → AMBER** `bg-rv-amber-50 border-rv-amber-edge` (was `neutral` white in 6.5 — the exact regression); "Next 7 / 30 Days: 7" → **quiet ink** `bg-rv-bg-1 border-rv-border text-rv-ink-600` (was `amber`). Super-admin "Due Today: 0" → calm green. Inversion confirmed: today's pile now the loudest, forecasts recede.
- **⏳ Operator follow-ups before Sprint 7.1:** (1) **disable Speed Insights** in the Vercel project dashboard (7.0-C — removes the edge-injected web-vitals script at source). (2) Optional: cold-load nav sweep on live `revisop.com` per role (7.0-B — the production-only path). SHIPPED as `6c78f73` on `main` (Vercel auto-deploy).

---

## Just Completed ✅ — Sprint 6.5: Verification Fixes — Phase 6 close-out (07/09/2026)

**Clears the two actionable Phase-6 live-verification findings. Frontend-only for A/B/C/E; D is diagnostic-first SQL, held for the operator (no DB access in the build session).** `npm run build` clean (18.1s); `npx eslint` clean on every changed file.

- **A — `Progress.jsx` migration onto `--rv-*`.** `src/pages/dashboard/Study/Progress.jsx` + `src/components/progress/StudyHeatmap.jsx` + `src/components/progress/SubjectMasteryTable.jsx` — class-reference swaps only (no DOM/layout change): `text-gray-*` → `text-rv-ink-*`, `border-gray-*`/`divide-gray-*` → `rv-border`, `bg-white`/`bg-gray-50/100` → `bg-rv-bg-1`/`bg-rv-bg-2`, `shadow-sm` → `shadow-rv`, `hover:*` → `--rv-*` equivalents, `font-plex` on the container. Section eyebrows → the 6.1 `<Label>` primitive; every stat/count/`%` numeral → the mono `<Num>` atom (`IBM Plex Mono` / `tabular-nums`). QuestionType + SubjectMastery progress bars `bg-amber-500` → `bg-rv-navy` on `bg-rv-slate-50` (6.3 accuracy-bar precedent). Suspended collapsible amber→navy-tint; Mastered collapsible green→`--rv-green` tokens (semantic kept, on-token). StudyHeatmap 4-step green ramp → single-hue navy ramp (`bg-rv-navy/20·/40·/70·navy`) — magnitude, not status. Stat-tile accent icons + container `rounded-lg` left (colour/type/border/bg only, 6.3 precedent).
- **B — "Items Mastered" stat re-pointed.** The lifetime-stats effect no longer computes `mastered` as `new Set(activeReviews.map(r => r.flashcard_id)).size`; it reads `supabase.rpc('get_mastered_cards', { p_user_id }).length` — the exact SSOT (`reviews.status='mastered'`) the Sprint 6.3 dashboard "Mastered" tile and this page's own "Mastered Items (N)" list use. One added RPC on mount, same `lifetimeLoading` gate, no schema change. **Expected before/after for TestOutlook: 29 → 1** (live check pending).
- **C — "Due Today" alarm threshold + tokens.** `ForecastCard` refactored from a raw `accent` string to a `tone` prop, all on `--rv-*` (no `red-50`/`amber-50`). **Threshold `DUE_TODAY_ALARM_THRESHOLD = 24`** (matches the Sprint 6.0 "~24" note): `due_today === 0` → `calm` (muted `--rv-green`); `1..24` → `neutral` (`--rv-ink-900` on `--rv-bg-1`, **no alarm**); `> 24` → `danger` (`--rv-danger` text + border). "Next 7 / 30 Days" → `amber` (`--rv-amber-50` / `--rv-amber-edge`). Tile numeral `<Num className="text-inherit">` so it takes the tone. Implemented as the recommendation; flagged for review.
- **D — `get_following_leaderboard` `[FIX]` + Task 6.5-D reconstruction audit — RESOLVED, faithful (07/09/2026).** `[FIX]`: `02_FUNCTIONS` DEPLOYED 07/09/2026 (`CREATE OR REPLACE` ran clean) — **in-place fix, no OUT-column rename**: `#variable_conflict use_column` + `DENSE_RANK()` aliased `rnk` + every ref table-qualified; RETURNS TABLE / SECURITY DEFINER / STABLE / unquoted `search_path` / `auth.uid()` gate / `authenticated` grant all preserved → `LeaderboardWidget.jsx` `row.rank` untouched, SQL shipped alone. `03_TEST` **9/9 PASS**; Following tab renders a row live, no 400, no console error. **Audit (`04_AUDIT_current_definition.sql` + `04_AUDIT_membership_test.sql`):** the 6.5 thread replaced the whole body (01_DIAGNOSTIC's live-body capture was lost before 02 overwrote it), and `03_TEST` validates the stat math, not the membership set. **Current live body captured 07/09/2026 via `pg_get_functiondef` → byte-identical to `02_FUNCTIONS`** (STABLE / SECURITY DEFINER / `search_path=public, extensions` confirmed; the `TO 'public', 'extensions'` echo is two quoted identifiers, not the single-string outage form). **Original body unrecoverable from git** — Sprint 3.5 ran the `CREATE` directly in Supabase, no `.sql` ever committed (`git log -S 'get_following_leaderboard' --all` = 3 commits; `--diff-filter=A/D` over `*leaderboard*` = only sprint6.5/01–03). Diffed the (now confirmed live) body against the Sprint 3.5 behavioural contract in `DATABASE_SCHEMA.md` @ `071395d`: **(1) follow-graph join** `cohort` = `SELECT auth.uid() UNION SELECT f.followee_id FROM public.follows f WHERE f.follower_id = auth.uid()` — directional, correct for "Following"; no `friendships`, no reciprocal `AND EXISTS (reverse follow)`. **(2) population filter** students-only (`p.role = 'student'`), no course/`account_type` filter. **(3) result window** `DENSE_RANK() OVER (ORDER BY reviews_this_week DESC, study_time DESC)` over the full cohort → `WHERE rnk <= 20 OR uid = auth.uid()`, N=20, self always in, exact rank, one `is_self`. **All three MATCH → faithful; no `[FIX]` shipped.** `04_AUDIT_membership_test.sql` (independent oracle: caller ∪ followed-students == RPC `user_id` set, no extras/missing, rank by reviews DESC) **RUN 07/09/2026 — 5/5 PASS** on account `f9377860…`. Live follow graph is sparse (6 rows, 4 followers, **max 1 followed-student per student** — nobody follows ≥2), so the test used the richest available account (follows 1 → 2 expected rows) and the >20 top-N cutoff is unexercised on live data (contract-trivial, code matches). This also explains the earlier ambiguous "TestOutlook (you)"-only row: almost nobody follows anyone. **Finding 2 fully closed.**
- **E — timezone log gated.** Module-scoped `tzAlreadySetLogged` in `src/contexts/AuthContext.jsx` — `⏰ Timezone already set: …` logs ≤ 1×/session (was ~77×/page). Other sync branches untouched.
- **Verified — local + live per-role (07/09/2026, dev server → live Supabase, inline browser):** `npm run build` clean; `npx eslint` clean on the 4 changed files.
  - **Student (TestOutlook):** eyebrows exact `Label` spec (Plex 11px/500/0.77px/`#6b7280`); all numerals `IBM Plex Mono` + `tabular-nums`; bars `bg-rv-navy` on `bg-rv-slate-50`; heatmap navy ramp (green gone). **B — three-way Mastered equality: Progress tile `1` == "Mastered Items (1)" == Dashboard "Mastered" `1`** (was 29). **C — "Due Today: 0" → `bg-rv-green-50`/`text-rv-green`, calm, not red.** **D — Leaderboard Following tab renders a row, no 400, no console error.**
...
## (prior) Sprint 6.0 → 6.4 status — Phase 6 CODE COMPLETE (05/09/2026)
**6.0 → 6.4 CODE COMPLETE (05/09/2026).** SRS Ladder Epic + 6.1 (foundation) + 6.2 (chrome) shipped; 6.3 (dashboards) code-complete + RPCs deployed; **6.4 (study loop) code-complete — no core-reskin SQL, not yet pushed.** Phase 6 closes once 6.3 + 6.4 are pushed and live-verified on `revisop.com` with an operator student session. Phase 5 COMPLETE (shipped 2026-07-02). **Landmine Cleanup L1–L4 + L5 ALL COMPLETE.** **Sprint 6.0 (Correctness) COMPLETE — SQL deployed & verified, frontend pushed to main (02/09/2026).** **SRS Ladder Epic — Phase 1 + 2 + 3 ✅ SHIPPED (03/09/2026): all SQL (`01`–`06`) deployed & verified in Supabase; frontend pushed to `main` (commit `8323ee3`) → Vercel auto-deploy. ⏳ Final step: live-verify on recallapp.co.in with a logged-in student (grade-up / rung-table / MASTERED / one-write-per-grade). Deferred to Sprint 6.4: grade-button reskin + prominent interval UI + the "Items Mastered" headline stat.** **Sprint 6.1 (Design Foundation) ✅ SHIPPED — committed + on `main` (`f73e444`): additive `--rv-*` token layer + self-hosted Plex/Literata + `src/components/revisop/*` primitives + `/__design` QA route; zero pages migrated, zero behaviour change, zero SQL.** **Sprint 6.2 (Shared Chrome Reskin) ✅ CODE COMPLETE (04/09/2026) — nav shell (`Navigation`/`NavDesktop`/`NavMobile`) + `Button`/`Input`/`Textarea`/`Label` atoms migrated to `--rv-*` + Plex; `<Wordmark />` live in nav; active-item amber→navy; `/__design` Chrome section; `context.md` stale branding line replaced. No SQL, no DOM/layout/behaviour change. `npm run build` clean; eslint clean on changed files (`button.jsx` `react-refresh` error is pre-existing baseline). ✅ Pushed to `main` (`a858e3b`) + live-verified on revisop.com with both a super_admin and a real student (TestOutlook) session — nav tokens, 7/7 nested-route active-state, mobile sheet, `Button` ripple, study loop untouched, non-chrome pixel-identical, console clean.** Deferred: **leaked-password protection is Pro-plan-gated** — cannot enable on the current Free plan. ⏳ **Do on Pro upgrade.** ✅ skip_card/suspend_card column bug fixed 04/07/2026.

---

## Just Completed ✅ — Sprint 6.4: Study Loop UI Reskin — Phase 6 close-out (05/09/2026)

**The front/back review loop is on the unified frame. No SRS behaviour change — presentation + interval display only.** `npm run build` clean (7.6s); `npx eslint` clean on every changed file. **No core-reskin SQL. Not yet pushed.**

- **Pre-work.** (a) `ReviewSession.jsx` delegates the whole card UI to `<StudyMode>` — **one reskin covers both entry paths** (review-session + standalone new-card study); ReviewSession keeps only a light chrome tokenisation. No separate front/back component exists — the flip is inline JSX in `StudyMode.jsx` gated on `showAnswer`. (b) Interval computation unchanged from the live SRS-Ladder Phase-3 `gradePreview` memo (one-time `get_srs_ladder_config` on mount, zero per-card network); the `GradeButtonRow` labels run the exact `srs_preview` / `submit_review` maths. (c) **Literata trigger** = revealed flashcard answer with `back_text.trim().length >= 320` (`LITERATA_MIN_CHARS`); no note/case-study bodies exist in this loop (Phase 7). Flag flipped ON. (d) **Items-Reviewed (7d) fix — operator-approved, NO schema/function SQL:** `reviews.last_reviewed_at` already exists (`DEFAULT now()`) and has always been maintained (pre-ladder `handleRating` + `submit_review` both set it); the bug is that the stat queries filter `created_at` (the card's *first* review — `submit_review` UPDATEs the one row per user/card). Re-pointed the "Items Reviewed" recency filter (Dashboard tile + GoalProgressWidget today-count + Progress 7d/30d window) to `last_reviewed_at ?? created_at`; **streak + weekly-accuracy left on `created_at`** for a Phase 7 analytics pass. (e) **Animation spec:** answered card `translateY(-10px) scale(.97)` + fade 240ms `cubic-bezier(.4,0,.2,1)`; next card rises from `+8px` + fade 180ms; `prefers-reduced-motion` → 100ms opacity-only. (f) **Before-screenshots + live full-session run: PENDING an operator-supplied student session on `revisop.com`** (no login in the build session).

- **Frontend — `StudyMode.jsx`:** card frame → `<Card elevated>` (r14, `--rv-bg-1`, `--rv-border`, Plex) + leading `<VerifiedEdge>` + `rv-forward-in/out` animation on a new `transitioning` state; grade row → `<GradeButtonRow>` fed by a new `gradeButtons` memo (`{label, rating, iv:'Nd', bucket}`), no traffic-light colour, mono interval, slate miss; `ANSWER` chip green→navy, `QUESTION` chips → neutral; answer body → Literata when `isReadingBody()`; progress bar + session counters + session-complete + empty + loading states → `--rv-*` / neutral glyphs / mono `Num`; page ground `bg-amber-50`→`bg-rv-bg-0`; Show Answer → full-width `min-h-[48px]`. `fmtInterval` + `CheckCircle/XCircle/AlertCircle` imports removed.
- **Frontend — `ReviewSession.jsx`:** subject-list cards, "All Caught Up! 🎉" (green→navy), "Reviewing: N cards due" banner (amber→navy), loading + grounds → `--rv-*` + Plex; counts → mono. shadcn `Card` radius left (6.3 precedent).
- **Frontend — primitives + re-point:** `AnswerOption.jsx` `correct` state de-greened (navy + glyph + label, no `rv-green`); `Dashboard.jsx` + `Progress.jsx` "Items Reviewed" recency → `last_reviewed_at ?? created_at`.
- **`index.css`:** additive `@keyframes rv-forward-out/in` + utilities + `prefers-reduced-motion` override. **`revisop-tokens.js`:** `LITERATA_MIN_CHARS` + `isReadingBody()`; `REVISOP_LITERATA_ENABLED` → `true`.
- **`/__design`:** new "Study loop" block in `ReskinGallery` — reskinned review card + `VerifiedEdge` + Q/A toggle + Literata long-answer + `GradeButtonRow` (`→ 1d / 7d / 14d`) + neutral `AnswerOption` row + replay-animation control + session-complete card.
- **SQL (`docs/database/sprint6.4/`, NOT deployed — optional):** `01_DIAGNOSTIC_items_reviewed_recency.sql` (read-only; measures `last_reviewed_at` NULL coverage — expected 0), `02_DATA_backfill_last_reviewed_at.sql` (idempotent NULL backfill; run only if `01` finds NULLs; not a deploy-order blocker — frontend falls back to `created_at`).
- **Verified (local):** `npm run build` clean, no `DesignShowcase` chunk in `dist/`; `npx eslint` clean on changed files. `/__design` **light + dark** computed styles — card `border-radius 14px` / `--rv-bg-1` / Plex; `VerifiedEdge` 3px; `GradeButtonRow` `min-height 88px`, `--rv-navy-400` border, mono `--rv-navy` label, no red/amber/green; `AnswerOption` correct = navy (no green); answer body computes `font-family: Literata` at ≥320 chars, `document.fonts.check('16px Literata')` true (lazy fetch fired on reveal); `rv-forward-*` keyframes registered. Green-removal grep clean across `StudyMode` / `ReviewSession` / `GradeButtonRow` / `AnswerOption`.
- **⏳ Pending (operator student session on `revisop.com`):** before-screenshots; live full-session run; ≥3-rung × {Hard,Medium,Easy} parity matrix (client preview == `srs_preview` == `submit_review` write); per-card network trace (one `get_srs_ladder_config`, one `submit_review`/grade, one `get_study_queue`, zero inter-card refetches); reduced-motion emulation; Literata load trace on a real long back; "Items Reviewed (7d) non-zero after a re-review"; console state.
- **⏳ Follow-up:** operator to `git commit` + push (bash `printf`), then live-verify on `revisop.com`, then Phase 6 is closed. Run `01_DIAGNOSTIC` in Supabase anytime to confirm the backfill is a no-op.

### Phase 6 Reskin — close-out rollup
- **SRS Ladder Epic** (pre-6.1) — deterministic server-side expanding ladder; `submit_review` write SSOT; `get_srs_ladder_config` / `srs_preview`; `reviews.rung` + `status='mastered'`.
- **6.1** — additive `--rv-*` token layer + self-hosted Plex/Literata + `src/components/revisop/*` primitives + `/__design`. Zero pages migrated.
- **6.2** — shared chrome: nav shell + `Button`/`Input`/`Textarea`/`Label` onto `--rv-*` + Plex; `<Wordmark/>` live; active-item amber→navy.
- **6.3** — all 4 `Dashboard.jsx` role branches onto `--rv-*` + Plex; `ForwardLedgerMacro` live on real scheduled data; `AnonymousStats` parked; educator accuracy-by-qtype + cohort ledger; overlay-surface atom family + nav dropdowns finish the colour migration; 3 RPCs deployed.
- **6.4** (this) — study loop onto `--rv-*` + r14; `GradeButtonRow` wired to `get_srs_ladder_config`; revealed-answer neutralised; post-forward animation; Literata activated; session-complete on tokens; Items-Reviewed re-point.
- **Parked to Phase 7:** question-type epic (MCQ multi / FITB / match / case-study + unified question frame), exam-date anchor, nav-shell rebuild, the 4× `401` investigation, streak/weekly-accuracy `created_at`→`last_reviewed_at`, `IntervalChip` `tone="green"` + study-loop destructive `text-red-600`.

---

## Just Completed ✅ — Sprint 6.3: Dashboards Reskin + Forward Ledger live + gamification parked (05/09/2026)

**Dashboards now visibly change — that's the point.** `src/pages/Dashboard.jsx` (all 4 role branches) migrated onto `--rv-*` + Plex; the Forward Ledger macro ships for the first time against real scheduled data; the "You vs Class" widget is removed from the student dashboard; the educator dashboard gains accuracy-by-question-type + a cohort forward-load ledger; the overlay/dropdown atom family + the 4 nav right-side dropdowns finish the colour migration deferred from 6.2. `npm run build` clean (17.4s); `npx eslint` clean on every changed file. **No push yet.**

**✅ SQL DEPLOYED & VERIFIED 05/09/2026** — `docs/database/sprint6.3/01_FUNCTIONS` live; `02_TEST` 12/12 real assertions PASS (grouping / hit-miss / skip-excluded / concept-card exclusion `[CRITICAL]` / IDOR `[CRITICAL]` ×3). One check initially FAILed as a **test-fixture bug** (summed all 8 lanes assuming the real test student had zero pre-existing reviews — got 5 vs 3; concept exclusion was in fact working, else it'd be 6) → rewritten delta-based. **Next: push the frontend → live-verify per role on `https://www.revisop.com`.**

- **Pre-work.** (a) One file renders all four dashboards — `Dashboard.jsx`, role branch on `userRole`. `/admin` + `/super-admin` consoles are **out of scope**. (b) **No gamified widget exists on any dashboard** — only a transient badge toast; the "My Achievements" link is already in the nav `ProfileDropdown`. Operator picked **park `AnonymousStats`** ("You vs Class") — removed from the student dashboard, component + RPC left in repo, **DB untouched**; Leaderboard + Streak tile kept (reskinned normally); achievements link left in place. (c) **Due trace already clean** — student `reviewsDue` uses `get_study_queue` (6.0); no dashboard does client-side due math; the "no due arithmetic" grep already passes. (d) **Educator accuracy data-check CLEARS** — `reviews.quality` ⋈ `flashcards.question_type` via the `get_professor_*` cohort join; accuracy = hit (`quality IN (3,5)`) / graded (`quality > 0`), `concept_card` excluded (operator-confirmed mapping). (e) **Forward Ledger data gap** — `get_due_forecast` gives only 3 cumulative scalars; operator approved **adding a small forecast-buckets RPC**. (f) **Items-Mastered re-point approved** as a ride-along on the tile rebuild (was "distinct cards ever reviewed"; now `get_mastered_cards` → `reviews.status='mastered'`).
- **SQL (`docs/database/sprint6.3/`, ✅ deployed & verified 05/09/2026):** `get_due_forecast_buckets(p_user_id)` → 8-lane student forward load; `get_educator_accuracy_by_qtype(p_professor_id, p_course_level)` → accuracy per question_type; `get_educator_cohort_forecast_buckets(p_professor_id, p_course_level)` → cohort forward load. All SECURITY DEFINER, L5 IDOR idiom, unquoted `search_path`, `REVOKE public/anon` + `GRANT authenticated`. `02_TEST` covers bucketing / hit-miss mapping / skip-excluded / concept-card exclusion `[CRITICAL]` / IDOR `[CRITICAL]`.
- **Frontend — dashboards (`Dashboard.jsx`):** `PageContainer` → `font-plex`; 8 section eyebrows → 6.1 `Label` spec; 6 stat numerals → mono tabular (inline `Num`); bulk palette swap to `--rv-*` (semantic green CTA / amber status / red review surfaces left, per the 6.2 precedent). Student: new "Forward load" `ForwardLedgerMacro` section (real `get_due_forecast_buckets` → `number[8]`, self-gating on non-empty); `<AnonymousStats>` + `classStats`/`fetchClassStats`/`hasUserActivity` removed; Mastered tile re-pointed. Educator: new "Accuracy by question type" bar widget + "Cohort forward load" ledger, both before the Activity Feed, `activeCourse` effect repurposed to refetch on course switch. **Settled-card rule honoured — `git diff` shows no new conditional around "Needs Attention" / "Needs Review".**
- **Frontend — surfaces:** `select`/`dropdown-menu`/`popover`/`command`/`switch` onto `--rv-*` (no shadcn token value edited); `ProfileDropdown`/`FriendsDropdown`/`ActivityDropdown`/`CourseSwitcher` finish the colour migration (semantic role/accept/decline colours + `CourseSwitcher.COURSE_COLORS` identity array left). `/__design` gains a Sprint 6.3 Dashboard + Surfaces block.
- **Verified (local):** `/__design` light + dark — computed styles confirm `SelectContent`/`SelectTrigger` on `--rv-bg-1`/`--rv-border`/`4px`/Plex, `Switch` on = `#1e1b4b`, dark gallery bg `#0d0f24` + navy swatch `#a5b4fc`; console clean; `npm run build` + `npx eslint` clean.
- **✅ Per-role live verification DONE 05/09/2026** (dev server → live Supabase, inline browser, real student / professor / super_admin sessions):
  - **Student** (TestOutlook): eyebrows `IBM Plex Sans 11px/500/uppercase/0.77px/#6b7280` (exact `Label` spec); stat numerals `IBM Plex Mono 24px tabular #0a0e1a`. **Forward Ledger from real data** — lanes `[2,1,0,0,0,0,0,0]`, and `get_study_queue().length` = 2 = the "2 items ready" header/CTA (the two RPCs agree). `AnonymousStats` gone (no gap); Leaderboard + Recent Activity kept; "Mastered" tile = real `get_mastered_cards` count (1), label "Items mastered".
  - **Professor** (CA Anand More, CA Intermediate): "Accuracy by question type" live — `Flashcard 87% · 2240 graded`, bar `#1e1b4b` @ 86.7% on `#f1f5f9` track. "Cohort forward load" live — 8 lanes `[1465,1,7,0,0,0,0,560]` (lane 0 ≈ the SQL test's 1469). "Needs Attention" renders unconditionally. `CourseSwitcher` + `ProfileDropdown` open on the real page = reskinned surfaces (`--rv-bg-1` / `--rv-border` / `4px` / Plex; role badge amber-50/navy; "My Achievements" present + functional).
  - **Super_admin** (Anand): Admin Tools + Super Admin Tools (red "Elevated" + red-border tiles kept semantic); "Needs Review" renders unconditionally. `Manage` nav dropdown + a `<Select>` open on `/dashboard/settings` = surface `#fff`/`#e2e8f0`/`4px`/Plex, highlighted option `bg #f0f0f8` (`--rv-navy-50`) / `#1e1b4b` (`--rv-navy`).
  - **Console:** the only errors are a **pre-existing** 4× `401` that also fires on the untouched `/dashboard/notes` (global artifact — likely push-subscription/notification for these accounts), plus `400 invalid_credentials` from failed login attempts. No Sprint 6.3 regression.
  - **No-regression:** `/dashboard/notes` + `/dashboard/settings` render unchanged apart from the intended overlay-atom reskin; **zero study-loop files touched** (`git diff --name-only` — the pre-6.2 traffic-light grade UI is untouched by construction).
- **Deviations:** `get_educator_cohort_forecast_buckets` is a 3rd RPC beyond the one approved (same pattern, own file, droppable at deploy). `CourseSwitcher.COURSE_COLORS` per-course identity array un-migrated. shadcn `<Card>` radius left at `rounded-xl` 12px (border migrated to `--rv-border`; radius unchanged to minimise regression risk).
- **⏳ Follow-ups:** cleared to `git commit` + push (SQL live, all 3 roles verified); after push, spot-check on `https://www.revisop.com`. Optional: the pre-existing 4× `401` deserves its own look sometime (not 6.3 scope). 6.4 = study loop + Literata + `AnswerOption` green removal + the 2 remaining carried items (seeded test card cleanup, Items-Reviewed `created_at`).

---

## Just Completed ✅ — Sprint 6.2: Shared Chrome Reskin (04/09/2026)

**First sprint that intentionally changes shipped UI — chrome skin only. Same DOM, same layout, same behaviour; new skin.** `npm run build` clean; `npx eslint` clean on every changed file (`button.jsx`'s `react-refresh/only-export-components` is **pre-existing on HEAD** — the shadcn atom also exports `buttonVariants` — baseline, not introduced here). No SQL. **Not yet pushed.**

- **Pre-work — shared-atom inventory.** Import counts: `Button` 55 files, `Input` 24, `Label` 16, `select` 15, `Textarea` 7, `switch` 1, `checkbox` 0 (no such atom). Button variant usage: `outline` 108, `ghost` 56, `destructive` 20, `link` 5, explicit `default` 2 (most `<Button>` rely on the `default` defaultVariant), `secondary` 0. **In scope:** `Button`, `Input`, `Textarea`, `Label` (`src/components/ui/*.jsx`) — genuinely app-wide chrome. **Deferred:** `select.jsx` (Radix portal dropdown — the trigger shares the input shell but content/item is a popover *surface*; belongs with `dropdown-menu`/`popover`/`command` as one unit → 6.3), `switch.jsx` (1 file), every overlay/feedback primitive (`dialog`/`sheet`/`alert`/`card`/`progress`/`toast` → 6.3/6.4).
- **Pre-work — baselines.** No student test session was available in this session; captured the authed baselines from the available **super_admin** session on revisop.com instead (super_admin nav is a strict superset of the student nav — Manage dropdown + admin sheet sections — so the restyle is exercised *more* thoroughly, not less): `/dashboard` (admin dashboard), a note-detail page (`/dashboard/notes/:id`), the study-loop "All Caught Up" empty state (`/dashboard/review-session` — account has no due cards, so no live review card / `GradeButtonRow`; that primitive is 6.4 scope and must not be touched here anyway). **The authenticated app has no dark-mode toggle** (`.dark` is only ever applied on `/__design`) → authed verification is light-only; nav dark styling is proven on `/__design`.
- **Nav** (`src/components/layout/`): `Navigation.jsx` shell `bg-white border-gray-200` → `bg-rv-bg-1 border-rv-border` + `font-plex`. `NavDesktop.jsx` + `NavMobile.jsx`: inline `#f59e0b`/`#1e1b4b` wordmark → shared **`<Wordmark />`**; active-item **amber → navy tint** (`bg-amber-50 text-amber-700` → `bg-rv-navy-50 text-rv-navy`, per the Pass 2 `SideRail` reference), inactive grays → `text-rv-ink-600 hover:bg-rv-bg-2`, nav-pill `rounded-md` → `rounded-rec`. **Sprint 6.0 route-prefix active-state logic untouched — only its className output restyled.** `NavMobile` `SheetContent` carries its own `font-plex bg-rv-bg-1 text-rv-ink-900` (it portals out of the `<nav>` subtree); sheet interior migrated colour-only; role badges + sign-out `text-red-600` deliberately left (semantic, not chrome palette).
- **`Button`** (`src/components/ui/button.jsx`): `default` → `bg-rv-navy text-rv-bg-1` (navy light / accent lavender dark — both tokens flip together so the label stays contrasting); `secondary` → navy-50 tint; `outline` → `border-rv-navy-400` neutral ink; `ghost` → `text-rv-ink-600 hover:bg-rv-bg-2`; `link` → `text-rv-navy`; `destructive` → `bg-rv-danger` (`#b91c1c` light / `#ef4444` dark) `text-white`. Base radius `rounded-md`→`rounded-rec` (all sizes), `font-plex`, focus ring `ring-rv-navy`. **`GradeButtonRow` unaffected — raw `<button>`, not this atom.**
- **`Input` / `Textarea`**: `rounded-rec`, `border-rv-border` (= the old `--input` value in light → border colour visually unchanged; deltas = radius 6→4px, Plex, navy focus ring, `placeholder:text-rv-ink-400`), `disabled:bg-rv-bg-2`, `aria-[invalid=true]:border-rv-danger`. `bg-transparent` kept. **`Label`** (shadcn form-field, ≠ `revisop/Label.jsx` eyebrow): `font-plex text-rv-ink-900`.
- **`/__design`** — new "Chrome" section: representative reskinned nav strip + all six `Button` variants + `Input`/`Textarea`/`Label` states (default/simulated-focus/disabled/error), inheriting the page's light↔dark toggle + both-themes view.
- **`context.md`** — the "⚠️ STALE (do not use)" blue→purple gradient tombstone (step 6, in-scope) replaced with a positive two-tone typographic branding description (flat Plex text, `Revis` amber / `Op` navy→accent, no gradient), pointing at blueprint §1041/§1209.
- **Verified (local):** `/__design` Chrome section renders correctly light **and** dark — nav strip active = navy tint, `Op` legible in dark (lavender on dark navy), all six Button variants correct (`destructive` = `#b91c1c`), Input disabled/error/focus states on tokens; `/login` (public, ripple target) renders with the navy Sign-In button + reskinned fields; console clean on both. Build clean, 6.80s.
- **✅ Live-verified on `https://www.revisop.com`** (deploy `a858e3b`) — **both** the super_admin session (Anand) **and** a real **student** session (TestOutlook):
  - Desktop nav computed styles: shell `bg` `#fff` (`--rv-bg-1`), border `#e2e8f0` (`--rv-border`), `font-family: "IBM Plex Sans"`; active item `bg #f0f0f8` (`--rv-navy-50`) / ink `#1e1b4b` (`--rv-navy`) / radius `4px` — amber gone. Student nav correctly has no "Manage" item.
  - **Nested-route active state — 7/7 routes:** `/dashboard/notes` → Study, `/dashboard/flashcards` → Study, `/dashboard/flashcards/new` → **Create** (tie-break over Study's broader prefix), `/dashboard/notes/new` → **Create**, `/dashboard/progress` → Study, `/dashboard/review-session` → Study, `/dashboard` → Dashboard (exact). Every highlight = `--rv-navy-50`. Sprint 6.0 `underAny`/`isCreateActive`/`isStudyActive` intact.
  - **Mobile sheet** (student, forced-open + computed styles): `bg #fff`, Plex Sans (the `font-plex` on `SheetContent` carries across the portal), header `bg #f0f0f8`, avatar `#1e1b4b`, inks on `--rv-ink-*`; no Admin/Super-Admin sections for the student; role badge (green "student") + Sign Out (red) preserved as intended.
  - **`Button` ripple:** study-loop "Show Answer" / "Start" / "Return to Dashboard" (`default`) → navy `#1e1b4b` + `rounded-rec` (were slate-black `rounded-md`); "Skip 24hr" / "Restart" / "Study New Cards" / "Dashboard" (`outline`) → `border-rv-navy-400` + `rounded-rec`; note-detail "Share" (`outline`) same.
  - **Study loop NOT touched:** opened a real due card (TestOutlook, "Auditing & Ethics") → the QUESTION/ANSWER card + the red/amber/green traffic-light Hard/Medium/Easy grade buttons are **exactly the pre-6.2 design** — 6.2's shared `Button` restyle did not leak into `GradeButtonRow` / the study-loop grade UI (that's 6.4). Did not grade (no mutation of the test account's SRS state).
  - **Non-chrome unchanged:** dashboard body, note-detail body, study-loop empty state — pixel-identical to the pre-work baselines. Console clean on every page.
- **⏳ Follow-ups:** 6.3 = dashboards + the dropdown-surface atom family (`select`/`dropdown-menu`/`popover`/`command`) + the nav right-side dropdown components; 6.4 = study loop + Literata + `AnswerOption` green removal + the three carried items (Items-Mastered stat re-point, seeded test card, Items-Reviewed `created_at`).
...
**Three corrections vs the original plan, once `09_DIAGNOSTIC` returned live ground truth:**
1. **`friends` tier was already enforced.** The plan assumed friends-tier content had never been RLS-granted. `09` found live `"Users can view friends notes/flashcards"` policies already exist (`visibility='friends' AND EXISTS(accepted friendship)`). So Stage A created **no** friends policy — it only touched the two public-read predicates.
2. **Stage A used `ALTER POLICY`, not DROP/CREATE.** Live policy names have spaces (`"Users can view public notes"`) and target `TO public` (not `authenticated`). `ALTER POLICY … USING (…)` swaps only the predicate in place, preserving name/roles/cmd; `TO public` kept (anon reads public content directly, unchanged from before).
3. **A hidden function dependency was caught before the drop.** `14_DIAGNOSTIC`'s function-body scan found `skip_topic_cards` + `suspend_topic_cards` still referenced `fc.is_public` — a redundant clause (`fc.visibility='public'` already in the same OR; `09` Block 5 proved zero drift). Migrated in `15_FUNCTIONS` (data-equivalent). Same audit-gap class as L1's `vw_study_items` trap — **column drops need a function-body scan, not just pg_depend + grep.**

**What deployed & verified:**
- **Stage A** (`10_SCHEMA`) — `ALTER POLICY` × 2 onto `visibility='public'`. `11_TEST` matrix: **24/24 cells PASS** (owner/friend/stranger/anon × public/friends/private × notes/flashcards), incl. the `[CRITICAL]` stranger/anon-vs-friends/private assertions.
- **Frontend** (commit `ca044d9`, live) — 8 files stripped of `is_public` (`FlashcardCreate`, `MyFlashcards`, `ProfessorTools`, `BulkUploadFlashcards`, `NoteUpload`, `NoteEdit`, `StudyMode` `.or()` filter, `MyNotes` filter + icon).
- **Function migration** (`15_FUNCTIONS`) — `skip_topic_cards` + `suspend_topic_cards` re-created off `is_public`. Final gate: `14` q1 → only `user_badges.is_public` hits remain (different column); `09` q2 → zero policies on `is_public`.
- **Stage B** (`12_SCHEMA`) — `DROP COLUMN is_public` on both tables (indexes auto-dropped). `13_TEST` all PASS: column gone, note + flashcard inserts succeed on `visibility` alone.

**Result:** `visibility` is the sole basis of all read RLS (public + friends tiers) on both tables. `is_public` no longer exists in the schema.

**Test-fixture lessons banked (from matrix + insert-test iterations):** fixtures must self-source CHECK-constrained values (`content_type`) rather than hardcode literals; RLS actor fixtures must be `role='student'` (an admin actor triggers `is_admin()` and masks a real leak/pass); multi-statement Supabase runs show only the LAST result set, and detail rows must come last so failures surface.

**Next:** L3 (search_path hardening, ~80 functions) then L4 (`profiles` FK → CASCADE), then Phase 6. Also queued: the deck-listing per-viewer visibility fix (task chip) — independent of L2, run now that `is_public` is gone.

Files Changed: `docs/database/landmines/09_*.sql` through `15_*.sql` (14 + 15 new this stage), `src/pages/dashboard/Content/FlashcardCreate.jsx`, `MyFlashcards.jsx`, `MyNotes.jsx`, `NoteUpload.jsx`, `NoteEdit.jsx`, `src/pages/dashboard/Study/StudyMode.jsx`, `src/pages/professor/ProfessorTools.jsx`, `src/pages/dashboard/BulkUploadFlashcards.jsx`, `docs/active/blueprint.md`, `docs/reference/DATABASE_SCHEMA.md`, `docs/active/now.md`, `docs/tracking/changelog.md`
...
- **Frontend change (pushed 02/07/2026, commit `921280b`, verified live):** stripped the four SRS keys from `FlashcardCreate.jsx`'s insert payload — no other behavior change. Was the hard prerequisite for the SRS-column drop (`05_SCHEMA`); the SQL was deployed only after the frontend went live and a single-card create was verified in production.
- **✅ DEPLOYED & VERIFIED 02/07/2026:** `02`–`05` + `08` all deployed; `06_TEST` 7/7 PASS (dead objects gone, legacy + SRS columns gone, flashcard create + badge/counter triggers + notes/upvotes reads all still work). blueprint.md §1.11 items #4–#8 + the SRS drop flipped to ✅ RESOLVED.
- **Audit gap caught mid-deploy:** `05_SCHEMA` first failed `2BP01` — the dead `vw_study_items` view still `SELECT`ed the SRS columns, a dependency the grep/index/RLS audit missed. Resolved by dropping the dead view (`08_CLEANUP`; it had zero real consumers — `get_anonymous_class_stats` queries `flashcards` directly — which also retired the Jun-30 anon-leak surface for good), then re-running `05`. **Lesson for L2+: every column/table drop needs a `pg_depend` view/rule dependency check, not just codebase grep + index/RLS.**
...
**✅ SQL DEPLOYED & VERIFIED 2026-07-02.** `docs/database/phase5/17`–`21` deployed; `21_TEST` returned 11/11 PASS (including the one-time-claim security check on the token grant). Frontend pushed to main.
...
**Root cause diagnosed:** The "topic skipping" feature students described was never properly implemented. Only "Suspend Topic" (indefinite removal) existed in the `...` dropdown. Additionally, the topic-level dropdown items had a null bug — they only rendered when `currentCard.topic_id` was truthy, silently hiding the option for cards with `custom_topic` (no `topic_id` FK).

**SQL deployed first (Supabase):**
- `skip_topic_cards(p_user_id, p_topic_id, p_custom_topic)` — new SECURITY DEFINER RPC. Bulk-sets `skip_until = tomorrow` on all active review records for the topic, upserts new records for unseen cards. Leaves suspended cards untouched via `WHERE reviews.status = 'active'` guard.
- `suspend_topic_cards` updated to v2 — added `p_custom_topic DEFAULT NULL` parameter so custom-topic cards can now be suspended via the same RPC. Backward compatible (existing callers unaffected).

**Frontend changes (`StudyMode.jsx`):**
- Added `handleSkipTopic`: calls `skip_topic_cards` RPC, shows "Topic snoozed" toast, removes same-topic cards from current session.
- Fixed `handleSuspendTopic`: null guard now checks `!topicId && !customTopic`; passes both `p_topic_id` and `p_custom_topic` to updated RPC.
- Fixed null bug: topic-level dropdown options now render when card has `topic_id` OR `custom_topic` (both question and answer side).
- Restructured both `...` dropdowns: Skip Topic (24hr) at top as safe action with blue icon; Suspend Card and Suspend Topic moved below separator with red/destructive styling.
- Confirm dialog: `skipTopic` uses default (safe) button variant + "Skip Topic" label; all suspend actions now use destructive variant.

**Documentation updates:**
- `helpContent.js` — `skip-suspend` section expanded to 5 bullets with accurate descriptions; added tip block; FAQ updated from "What does suspending a card do?" to "What is the difference between Skip and Suspend?"
- `guideContent.js` — Added "One topic too heavy? Skip it for today." step to the "Falling Behind" situation.

Files Changed: `src/pages/dashboard/Study/StudyMode.jsx`, `src/data/helpContent.js`, `src/data/guideContent.js`, `docs/active/now.md`, `docs/tracking/changelog.md`
