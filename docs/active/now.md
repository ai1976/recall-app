...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.3 (Dashboard Reporting Surface & Study-Time Split) ✅ SHIPPED 12/09/2026 (`77cfe0c`, pushed to `main` → Vercel auto-deploy).** `npm run build` clean; `npx eslint` clean on every changed file. Both SQL migrations (`docs/database/sprint7.3/01_SCHEMA` + `02_FUNCTIONS`) confirmed deployed and live-verified against the profile fetch (400→200) and the study-time split (sums match combined). Live-verified on the dev server → live Supabase, student (TestOutlook), desktop + 375px mobile: dashboard reorder, Goal Progress two-state + dismiss-persists + Settings round-trip, Study Time split numbers, nav timer chip (start/tap-to-stop/toast), dedicated `/dashboard/study-time` route. **Same-session follow-up:** Quick Actions + My Contributions also removed from the dashboard (confirmed with Anand — Recent Activity is now always the last card); "My Reports" moved to a new `/dashboard/my-reports` page ("Report History") linked from the profile dropdown/mobile menu. Inserted ahead of the `review_events`/`apply_review` engine sprint (still **7.4**). Sprint 7.2 (`c51a145`) — turned out to already be pushed to `origin/main` prior to this update (`git status` showed only 7.3's commit ahead of the remote when 7.3 was pushed); the earlier "not yet committed/pushed" note for 7.2 was stale. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C). → **Next: Sprint 7.4** (the `review_events` / `apply_review` de-risk sprint).
