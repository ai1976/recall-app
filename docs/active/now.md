...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.11 (`fitb` authoring + confidence-gated grading) — zero SQL needed, frontend live-verified end to end — ready to commit.** The last of the 5 D-10-gated types to get a real authoring path. Its whole point (D-13) is a confidence-gated three-way verdict: a matching typed answer is confident-correct (`is_correct=true`), a non-matching one is NEVER treated as wrong — it falls back to the full free-recall self-grade (`is_correct=NULL`). No code path exists that submits `is_correct=false` for this type. Sprint 7.10 (`case_study_mcq`) sits underneath, ✅ zero SQL needed, frontend live-verified end to end — ready to commit; see its own section below. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
