...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.0 (Global Perf & Correctness) CODE COMPLETE 08/09/2026.** Frontend only, no SQL. Clears the isolated perf/correctness debt from the Phase 6 live-verification report (Findings 3/5/6). Super-admin verified on the dev server (→ live Supabase); **student + professor + cold-load verification pending an operator session** before Sprint 7.1. Phase 6 Reskin (6.0 → 6.5) is COMPLETE + on `main` (6.4 `7edc31f`, 6.5 `72693fe`, Task 6.5-D audit `3e392b8`).
...
- **⏳ Pending (operator, on `revisop.com` — dev server → live Supabase, 3 role sessions):** student + professor `/dashboard/notes` fetch counts (`profiles ≤ 3`, nav RPCs ≤ 2); a **cold-load** nav sweep (dashboard ⇄ notes ⇄ flashcards ⇄ progress ⇄ leaderboard, `history.back()`) per role with a persistent `console.error` / `window.error` / `unhandledrejection` / failed-fetch capture — **zero 4xx from the nav/notification family** is the 7.0-B acceptance test; role gating (student = no Manage/admin; professor = Analytics not Manage) + notification/friend badges + mobile sheet render identically all 4 roles; professor Progress "Due Today: 7" → **amber** (was neutral), student Dashboard CTA amber when due>0 / calm-green "all caught up" when due=0; study loop pixel-identical (open one real due card, DO NOT grade). Then disable Speed Insights in Vercel. Then `git commit` + push per the Git rules **before Sprint 7.1**.
