...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.4 (review_events, apply_review, two-measure analytics semantics) ✅ SQL deployed & verified 13/09/2026; frontend code complete, build/lint clean, live-verified against production via a real student account — NOT YET COMMITTED.** All 4 SQL files (`docs/database/sprint7.4/01–04`) deployed to Supabase and confirmed: `04_TEST` 29/29 real assertions PASS (transition maths re-run against `apply_review`, atomicity via a real forced-failure test, both IDOR paths, `rung_before` NULL-on-new-card, `submit_review` compat shim). Frontend (`StudyMode.jsx` → `apply_review`; `Progress.jsx`/`Dashboard.jsx` two-measure widgets) live-verified via a real student account grading real cards against production — both new-card and existing-card/review-session paths returned 200 with correct rung/interval; Progress.jsx's widget confirmed rendering "Recall success 71.9%" / "Answer accuracy: No graded answers yet". Dashboard.jsx's professor widget was code-reviewed but not click-tested live (no professor test account this session). 7.4-E storage projection: ~142 bytes/row measured, no near-term concern — see blueprint.md D-11. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C). → **Next: git commit + push this sprint, then Sprint 7.5** (MCQ-single vertical slice — the architectural proof, and where D-10's professor-only role-gate lands).
...
- **⏳ Not yet committed** — awaiting `git commit` + push before Sprint 7.5 starts.
