...
**Current Phase:** **Phase 8 (B2B growth), Sprint 8.2 — Help documentation corrected to match D-15/D-16 (batch lifecycle) + new question-type guidance for all 8 types + CSV instructions. Content-only, no SQL. See its own section below.** Sprint 8.1 (batch Active/Archived lifecycle) and Sprint 8.0 (batch invite-link joining) underneath, both fully shipped. Phase 7 (question-type epic) closed out with Sprint 7.12 underneath that — see that section. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
