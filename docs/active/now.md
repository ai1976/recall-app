...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.5 (MCQ-single vertical slice) ✅ COMPLETE, live-verified against production, NOT YET COMMITTED.** Sprint 7.4 (review_events, apply_review, two-measure analytics) is ✅ **committed** (`48d6e66`, on `main`). Sprint 7.5: D-10's role-gate SQL deployed & verified live (SQL test + a real UI-level rejection); full authoring → bulk-upload → StudyMode loop live-tested against production with a real professor and a real student account (dev server, `revisop.com`'s live Supabase); "Answer accuracy" confirmed showing real data (50%) on both `Progress.jsx` and `Dashboard.jsx` for the first time ever on this platform. **Bonus:** live testing surfaced and fixed TWO real pre-existing platform bugs on `update_deck_card_count()` — a `visibility` desync (4 decks) and a `target_course` omission (2 decks, 34 public cards invisible to every student for 5 months) — both deployed, verified 0 bad rows remain in SQL, and the visibility fix additionally UI-verified live (Recent Activity + Browse both now surface the previously-invisible content, as a real CA Intermediate student). **Known gap:** the target_course fix's two affected decks are CA Foundation content — no CA Foundation student test account was available to UI-verify that one the same way; worth a quick check next time such an account is on hand. → **Next: git commit + push, then Sprint 7.6.** ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
...
- **⏳ Not yet committed** — awaiting `git commit` + push, then Sprint 7.6.
