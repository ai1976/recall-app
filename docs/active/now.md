...
- **D — `get_following_leaderboard` `[FIX]` (SQL — `02` DEPLOYED 07/09/2026; `03`/live pending).** `docs/database/sprint6.5/01_DIAGNOSTIC` (run — grants = `authenticated`/`postgres`/`service_role`), `02_FUNCTIONS` (**in-place fix, no OUT-column rename**: `#variable_conflict use_column` + `DENSE_RANK()` aliased `rnk` + every ref table-qualified; RETURNS TABLE / SECURITY DEFINER / STABLE / unquoted `search_path` / `auth.uid()` gate / `authenticated` grant all preserved → `LeaderboardWidget.jsx` `row.rank` untouched, SQL shipped alone) — **`CREATE OR REPLACE` ran clean**, so the reconstructed body compiled against the live schema (all referenced columns resolve). `03_TEST` re-issued after `42725 text || "char"` (`::text` casts) + repo `request.jwt.claims` JSON idiom + a semantic-parity check → **9/9 PASS** (shape identical, security preserved, grant `authenticated`, no `42702`, rank monotonic, one `is_self`, students-only, null-session rejected, **weekly stats == `get_friends_leaderboard` for shared users, 0 mismatches**). **✅ Live-verified 07/09/2026** — student Leaderboard → Following tab renders a data row, no 400, no console error. D done.
...
- **⏳ Only follow-up:** operator to `git commit` + push (bash `printf`) — **Phase 6 reskin fully closed** once 6.5 is committed.
