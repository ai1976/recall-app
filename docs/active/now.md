...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.10 (`case_study_mcq` authoring + StudyMode rendering) — zero SQL needed, pre-flight confirmed live (3/3 checks), frontend live-verified end to end — ready to commit.** First genuinely new feature-building sprint since 7.8 — 7.9 was pure hygiene. A shared scenario narrative fans out into N independently-gradeable mcq questions, each its own SRS card, sharing one `batch_id`. Renders through the same shared `AnswerOption`/hybrid-grading branch as mcq (not a new mechanic), with a collapsible scenario block layered on top. Sprint 7.9 (all 4 items) sits underneath, ✅ SQL deployed & verified live (30/30 PASS), frontend live-verified end to end — ready to commit; see its own section below. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
