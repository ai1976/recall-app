...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.9 (schema hygiene, extended scope) — all 4 items ✅ SQL deployed & verified live (30/30 PASS across two test files), frontend live-verified end to end — ready to commit.** Sprint 7.8 landed as `b85afe0`; Sprint 7.9 sits on top of it. Items 1-3: collapsed `test_your_understanding` into `theory`+`subtype` (D-10 correction), split grading-rationale text off `points_to_remember` into a new `explanation` column for graded types, and retired the never-reachable `integrated_case` value (D-12). Item 4 (raised by Anand mid-session, after items 1-3 had already shipped): CA Revision Portal's own schema docs `true_false`/`correct_incorrect` as functionally identical (renderer, fields, everything but the button-label pair) and a real-usage census there found `correct_incorrect` used 6× live vs `true_false`'s 0 — same shape as D-12's `integrated_case` finding, so `true_false` was removed too (D-14). See "Just Completed" below for full detail. ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
