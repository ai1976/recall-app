...
**Current Phase:** **Phase 7 (question-type epic) — Sprint 7.8 (`match_the_following`) ✅ CODE COMPLETE, SQL confirmed needing zero changes, LIVE-VERIFIED end to end — ready to commit.** Sprint 7.7 landed as `1693e1a`; Sprint 7.8 sits on top of it. The first genuinely new interaction mechanic since mcq (left/right pairing, own StudyMode branch) rather than a collapse into existing machinery. `docs/database/sprint7.8/00_DIAGNOSTIC_preflight.sql` was run by the operator 14/09/2026 — `match_the_following` confirmed already live in both the CHECK constraint and the D-10 RESTRICTIVE policy IN-list, exact string match, zero pre-existing rows. Real finding: this sprint needed zero SQL, same as true_false/correct_incorrect in 7.7. Live-verified 14/09/2026 (dev server → live Supabase, real professor + student accounts, account owner typing credentials directly into the driven browser — Claude never saw or entered a password): authoring, both hybrid-grading paths, Browse Study Sets filter, analytics widgets. Also caught and fixed a real pre-existing bug in `Progress.jsx` along the way (stale local question-type label map). ⏳ **Still-open 7.0 operator follow-up:** disable Speed Insights in the Vercel dashboard (7.0-C).
...
- **✅ Ready to commit** — SQL confirmed clean, frontend live-verified end to end on both accounts. Operator to commit + push before Sprint 7.9 starts.
