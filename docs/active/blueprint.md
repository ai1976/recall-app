...
- **Superseded (SRS Ladder Epic, ⏳ SQL not deployed):** scheduling is now driven by `reviews.rung` + `srs_ladder_curves`, computed server-side by `submit_review` (D-2A). `easiness`/`interval`/`repetition` become legacy-cosmetic (still written for continuity, no longer read for scheduling). The client SELECT→UPDATE/INSERT in `handleRating` is replaced by a single `submit_review` RPC call in Phase 3.

**D-2A: Deterministic expanding SRS ladder (SRS Ladder Epic — Phase 0 approved 03/09/2026; Phase 1/2 SQL written, ⏳ NOT deployed)**
