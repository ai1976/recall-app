...
| `get_platform_stats()` | Public headline stats: `student_count`/`educator_count`/`total_flashcards`/`total_notes`, plus `public_flashcards`/`public_notes` (visibility='public' counts). **⏳ SQL authored — not yet deployed** (`docs/database/phase5/13`, S4) | Home.jsx, AdminDashboard.jsx |
...
> **✅ RESOLVED (Phase 5 S4, 2026-07-01).** `Home.jsx` no longer issues any direct anon table reads — the `profiles` role-count reads and the `flashcards`/`notes` public-count reads were deleted. `get_platform_stats()` now returns `public_flashcards`/`public_notes` alongside the existing totals (SQL: `docs/database/phase5/13`, **⏳ not yet deployed** — until deployed, those two fields are `undefined` and render as `0`, not an RLS-filtered wrong number). `Home.jsx` has zero direct `.from()` calls as of this sprint.
...
- **Reads:** `get_user_activity_stats()`, profiles (all), flashcards (all), notes (all via super_admin RLS), reviews (all), content_flags, admin_audit_log, `get_pending_featured_nominations()` + `get_live_featured_content_admin()` (Phase 5 S3) **✅ deployed 2026-07-01**
- **Writes:** profiles (role change, suspend), admin_audit_log (INSERT on every action), `approve_featured_nomination` / `reject_featured_nomination` / `unfeature_content` RPCs (Phase 5 S3) **✅ deployed 2026-07-01**
