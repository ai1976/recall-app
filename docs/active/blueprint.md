...
| 2 | ⏳ **SQL PREPARED (02/07/2026), NOT DEPLOYED** — `is_public` is load-bearing, not vestigial | `notes` public-read RLS uses only `is_public = true` (ignores `visibility`); `flashcards` uses `is_public = true OR visibility = 'public'` (per this table; live `pg_policies` predicate to be confirmed via `09_DIAGNOSTIC` before deploy — see below). Critically, the `friends` visibility tier has **never been enforced in RLS at all** — friends-tier content is only reachable by its owner today. | Landmine Cleanup Sprint L2. Stage A: `docs/database/landmines/10_SCHEMA_rewrite_notes_flashcards_rls_to_visibility.sql` rewrites both public-read policies onto `visibility = 'public'` and adds the first-ever `visibility = 'friends'` policy (bidirectional accepted-friendship join). Verified via the full read-access matrix in `11_TEST_verify_visibility_rls_matrix.sql` (owner/friend/stranger/anon × public/friends/private, both tables — 24 cells, stranger/anon-vs-friends/private are the critical assertions). Stage B (frontend-first): 7 files stripped of `is_public` writes (`FlashcardCreate.jsx`, `MyFlashcards.jsx`, `ProfessorTools.jsx`, `BulkUploadFlashcards.jsx`, `NoteUpload.jsx`, `NoteEdit.jsx`) + 1 read-query migrated (`StudyMode.jsx`'s `.or()` filter) + 2 read/display sites migrated (`MyNotes.jsx`'s filter dropdown and public/private icon) — written, not yet pushed. `12_SCHEMA_drop_is_public_notes_flashcards.sql` drops the column once Stage A is live-verified and the frontend is deployed+verified. **Not deployed — gated on founder running `09_DIAGNOSTIC` first.** |
