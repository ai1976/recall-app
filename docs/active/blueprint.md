...
| NavMobile.jsx | Mobile TOP bar — slimmed (Sprint 7.1) to Wordmark + `FriendsDropdown` + `ActivityDropdown` |
| NavBottomTabs.jsx | **Sprint 7.1** — mobile bottom-tab bar (`md:hidden`, fixed, safe-area padded). Data-driven `TABS`: Dashboard · Review · ＋ (create action-sheet) · Progress · Menu. Consumes `navProps` only, no own fetch. Returns `null` while `useStudySession().inStudySession` (set by `StudyMode` on mount) |
