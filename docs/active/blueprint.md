...
- **Reads:** flashcards (own), flashcard_decks (grouped by batch_id display)
- **Writes:** flashcards (delete), flashcard_decks (visibility toggle cascades)
- **UI:** Cards grouped by batch_id; inline edit; visibility toggle; suspend/skip per card
...
- **Reads:** `get_user_activity_stats()`, profiles (all), flashcards (all), notes (all via super_admin RLS), reviews (all), content_flags, admin_audit_log
- **Writes:** profiles (role change, suspend), admin_audit_log (INSERT on every action)
- **UI:** "Needs Review" content flag card (always visible), user management table, stats cards
...
|----------|---------|--------|--------|
| 🔴 CRITICAL | `vw_study_items` is a **SECURITY DEFINER view** | Runs with the view creator's privileges and **bypasses the querying user's RLS**. Verified: used only inside server-side RPCs, NOT queried by the frontend (`src/`) — so not leaking to clients today. | Recreate with `security_invoker = true` so it respects the caller's RLS — but FIRST confirm the RPCs that read it don't rely on the RLS-bypass (could break review-stats functions). Tighten before any public/anon content surface ships. |
