...
| ✅ **RESOLVED (04/07/2026, L3)** — was 🟡 Sev 3 | Functions flagged **"Function Search Path Mutable"** — no fixed `search_path` means they inherit the caller's at run time (privilege-escalation vector, esp. for SECURITY DEFINER). | **DONE (L3, `16`–`18`):** `16_DIAGNOSTIC` found **50 our-owned unpinned functions** (44 SECURITY DEFINER + 6 not; excludes extension-owned + already-pinned like `get_public_deck_preview` / `skip_topic_cards`), confirmed zero unqualified extension-function deps (Block 3 empty), and that `anon`/`authenticated` cannot `CREATE` in `public` (Block 4 false/false). Chose **behavior-preserving pin `SET search_path TO 'public, extensions'`** (matches the live role default per Block 1) over strict `''` — no body rewrites, zero runtime change. `17_SCHEMA` applied all 50 (SECDEF-first, one transaction, generator-reviewed). `18_TEST` 3/3 PASS: 0 unpinned remain, every extension-using fn's pin includes `extensions` `[CRITICAL]`, and a SECDEF fn still executes post-pin. Rollback = `ALTER FUNCTION … RESET search_path`. |
