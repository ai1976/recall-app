...
| `get_following_leaderboard()` | Top 20 followees + caller by weekly reviews. **Sprint 6.5 `[FIX]` (`02` deployed 07/09/2026; `03`/live pending): ambiguous `rank` (`42702`) 400s the "Following" tab — fixed in place (no signature change) via `#variable_conflict use_column` + window alias `rnk`; `docs/database/sprint6.5/`.** | LeaderboardWidget.jsx |
