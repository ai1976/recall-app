...
| `get_following_leaderboard()` | Top 20 followees (students) + caller by weekly reviews, `DENSE_RANK` over the full cohort. Directional follow-join: `follows WHERE follower_id = auth.uid()` → `followee_id` (no `friendships`, no reciprocal clause). **Sprint 6.5 `[FIX]` (`02` deployed 07/09/2026; `03_TEST` 9/9): ambiguous `rank` (`42702`) 400'd the "Following" tab — fixed in place (no signature change) via `#variable_conflict use_column` + window alias `rnk`. Task 6.5-D audit (`04_AUDIT_*`): original body unrecoverable from git; reconstruction diffed vs the Sprint 3.5 contract @ `071395d` — follow-join / filter / window all MATCH → faithful, no `[FIX]`. Live membership set-equality assertion scripted, operator-run pending. `docs/database/sprint6.5/`.** | LeaderboardWidget.jsx |
