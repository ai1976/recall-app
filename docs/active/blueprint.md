...
| NavBottomTabs.jsx | **Sprint 7.1**, badge added **7.2-F** — mobile bottom-tab bar (`md:hidden`, fixed, safe-area padded). Data-driven `TABS`: Dashboard · Review (due-count pill) · ＋ (create action-sheet — Upload Note / Create Flashcard / Create Group / Bulk Upload) · Progress · Menu. Consumes `navProps` only, no own fetch. Returns `null` while `useStudySession().inStudySession` (set by `StudyMode` on mount) |
