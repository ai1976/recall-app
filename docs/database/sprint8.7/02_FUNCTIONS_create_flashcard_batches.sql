-- Name: [FUNCTIONS] Sprint 8.7.1 — create_flashcard_batches (atomic provenance + card creation)
--
-- Description: SECURITY DEFINER RPC that is, after 01_SCHEMA is deployed, the ONLY
-- way to create a flashcard batch that has a valid provenance record (direct client
-- INSERT into flashcards now requires a pre-existing provenance row per batch_id).
-- One call may mint multiple batch_ids sharing one declared source (case_study_mcq
-- uploads mint one batch_id per case group). All-or-nothing: any validation failure
-- or insert failure aborts the whole call — no partial provenance, no partial cards.
--
-- *** SECURITY-CRITICAL — READ BEFORE MODIFYING ***
-- The function owner is postgres, which owns public.flashcards/notes/profiles. Per
-- Step 0 diagnostic (section 2c), FORCE ROW LEVEL SECURITY is NOT set on flashcards
-- or flashcard_batch_provenance. That means as a SECURITY DEFINER function running
-- as table owner, this function is COMPLETELY INVISIBLE to RLS — none of the policies
-- in 01_SCHEMA apply to it. Every authorization guarantee a direct client insert would
-- have gotten from RLS must be reproduced explicitly in this function body:
--   1. user_id is derived from auth.uid() server-side — NEVER from caller input.
--   2. The D-10 verdict-bearing question_type gate (flashcards_gate_verdict_types_insert)
--      is reproduced via is_professor_or_admin() before any insert.
-- Do not add a code path that trusts a caller-supplied user_id, contributed_by,
-- creator_id, content_creator_id, or is_verified value.
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.flashcards (
      ...
      correct_answer, points_to_remember, explanation, subtype
    )
    SELECT
      ...
      c->'points_to_remember', c->'explanation', c->>'subtype'
...
COMMENT ON FUNCTION public.create_flashcard_batches(text, text, jsonb) IS
...
-- (confirmed live via 03_TEST/T8b — REVOKE ... FROM PUBLIC alone left anon able to
-- execute this function). Deliberately NOT granted to anon — no verified reason for
-- anonymous batch creation.
