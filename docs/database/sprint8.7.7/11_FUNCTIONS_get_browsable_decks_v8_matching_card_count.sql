-- Name: [FUNCTIONS] get_browsable_decks v8 — matching_card_count
--
-- Description: Sprint 8.7.7 (D-26). Adds ONE additive return column, matching_card_count, at the END of the
-- RETURNS TABLE. It is the number of cards in the deck that the VIEWER may see (identical visibility predicate to
-- visible_card_count) AND that have question_type = p_question_type. With p_question_type NULL it equals
-- visible_card_count. card_count (= visible_card_count, the deck's whole visible total) is UNCHANGED, as is the deck
-- inclusion rule, provenance columns, ordering, SECURITY DEFINER and search_path.
--
-- Why: with a type filter on, Browse Study Sets summed card_count, so "N cards available" / "Study All (N)" showed
-- deck totals and a mixed-type deck was counted under every filter (live: 699+397+129=1,225 vs 1,091 for All Types).
--
-- Reproduced verbatim from v7 (docs/database/sprint8.7.4/02_FUNCTIONS_get_browsable_decks_v7_provenance.sql) with
-- three additive edits, each marked "v8": the new RETURNS TABLE column, the new SELECT-list column, and one new
-- aggregate in the existing vc lateral. As in v5/v6/v7, changing the return type means CREATE OR REPLACE cannot be
-- used — the one-arg overload is DROPped first. DROP + CREATE reset the function's ACL to the schema defaults:
-- compare 10_DIAGNOSTIC (before) with 12_TEST T1 (after).
--
-- Deployment: run 10_DIAGNOSTIC first, run THIS file alone, then 12_TEST, then confirm before the frontend push.
-- Rollback: 13_FUNCTIONS_ROLLBACK_restore_get_browsable_decks_v7.sql (frontend must be rolled back first if deployed).
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    vc.matching_card_count                          -- v8: viewer-visible cards of the requested type (= visible_card_count when no filter)
