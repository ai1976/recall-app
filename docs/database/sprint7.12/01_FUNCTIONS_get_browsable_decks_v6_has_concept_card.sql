-- Name: [FUNCTIONS] get_browsable_decks v6 — has_concept_card flag
-- Description: Sprint 7.12 adds one additive, non-breaking-for-callers return
-- column, has_concept_card BOOLEAN, so ReviewFlashcards.jsx (Browse Study
-- Sets) can render a secondary "Read Concepts" action on a deck tile without
-- an extra round-trip per deck. True when the deck has at least one
-- concept_card row VISIBLE TO THE VIEWER — same visibility predicate as the
-- existing vc.visible_card_count lateral (owner sees own private cards,
-- public visible to all, friends-visibility visible to accepted friends,
-- admins see everything, group-shared decks visible to group members).
-- Computed in the SAME lateral subquery that already computes
-- visible_card_count (bool_or over the same filtered row set) rather than a
-- second EXISTS, to avoid re-scanning the deck's cards twice per row.
--
-- CORRECTION (inherited from v5's own note): adding a return column changes
...
-- DROPped explicitly first. No arity change this time (still one param), so
-- there is only one overload to drop, not two.
--
-- Every existing caller (ReviewFlashcards.jsx's `supabase.rpc('get_browsable_decks', ...)`
-- call) is unaffected by the new column — callers that don't read
-- has_concept_card off the returned rows see no behavior change at all.
--
-- Reproduced verbatim from v5 (docs/database/sprint7.6/01_FUNCTIONS_get_browsable_decks_v5_question_type_filter.sql)
-- with one additive SELECT-list column and one additive lateral-subquery column.
...
BEGIN
  ...
  -- plus (Sprint 7.12) whether any of those visible cards is a concept_card.
  CROSS JOIN LATERAL (
...
-- 02_TEST's role-impersonation pattern, or the app itself):
