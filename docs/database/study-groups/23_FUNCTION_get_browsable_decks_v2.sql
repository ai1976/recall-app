-- Name: [FUNCTIONS] Get Browsable Decks v2 (with invitation status filter)
...
-- SECURITY: Filter out 'invited' status. Only 'active' members can view content.
...
BEGIN
  ...
  WHERE
    ...
    AND (
      ...
      -- Group-shared decks
      -- SECURITY: Only 'active' members can view content. 'invited' users are excluded.
      OR EXISTS (
