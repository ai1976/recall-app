...
### [16/09/2026] Bulk Upload CSV silently downgrades `match_the_following`/`concept_card` rows to a plain flashcard instead of rejecting them — ⚠️ OPEN (documented, not fixed — out of scope for a content-only sprint)
- **Found while:** verifying the real CSV column set for the new Help documentation against `BulkUploadFlashcards.jsx`'s parser.
- **Symptom:** `RECOGNIZED_QUESTION_TYPES` (`src/pages/dashboard/BulkUploadFlashcards.jsx:35`) is `['mcq', 'correct_incorrect', 'theory', 'case_study_mcq', 'fitb']`. The parser's fallback (`flashcard.question_type = RECOGNIZED_QUESTION_TYPES.includes(cleanQuestionType) ? cleanQuestionType : 'flashcard'`, line 435) means a row with `question_type` set to `match_the_following` or `concept_card` — or any typo, e.g. `mcqq` — is never flagged as an error. It is silently created as a plain `flashcard` using whatever `front`/`back` text was supplied, with every type-specific column (options, scenario, key terms, etc.) discarded.
- **Root Cause:** the fallback was written as a permissive "unknown type → safe default" for genuinely blank cells (the common case — most rows have no `question_type` at all), but it does not distinguish "blank" from "recognized-but-unsupported" from "misspelled." All three fall into the same silent downgrade.
- **Impact:** a professor trying to bulk-create match_the_following or concept_card content (both unsupported by CSV, manual-authoring only) gets no error message — just a pile of unexpectedly-plain flashcards missing the structure they typed into the wrong columns. A simple typo in `question_type` for any row has the same silent effect.
- **Fix (not built this sprint):** the parser should distinguish a genuinely blank cell (→ flashcard, current behavior, correct) from a non-blank but unrecognized value (→ a parse error naming the bad value, like every other missing-required-field case in this same function) rather than treating both identically.
- **Mitigation shipped this sprint:** documented explicitly in the new `bulk-csv-basics` Help section ("If question_type is spelled anything other than flashcard, theory, mcq, correct_incorrect, case_study_mcq, or fitb... the row is not rejected. It is silently created as a plain Flashcard instead.") so at least the behavior is discoverable before someone hits it blind.
- **Status:** ⚠️ OPEN — real defect, disclosed, not fixed. Candidate for a small follow-up sprint (parser-only change, no schema impact).
