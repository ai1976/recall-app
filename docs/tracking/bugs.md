...
- **Status:** ✅ Fixed and live-verified 25/09/2026 (record H, `CAFA-AUD-SM1-C05-CS01-TQ01`) — scenario survives CSV → DB → render with no duplication into `front_text`.

### [25/09/2026] Bulk-upload silently destroyed `[[TABLE]]` rich content in `front`/`back` via newline collapse — ✅ FIXED & LIVE-VERIFIED
- **Found:** live Stage 8 proof for records F/G/H/I. The first upload attempt rendered literal `[[TABLE]]`/`|` markers as plain text instead of a table — DB inspection showed `front_text`/`back_text` had zero `\n` characters despite the source CSV cells containing real newlines.
- **Root cause:** `BulkUploadFlashcards.jsx`'s whitespace-normalization step, predating this sprint, ran `flashcard.front = flashcard.front.replace(/[\r\n]+/g, ' ').trim()` (same for `back`) unconditionally on every row, collapsing all internal newlines to spaces. This destroyed `[[TABLE]]` marker structure — the parser requires the marker on its own trimmed line — before the value was ever stored. `scenario` uses a different, trim-only code path and was unaffected, which is why Fixtures A–E's earlier proof (table content only in `scenario`) never surfaced this.
- **Impact:** any bulk-uploaded `front`/`back` field containing a `[[TABLE]]` block (or any other newline-dependent multi-line formatting) would have its structure silently flattened on import — the renderer's fail-closed fallback masked this as "plain text" rather than an error, so it would not have been obvious without inspecting the stored value directly.
- **Fix:** replaced the blanket newline collapse with a plain `.trim()`, matching `scenario`'s existing treatment. No behavior change for genuinely single-line content; multi-line plain text was already rendered with `whitespace-pre-wrap`, so this also fixes a latent (undetected) case where legitimate multi-paragraph front/back content was being flattened on import even before this sprint.
- **Status:** ✅ Fixed and live-verified — deleted the 7 malformed disposable cards, re-uploaded from the same CSV with the fix live, confirmed `front_text`/`back_text` now retain newlines and render real `<table>` elements end to end (Practice Mode desktop + mobile).
