...
### Deviations
- **Task D `02_FUNCTIONS` is a reconstruction** — built in the no-DB-access build session from `DATABASE_SCHEMA.md` + the live `get_friends_leaderboard` sibling. Deployed 07/09/2026; it compiled clean against the live schema and `03_TEST`'s semantic-parity assertion (weekly stats == `get_friends_leaderboard` for shared users → 0 mismatches) passed, so it is behaviourally faithful. A diff against the original `pg_get_functiondef` (from `01` step 1) would be belt-and-braces if still available.
